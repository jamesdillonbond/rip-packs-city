...
- ⚠ **`?? 0` on a supabase count, `|| 1` as a divide-guard, and a DEFAULTED DB COLUMN beside a NULL `*_checked_at` are the fabricated-value shapes.** supabase-js **RETURNS** errors rather than throwing, so `?? 0` publishes a measured zero; `|| 1` on a $0 baseline rendered **"↑ 50000.0% / 30D"**. ⭐ **The DB form's tell is a PERFECT CORRELATION — if `never_checked AND value=true` is EXACTLY 0, the value is the DEFAULT** (#112: 1.16M rows claimed "not locked" unchecked; a sibling hardcode discarded 370 REAL ones). ⛔ **A function projecting such a value must project its PROVENANCE too, or no reader downstream can be honest.** `no-fabricated-divisor-ratchet` is a **ban at population zero**.

...
- **Pinnacle per-render item 2 — FMV recompute PREPARED, NOT SHIPPED (`b560d0b`).** Pricing logic, review-gated per the handoff guardrail. Inert `pinnacle_fmv_snapshots.render_id` column added (`audit_20260606_pinnacle_fmv_snapshots_render_id_column`); full proposal + validated evidence (measured **~16x within-legacy-key spread**: Digital-Display Kylo Ren Helmet $278 vs $17–32 set-mates) + atomic reader-cutover plan in [docs/handoff-2026-06-06-pinnacle-per-render-fmv-recompute-review.md](docs/handoff-2026-06-06-pinnacle-per-render-fmv-recompute-review.md). **Trevor reviews + applies the writer swap.** Queued PIN-FMV-REKEY.

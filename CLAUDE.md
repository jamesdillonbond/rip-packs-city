...
- **`audit_20260525_promote_unmapped_sales_skip_null_price`** — guards `promote_unmapped_sales` against promoting rows whose price is NULL. The V1-Dapper decode path writes 84 rows with `price_extraction='v1_tx_decode_budget_exhausted'` and `price_usd IS NULL` per backlog snapshot; without the guard these would land in `sales` with `price_usd=NULL` once their edition mapping resolved. Price-NULL rows now wait in `unmapped_sales` until their tx is re-decoded.

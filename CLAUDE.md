...
- **Queued (this program):** authoritative per-parallel circulation (GQL searchEditions); bulk per-parallel art fill; Phase 2 parallel-aware edition pages (`::` slug + Parallels module); Phase 3 per-parallel owners MV + serial-FMV model; Phase 4 guard teardown post-convergence. Full detail + revert paths: [docs/overnight/ledger.md](docs/overnight/ledger.md).

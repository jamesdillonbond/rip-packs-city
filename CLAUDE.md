...
Trevor-approved 4-part visual upgrade, all shipped (`4c25ffc`+`23fdef8`+`816a095`, prod READY): (1) **brand typography** — Barlow Condensed Black + Share Tech Mono TTFs vendored under `public/fonts` (OFL licenses committed) and embedded subset via `@pdf-lib/fontkit`, with Standard-14 fallback so a font hiccup can never 500 the export; (2) **holo slab panels** — satori-rendered per-accent backgrounds (dark gradient, tier-colored glow, shadow well) replace flat rectangles, cached per accent; (3) **serial = hero stat** at 18pt, gold when special, full **gold slab for 1-of-1s**; (4) **footer QR** deep-linking `/profile/<u>` (pure-JS `qrcode`). Also fixed: relative Pinnacle thumbnail paths absolutized + browser UA on art fetch. **Found + queued:** `assets.disneypinnacle.com` 403s ALL datacenter egress (documented in the pinnacle-image route) ⇒ Pinnacle renders can't be embedded in ANY server-generated surface; interim = branded tier-colored pin-crest placeholder tile; durable lever = pinnacle-proxy worker passthrough (CC/wrangler, PINNACLE-ART-DATACENTER-BLOCK in the ledger). New deps: `@pdf-lib/fontkit`, `qrcode`. **v7 (`9404921`+`bb0a119`):** per-collection watermark motifs (RPC-original monoline shapes, tier-tinted, peeking beside the art) + collector notes line. **Same session: BADGE-SWEEP-COVERAGE-GAP DRAINED** — per-set runs of `backfill-badges-from-sets` via a temp (now 410-stubbed) edge-fn bridge took missing canonical TS badge rows 3,163→69 (+2,849 rows; 852 recovered Top Shot Debuts — the Lillard-Cosmic class, site-wide). Residual: set 149 chunk-write error (CC), no-uuid sets. **Revert:** `git revert bb0a119 9404921 816a095 23fdef8 4c25ffc`.

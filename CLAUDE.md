...
- ⚠ **Name the caller before you touch the function** — an afternoon went into one with **zero** callers. **EIGHT sources, and the last two (cron-job.org, this box's Task Scheduler) are INVISIBLE from a sandbox**; a TRIGGER function has no textual caller and `pg_stat_statements` misleads BOTH ways. Full list: [cron-and-schedulers.md](docs/reference/cron-and-schedulers.md).
...
- ⚠ **`check_*` functions have MIXED return shapes.** A jsonb-array one returns `count(*) = 1` when CLEAN — read the array LENGTH; a SETOF one returns **zero rows** when clean. **Check the return type before interpreting the count** (which is which: database.md).
...
- ⛔ **A UNIQUE INDEX ON A PARTITIONED TABLE MUST CONTAIN THE PARTITION KEY — a dedupe key that omits it is IMPOSSIBLE at the parent, so index the PARTITIONS** (`sales` is `RANGE (sold_at)`; 33,000 duplicates slipped past, #68). ⭐ **The tell was EXACTLY zero violations, not a small count.** ⚠ **A keep-rule is a CLAIM about the data and a new constraint must be PROVEN to reject.** [database.md](docs/reference/database.md)
- ⚠ **A `*_at` name is not its contract — read `col_description()` before treating a timestamp as freshness.** 3 such columns here; one cost a retraction.
...
- **`apply_migration` for DDL; `execute_sql` for reads/verification.** ⚠ `CREATE INDEX CONCURRENTLY` cannot run inside `apply_migration` (transactional) — ⭐ **but it DOES run via `execute_sql`** (a 105 MB index, 09-09); the old "pg_cron only" claim is RETRACTED. Record it after with an `IF NOT EXISTS` migration so it is not fileless. FMV writes are delete-then-insert, NEVER upsert. Always query `information_schema.columns` before writing a route handler; Supabase MCP multi-statement queries return only the last result.

...
- 🚨 **Needs TREVOR, not code — three:** the **DEFEATED credential purge** (public branch `claude/todo-implementation-e4tib3` still carries the pre-purge blob, re-verified 08-27 — triage `ee94c8a2a`, delete via the GitHub UI, GC, **rotate regardless**, #22) · the three board-MV cron jobs' 600 s timeout (#27) · **approve the replacement 2-hourly Routine's card in the DESKTOP app, repo folder attached** — today's is cloud-only so it CANNOT PUSH and the binding is immutable (#55).

...
>
> **KEEPING IT UNDER: the limit is on CHARACTERS and `wc -c` counts BYTES.** ⚠ It runs several hundred bytes longer than it is characters (`⚠ — ·` are multi-byte) and lives inside that gap — `wc -c` once read **40,086 on a file whose true length was 39,610**. `wc -m` is platform-dependent. **The BINDING number is Node's `.length` — a JS harness enforces the limit — count with `node -e`, not `wc`** (4-instrument table: [tooling-gotchas.md](docs/reference/tooling-gotchas.md)). ⚠ **The file is at its size EQUILIBRIUM, so a new durable rule must DISPLACE one** — put the displaced text **verbatim** in the matching `docs/reference/*.md` with a one-line pointer from here. **Over the limit the whole file is flagged and stops being trustworthy context.**
>
...
- ⚠ **Name the caller before you touch the function** — an expensive-looking function is not a cost until you have; an afternoon went into one with **zero** callers. Require SIX sources: `pg_proc.prosrc`, `pg_views.definition`, `cron.job.command`, `pg_trigger`, a full-repo grep — ⚠ **and the Cowork artifacts' HTML, outside BOTH repo and catalogue** (8 artifact-only views; a sweep without it breaks 3 live boards). ⚠ **A TRIGGER function has no textual caller, and `pg_stat_statements` misleads BOTH ways** ([database.md](docs/reference/database.md)). ⚠ **An EDGE function needs a SEVENTH: cron-job.org** — invisible to all six *and* `cron.job`; the 08-22 canary greps ZERO callers, runs every 30 min.
...
- **The sports-proxy `403` — highest-value open item; TWO causes, NOT a secret.** ESPN: 200 residential / 403 from Supabase egress ⇒ proxy it. `cdn.nba.com` 403s **residentially too** ⇒ ⛔ **UA-refresh and 403-retry are measured useless.** ⚠ **Restoring ESPN alone will NOT refill `nba_players`** — the roster fetch is SLATE-GATED and the season ended 08-04, so **do not read a flat catalogue as the fix having failed** until ~October. `match-topshot-players` queues behind it. Mechanism + the unbuilt 30-team sweep: #8 in [known-issues.md](docs/reference/known-issues.md).

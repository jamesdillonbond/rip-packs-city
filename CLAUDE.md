...
- Phase D — relocate Flow-specific primitives under `lib/chains/flow/` with shim re-exports at old import paths; zero caller breakage. **Plan committed 2026-05-30 (`ce19f35` + `4938fbb`):** [docs/handoff-phase-d-lib-chains-flow-reorg.md](docs/handoff-phase-d-lib-chains-flow-reorg.md) — 833 `@/lib/...` imports across the repo means shims are bulletproof for every caller. Verified caller counts per Tier-1 file (top fan-in: `lib/flow.ts` ~25, `lib/topshot.ts` 16). Critical trap baked in: `lib/flow.ts` is the only file with `export default`; its shim needs `export { default } from "@/lib/chains/flow/flow"` alongside the `export *` or default-importers silently break. Stay-at-top-level list includes `lib/evm-rpc.ts` (Base/EVM plane — must NOT land in the Flow dir).
...
**Pending DB (gated on Phase D shipping):** Phase F — `ALTER TABLE collections ALTER COLUMN chain DROP DEFAULT` once `lib/collections.ts` enforces chain at the type level.

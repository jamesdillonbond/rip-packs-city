...
- **Item 9 — pack-events-ingest re-wedge FIXED (`bee4da2`, worker `2026-06-11-p25-chunked-write-upserts`, deployed via wrangler).** The `topshot_pack_purchases` cursor was deterministically wedged on a 1796-row `pack_purchases` `INSERT…ON CONFLICT` statement timeout (green cron, frozen cursor). All three writes (`pack_purchases`/`pack_rips`/`moment_acquisitions`) now chunk at 400 rows; `ignoreDuplicates` keeps it idempotent. Deployed-vs-HEAD drift checked first (was p24, no drift).

...
- 🚨 **Needs TREVOR, not code — two:** the **credential-purge residue** (the leak branch was DELETED from origin 09-07; what is left is asking GitHub to **GC the unreachable objects** — the blob is sha-fetchable until they do — and **rotating regardless**, #22) · ⛔ **the 2-hourly-Routine approval card DOES NOT EXIST — both are DISABLED, replaced by a desktop-local task invisible to `RemoteTrigger`; done-check is a run that pushes itself (#55).**

...
- **The sports-proxy `403` — ⛔ "PROXY ESPN" IS MEASURED DEAD** (ESPN 403s residentially too; the "no alert" gap is a MYTH — deliberately suppressed). Still dead 08-27: `sync-nba-projections` **0 ok / 16 in 48 h**, `all_upstreams_failed`. Bullet + discriminator: #8 in [known-issues.md](docs/reference/known-issues.md).
...
- 🚨 **Needs TREVOR, not code — three:** the **DEFEATED credential purge** (public branch `claude/todo-implementation-e4tib3` still carries the pre-purge blob, re-verified 08-27 (REST branch listing) — triage `ee94c8a2a`, delete via the GitHub UI, GC, **rotate regardless**, #22) · the three board-MV cron jobs' 600 s timeout (#27) · `atlas-proxy` (#20) — ⚠ **now the BINDING half**: #30's DB timeouts are fixed (index, 08-26) and the residual fails are **6/15 `egress_blocked`** (08-27).
- **ONE measured-but-unshipped DB fix left, blocked on a DECISION not a diagnosis:** `compute_pack_ev_per_edition_weighted`'s `fmv_current` leg (**18,766 vs 1,046,192 buffers**, re-seeds a pinned fixture — Trevor's call; still unshipped in live `prosrc` 08-27). ⭐ A sibling's stated blocker turned out to be a MEASUREMENT, not a decision — **re-read a "blocked" item's blocker before inheriting it.**

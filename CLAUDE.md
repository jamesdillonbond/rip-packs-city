...
- **Challenges honesty — `ends_at` display filter shipped** (migration `audit_20260716_challenges_ends_at_display_filter`; carried LOW closed). Live finding that promoted it from cosmetic: ALL 31 status='active' challenges had actually ended — `ingest-topshot-challenges` fetches 0 once a wave ends and never flips absentees to inactive, so the page showed a dead wave as active for 2 days. Guard added to the `ch` CTE in `get_active_challenges` + `get_challenge_plan`. **NEW QUEUED (CC, ingest-owner):** add an ends_at/absence status-flip to `ingest-topshot-challenges` (the filter papers over statuses that never expire). **Revert:** re-apply prior fn defs.

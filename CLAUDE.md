...
- ⚠ **A `*_at` is not its contract, and its stamp can come from a DIFFERENT lane than owns the VALUE** — TS `edition_offers.updated_at` fresh, `highest_offer` unwritten 13d (#81).
...
- **`apply_migration` for DDL; `execute_sql` for reads/verification.** ⚠ `CREATE INDEX CONCURRENTLY` cannot run inside `apply_migration` (transactional) — ⭐ **but it DOES run via `execute_sql`** (105 MB index, 09-09; the "pg_cron only" claim is RETRACTED). Record it with an `IF NOT EXISTS` migration so it is not fileless. FMV writes are delete-then-insert, NEVER upsert. Always query `information_schema.columns` before writing a route handler; Supabase MCP returns only the last result of a multi-statement query.

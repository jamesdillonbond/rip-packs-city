...
🚨 **`git revert <sha>` paths recorded BEFORE 2026-08-03 no longer resolve** — that day's `filter-repo` rewrote every pre-purge sha; find the commit by MESSAGE (`git log --grep=`). The **DB half of every revert path is unaffected**. 🚨 **The purge is STILL defeated although the branch is GONE:** `claude/todo-implementation-e4tib3` was deleted from origin 2026-09-07, but its blob stays fetchable BY SHA until GitHub GCs it. Operator-only: #22.
...
- **The worst sub-classes:** an account-level false claim; a page that **LOADS state and WRITES IT BACK** (a failed read there is a *delete*); an **alert**; a **guard** (`?? 0` fails it *open*); an empty state that **CONCLUDES**; a **SWEEP whose `ok` means it COMPLETED, not that its LANES worked**. Cases: key-files-and-honesty.md
- ⚠ **`?? 0` on a supabase count, `|| 1` as a divide-guard, and a DEFAULTED DB COLUMN beside a NULL `*_checked_at` are the fabricated-value shapes.** **ANY unwrapper that RETURNS on failure** — supabase-js, an MCP `extractRows` (#114) — publishes a measured zero AND leaves every downstream `catch` dead; `|| 1` on a $0 baseline rendered **"↑ 50000.0% / 30D"**. ⭐ **The DB form's tell is a PERFECT CORRELATION — if `never_checked AND value=true` is EXACTLY 0, the value is the DEFAULT** (#112: 1.16M rows claimed "not locked" unchecked; a sibling hardcode discarded 370 REAL ones). ⛔ **A function projecting such a value must project its PROVENANCE too, or no reader downstream can be honest.** `no-fabricated-divisor-ratchet` is a **ban at population zero**.
...
- ⚠ **A permanently-red or permanently-zero instrument is indistinguishable from a broken one at a glance** — check the LOG, not the badge, and **prove a watcher can see a FAILURE** before relying on it. ⚠ **An ALARM SHARING ITS SUBJECT'S SCHEDULER is no alarm** — GitHub shed every tick 2.8h, taking all four watchers (#80).
...
- ⚠ **An exclusion justified by ANOTHER instrument is a claim about it — check that one can SEE the property**, and know what NOTHING here measures (LAYOUT, the BUILT BUNDLE): [testing-and-ci.md](docs/reference/testing-and-ci.md).

Full detail: [docs/reference/testing-and-ci.md](docs/reference/testing-and-ci.md).

### Measurement discipline

- ⚠ **A filed FINDING is a hypothesis — re-derive what it measured before acting** (several refuted; one "fix" would have made an accurate surface INaccurate). ⚠ **So is a filed DECISION NOT TO ACT, and that is the one nobody re-checks — the tell is a cost stated with no number in it.** ⚠ **A WEAK reason CROWDS OUT the strong one and becomes PERMISSION when it dissolves.** ⚠ **Re-TEST a stated exit condition, never re-read it** — a "once cleared" 114 was 5.
...
- ⚠ **`SET statement_timeout` on a function is INERT on pg_cron; via PostgREST only a HIGHER one applies (gateway cap ~120 s).** ⛔ 122 of 195 are load-bearing — do NOT strip.
...
- ⛔ **A UNIQUE INDEX ON A PARTITIONED TABLE MUST CONTAIN THE PARTITION KEY — one that omits it is IMPOSSIBLE at the parent, so index the PARTITIONS** (`sales`, 33,000 dupes, #68). ⭐ **The tell was EXACTLY zero violations.** [database.md](docs/reference/database.md)
- ⚠ **A `*_at` name is not its contract — and the contract is its WRITER's, so REPLACING a writer silently REDEFINES the column while the name holds.** `edition_offers.updated_at` went confirmed→changed on 08-28; 3 surfaces kept the old reading. `col_description()` first: [database.md](docs/reference/database.md).
...
- **`apply_migration` for DDL; `execute_sql` for reads/verification.** ⚠ `CREATE INDEX CONCURRENTLY` needs `execute_sql` and runs **only if it FINISHES inside the 60 s client cap** — a timeout ABORTS it, leaving an `indisvalid=false` index (09-13). Record it with an `IF NOT EXISTS` migration so it is not fileless. FMV writes are delete-then-insert, NEVER upsert. Always query `information_schema.columns` before writing a route handler; Supabase MCP returns only the last result of a multi-statement query.

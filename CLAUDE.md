...
- ⛔ **An ADD-ONLY refresh never retires what its source STOPPED returning** — the row asserts its last sighting for ever (23 phantom "unopened" packs in users' own inventories). Trust an ownership claim only at or after the walk that CONFIRMED it, keep that floor where a re-dispatch cannot clear it, and fail OPEN when no clean walk exists. ⭐ **100% of them were ONE status class — that names the missing re-check path.**
...
- ⚠ **Displaced 09-20 to [database.md](docs/reference/database.md) (verbatim, end of file): `check_*` MIXED return shapes — THREE of them, and a jsonb OBJECT makes `jsonb_array_length` ERROR, while LENGTH IS NOT SEVERITY · UNIQUE INDEX on a PARTITIONED table · `pipeline_runs` ~73h retention.**

...
- ⚠ **THE WRITE SIDE, which the table above does not cover (R120):** a swallowed write error + a hardcoded `ok=true` + a count that CANNOT GO DOWN publishes a FAILED WRITE as a successful run — `if (error) console.log(…)` beside `logRun(…, true, null, …)` hid a statement-aborting FK rejection for **66 days**. **DERIVE `ok` from whether the write landed, pair every count with its own `_error`, and make a count mean rows WRITTEN** (`rows.length` under a written-sounding name is a fabricated value). ⛔ **An `insert` awaited with NO destructuring is unreadable by construction, not merely ignored.**
...
- ⚠ **Displaced 09-20 to [cron-and-schedulers.md](docs/reference/cron-and-schedulers.md) (verbatim, end of file): DISCOVERY must not double as the REFRESH list · an ELIGIBILITY count is not a GAIN count.**
...
- ⚠ **Displaced 09-20 to [database.md](docs/reference/database.md) (verbatim, end of file): `check_*` MIXED return shapes · UNIQUE INDEX on a PARTITIONED table · `pipeline_runs` ~73h retention.**

...
**Panini (in prep, INDEPENDENT):** infrastructure staged inert 2026-07-16 (`panini_editions`/`_fmv_snapshots`/`_pack_state` tables + read views + `app/api/cron/panini-ingest` route + `scripts/ingest-panini-runner.mjs`). Its go-live gate is its OWN data flowing (a residential logged-in box running the runner) — NOT Candy. See [docs/strategy/panini-roadmap-2026-07-16.md](docs/strategy/panini-roadmap-2026-07-16.md).
...
The `[collection]` dynamic segment serves all 5 published collections: NBA Top Shot, NFL All Day, LaLiga Golazos, Disney Pinnacle, UFC Strike. Each collection's tab set is its `pages: [...]` array in `lib/collections.ts` (verified 2026-07-16):

- **All 5 published:** `overview`, `collection`, `sniper`, `analytics`.
- **`market` + `packs`:** all except UFC.
- **`sets`:** all except Pinnacle.
- **`pack-sniper`:** Top Shot + AllDay only.
- **`challenges` + `hot-floors`:** Top Shot only.

Top Shot also has Fast Break and RTR (Road to the Ring) game features — `fast-break/` + `road-to-the-ring/` route dirs, NOT registry tabs (they appear in no `pages` array). Entity/detail routes under `[collection]` (also not tabs): `edition`, `moment`, `set`, `series`, `player`, `team`, `pack`, `profile`. There is NO standalone `badges` tab — the page type lingers in `lib/collections.ts` but no collection lists it and `/[collection]/badges` 307-redirects to `/overview` (badges render inline on edition/moment pages via `get_edition_badges_unified`).
...
- **CI ratchet (do not defeat).** `vitest.config.ts` `thresholds` sit just below the live baseline (2026-07-17 after the deep-loop coverage program, waves 1–8: thresholds ~68.0 stmts / 54.1 branch / 74.0 funcs / 70.3 lines vs actual ~68.2 / 54.3 / 74.2 / 70.5 — up from ~45/37/53/47 at the program's start; the full comment history in `vitest.config.ts` records every wave's numbers), so a coverage **drop** fails CI while normal noise passes. **Raise these as coverage climbs; NEVER lower them to make a red build pass** — but keep a real ~0.1–0.2 buffer under actuals: on this multi-session repo, concurrent pushes add uncovered code and a zero-margin threshold reds CI on otherwise-green work (lesson `47f901a1`). CI job is `unit-tests` in [.github/workflows/ci.yml](.github/workflows/ci.yml), which runs `npm run test:coverage`.
- **Cadence tests** — `npm run test:cadence` extracts inline Cadence (`scripts/extract-cadence.mjs`) and runs `flow cadence lint` against `tests/cadence/fixtures/`. Gated in CI (`cadence-lint` job, needs `flow dependencies install`). See `docs/cadence-testing.md`. Separately, a real `flow test` suite exists for the (undeployed) RPCTradeEscrow contract at `cadence/tests/RPCTradeEscrow_test.cdc` — **16/16 green as of 2026-07-17**, all 12 audit scenarios covered — but it is NOT run in CI (needs flow-cli built from master + pinned ExampleNFT v1.2.2; one-time setup in `cadence/tests/README.md`).

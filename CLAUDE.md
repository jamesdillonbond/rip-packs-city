...
- **`apply_migration` for DDL; `execute_sql` for reads/verification.** ⚠ `CREATE INDEX CONCURRENTLY` needs `execute_sql` and runs **only if it FINISHES inside the 60 s client cap** — a timeout ABORTS it, leaving an `indisvalid=false` index. Record it with an `IF NOT EXISTS` migration so it is not fileless. ⚠ **A pg_cron command with a `SET …;` prefix runs in a TRANSACTION BLOCK — CIC cannot, and its budget must come from a ROLE default** (postgres owns but has no role timeout; `cron_heavy` has 600 s but no ownership). FMV writes are delete-then-insert, NEVER upsert. MCP + schema gotchas: [tooling-gotchas.md](docs/reference/tooling-gotchas.md).

...
- **Operator follow-ups.** (1) Wire the daily guard refresh to the new fast **`refresh_topshot_conflated_editions_detector_only()`** — the full `refresh_topshot_conflated_editions()` self-healer scan now exceeds its 120s timeout and is redundant post-drain+writer-fix (the daily cron was never wired — only 1 failed run ever, 06-20). (2) Optionally wire `/api/admin/drain-topshot-misattribution?rekey=1` low-cadence as a safety net (returns 0 targets now). (3) Force-stale cron to finish repricing the 5,990.

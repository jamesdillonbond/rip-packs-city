...
/**
 * NFL All Day on-chain discovery — probes Flow contract, MetadataViews, GQL, and Flowty.
 *
 * Usage: node scripts/allday-discovery.mjs
 */
...
const CONTRACT = "0xe4cf4bdc1751c65d"

async function script1_getOwnedIds() {
  console.log("\n══ Script 1: Get owned All Day NFT IDs ══")
  // Try Cadence 1.0 syntax first
  const cadence = `
    import AllDay from ${CONTRACT}
    import NonFungibleToken from 0x1d7e57aa55817448

    access(all) fun main(addr: Address): [UInt64] {
      let acct = getAccount(addr)
      let ref = acct.capabilities.borrow<&{NonFungibleToken.CollectionPublic}>(/public/AllDayNFTCollection)
      if ref == nil { return [] }
      return ref!.getIDs()
    }
  `
  try {
    ...
    console.log(`Found ${ids.length} All Day NFTs`)
    console.log("First 10 IDs:", ids.slice(0, 10))
    return ids
  } catch (err) {
    console.log("Cadence 1.0 failed:", err.message)
    // Try alternate public path
    try {
      const cadence2 = `
        import NonFungibleToken from 0x1d7e57aa55817448
      ...
      console.log(`Found ${ids.length} All Day NFTs (alt path)`)
      console.log("First 10 IDs:", ids.slice(0, 10))
      return ids
    } catch (err2) {
...
async function script3_probeGql() {
  console.log("\n══ Script 3: Probe NFL All Day GQL ══")
  try {
...
async function script4_probeFlowty() {
  console.log("\n══ Script 4: Probe Flowty for All Day ══")
  try {
    const res = await fetch("https://api2.flowty.io/collection/0xe4cf4bdc1751c65d/AllDay", {
      method: "POST",
      headers: {
...
async function main() {
  console.log("═══════════════════════════════════════════")
  console.log("  NFL ALL DAY ON-CHAIN DISCOVERY")

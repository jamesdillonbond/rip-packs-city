...
export const METHODOLOGY: Record<string, MethodologyEntry> = {
  ...
  listings: {
    ...
    paragraphs: [
      ...
      "cached_listings is the marketplace ask side and is explicitly a subset, not a full orderbook snapshot. It is populated by the Sniper deal-feed scanner, which walks each marketplace looking for below-FMV inventory worth surfacing as deals, and holds the same shape across NFL All Day, Golazos, and others. ts_listings, the Top Shot equivalent, is RETIRED: the sampler was switched off on 2026-05-26 and its last row was written on 2026-05-15, so no Top Shot orderbook depth is published anywhere on the site. Live Top Shot ask data comes from edition_offers via the Sniper deal feed instead. Median ask is the most representative summary statistic on these tables — min and max can be skewed by listing-reward farming asks, and the dataset is small enough that count alone doesn't tell the full story.",

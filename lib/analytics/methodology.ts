...
export const METHODOLOGY: Record<string, MethodologyEntry> = {
  ...
  fmv: {
    ...
    sources: [
      ...
      "ts_listings + cached_listings (Supabase) — ask-side validation; ts_listings is RETIRED (see the Listings methodology)",
  ...
  listings: {
    ...
    paragraphs: [
      ...
      "cached_listings is the marketplace ask side and is explicitly a subset, not a full orderbook snapshot. It is populated by the Sniper deal-feed scanner, which walks each marketplace looking for below-FMV inventory worth surfacing as deals, and holds the same shape across NFL All Day, Golazos, and others. ts_listings, the Top Shot equivalent, is RETIRED: the sampler was switched off on 2026-05-26 and its last row was written on 2026-05-15, so no Top Shot orderbook depth is published anywhere on the site. Live Top Shot ask data comes from edition_offers via the Sniper deal feed instead — written by one cron, offers-sweep, which re-checks every edition roughly hourly when healthy. That is the target, not a guarantee: on 2026-08-29 an upstream outage left the whole column unrefreshed for over 30 hours, so the boards and the edition pages now show the age of any ask we have not re-confirmed recently rather than implying it is live. Median ask is the most representative summary statistic on these tables — min and max can be skewed by listing-reward farming asks, and the dataset is small enough that count alone doesn't tell the full story.",
    ...
    sources: [
      ...
      "ts_listings (Supabase) — RETIRED 2026-05-26, last row 2026-05-15; no longer read by any surface",

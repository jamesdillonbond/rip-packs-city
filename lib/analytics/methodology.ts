...
export const METHODOLOGY: Record<string, MethodologyEntry> = {
  ...
  fmv: {
    ...
    sources: [
      ...
      "edition_offers (Supabase) — the live ask per edition, written by the offers-sweep cron; the ask-corroboration and ASK_ONLY inputs, both bounded by how recently the ask was confirmed",
      "ts_listings + cached_listings (Supabase) — ask-side validation; ts_listings is the Top Shot order book, rebuilt from the Atlas firehose every ~2 minutes (see the Listings methodology)",
  ...
  listings: {
    ...
    paragraphs: [
      ...
      "cached_listings is the marketplace ask side and is explicitly a subset, not a full orderbook snapshot. It is populated by the Sniper deal-feed scanner, which walks each marketplace looking for below-FMV inventory worth surfacing as deals, and holds the same shape across NFL All Day, Golazos, and others. ts_listings, the Top Shot equivalent, works differently again: the original sampler was switched off on 2026-05-26, but the table was rewired to the Atlas firehose on 2026-09-07 and is now rebuilt roughly every two minutes as a full snapshot of the open Top Shot book rather than a deal-scan subset. Order Book Depth publishes it only while it is fresh — the block carries the age of its own newest row, and if that age passes six hours the card reports the age instead of the depth, because a book nobody has rebuilt is not a market fact. Live Top Shot ask data is also available per edition from edition_offers via the Sniper deal feed — written by one cron, offers-sweep, which re-checks every edition roughly hourly when healthy. That is the target, not a guarantee: on 2026-08-29 an upstream outage left the whole column unrefreshed for over 30 hours, so the boards and the edition pages now show the age of any ask we have not re-confirmed recently rather than implying it is live. Median ask is the most representative summary statistic on these tables — min and max can be skewed by listing-reward farming asks, and the dataset is small enough that count alone doesn't tell the full story.",
    ...
    sources: [
      ...
      "ts_listings (Supabase) — the open Top Shot book, rebuilt from the Atlas firehose every ~2 minutes; published as Order Book Depth only while its newest row is under six hours old",
    ...
    // ⚠ This is the PAGE's revalidate, not the ask feed's cadence, and stating it alone
    // read as "these asks are five minutes old". The two are different numbers and the
    // gap between them is the whole point of the per-row ages the surfaces now carry.
    refresh: "Page revalidates every 5 minutes; the underlying ask feed re-checks each edition roughly hourly when healthy",
  },
  "wallet-profiles": {

...
async function fetchDepositEvents(start: number, end: number): Promise<FlowEventBlock[]> {
  ...
  if (!res.ok) {
    ...
    // per-chunk try/catch, which `break`s and leaves the cursor untouched (see the
    // "if we bail we don't advance" init of lowestScanned). Returning [] looks like
    // an empty window and lets the reverse-walk cursor step past this range → the
    // Deposit (owner snapshot) events in it are never re-scanned (silent loss).

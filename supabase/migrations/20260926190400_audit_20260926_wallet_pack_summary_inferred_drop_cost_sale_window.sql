-- 2026-09-26 (PT) — get_wallet_pack_summary: inferred_primary_* only for packs
-- acquired inside their drop's sale window (the history's rule, 20260926190300):
-- start_time - 1 day .. + 30 days, acquisition = Dapper's index date for the
-- wallet, else the first sale / open of the pack; All Day drops from 2022-12-16.
...
--   supabase/migrations/20260926190100_audit_20260926_wallet_pack_summary_reports_inferred_drop_cost_apart.sql
-- and repoint its pin.
...
BEGIN
  ...
  FROM (
    SELECT CASE WHEN k.collection_id = v_ad
                ...
                ELSE public.pack_retail_usd(pd.metadata->>'retail_price_usd')
           END AS retail
    FROM (

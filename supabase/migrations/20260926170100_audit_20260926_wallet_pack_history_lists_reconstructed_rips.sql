-- 2026-09-26 (PT) — get_wallet_pack_history v9: the packs a wallet opened with
-- NO pack NFT (custodial Top Shot packs, 2021 on) are listed as RIPPED rows,
-- from wallet_reconstructed_rips (20260926170000).
--
-- WHY: Trevor, "I have definitely ripped more than 119 packs". Every source the
-- list read was pack-NFT-keyed, so a pack opened inside the Top Shot account
-- never appeared. 0xbd94cade097e50ac: 2,736 such reveals since 2021-03, 2,720
-- fully priced.
--
-- WHAT: wallet_rips gains rip_source ('rip' | 'reconstructed') and the
-- reconstructed arm (this wallet only); pull_value_source 'delivery_burst';
-- pulls_total / identified / priced fall back to the burst's own counts; a
-- reconstructed row's name is "<Collection> pack (no pack NFT, reconstructed)"
-- -- never a borrowed distribution -- with dist, buy and P&L NULL; new output
-- field rip_source; coverage text names the source and its limits.
-- Header preserved (STABLE SECURITY DEFINER, search_path=public, 20s).

-- SNAPSHOT of public.get_wallet_moments_with_fmv as it runs in production after
-- 20260913231145 (lock provenance + Pinnacle stops discarding its own locks).
--
-- ⚠ WHY A SEPARATE SNAPSHOT FILE. 20260913231145 applies its change as a GUARDED
-- TRANSFORM of whatever is live — the right shape for applying it, because it
-- cannot mis-transcribe 7,747 characters and cannot clobber a concurrent edit it
-- did not anticipate. But it contains no literal DDL, and the repo's DB-invariant
-- pin machinery (__tests__/db-invariants-drift-guard.test.ts) compares the pinned
-- supabase/tests/*.sql copy against a migration's literal
-- `CREATE OR REPLACE FUNCTION … $function$;` block. So the transform migration
-- does the work and THIS file is what the pin points at.
--
-- ⛔ Captured with pg_get_functiondef() straight to disk by script, never typed.
-- A hand-copied pin is a pin that silently describes a function that no longer
-- runs — the exact failure `scripts/check-db-pin-staleness.mjs` exists to catch,
-- and which it caught here (this file is the fix for that red).
--
-- Re-applying this file is a no-op against an unchanged database.
--
-- anon-exec: intentional — get_wallet_moments_with_fmv backs the un-gated
-- collection tabs and the public /share/[wallet] surface, so anon must keep
-- EXECUTE. ⚠ And this is a SNAPSHOT: `CREATE OR REPLACE FUNCTION` does NOT
-- reset a function's ACL, so adding a REVOKE here would silently CHANGE
-- production while presenting itself as a no-op capture of what already runs.

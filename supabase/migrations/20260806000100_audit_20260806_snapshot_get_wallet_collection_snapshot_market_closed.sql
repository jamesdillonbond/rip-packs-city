-- Snapshot migration: public.get_wallet_collection_snapshot — capture the LIVE
-- definition so its DB-invariant pin validates what actually runs in production.
--
-- The prior pin (20260729000100) predated the 2026-08-03 UFC market-closure work
-- (collections.market_closed_at): a closed collection's moments still COUNT but
-- its dead-market value is excluded from the grand totalFmv, and perCollection
-- now carries market_closed_at. That change was applied to prod via MCP without a

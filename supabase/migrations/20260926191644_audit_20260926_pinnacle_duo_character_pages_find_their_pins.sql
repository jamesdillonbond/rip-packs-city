-- audit_20260926_pinnacle_duo_character_pages_find_their_pins
--
-- WHY. Follow-up to 20260926191419, which moved Pinnacle character pages onto
-- the render catalog and took empty pages from 36 to 2. Six characters are DUOS
-- whose players row carries the combined name ("Maurice & Cogsworth",
-- "Monterey Jack & Zipper", "Mickey Mouse Donald Duck", "Merida Angus", "Ennui
-- Embarrassment", "Darth Vader Luke Skywalker"): no single element of a pin's
-- Characters trait equals that name, so they fell back to pinnacle_editions —
-- and two of them rendered NO editions.
--
-- WHAT. A pin also matches a character when it lists MORE THAN ONE character
-- and its full Characters list, joined with " & " or with a space, equals the
-- name. Measured before applying: that clause matches pins for exactly those 6
-- characters (1–4 pins each) and for no single-name character. Same change in
-- get_player_editions and get_player_detail (grid and header stay in step).
-- Every other line is 20260926191419's body unchanged.
--
-- REVERT: re-apply both functions from 20260926191419.

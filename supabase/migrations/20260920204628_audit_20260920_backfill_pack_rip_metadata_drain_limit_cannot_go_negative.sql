-- audit_20260920_backfill_pack_rip_metadata_drain_limit_cannot_go_negative
--
-- ONE LINE CHANGES: `LIMIT (v_safe_limit - ...)` on the null_drain leg becomes
-- `LIMIT GREATEST(0, v_safe_limit - ...)`. Everything else is byte-identical to
-- 20260920204056, applied six minutes earlier.
--
-- WHY. Every OTHER leg's share carries a `GREATEST(1, ...)` floor, so the shares
-- have a combined minimum of 4 (5 legs, one of which takes the remainder). Below
-- that the remainder expression goes negative and `LIMIT -1` is a runtime error
-- (22023 invalid_row_count_in_limit_clause), not a small batch -- so the whole
-- RPC throws. It errored for p_limit 1..3 after 20260920203815 added two legs,
-- and for 1..2 before that, i.e. THIS HAZARD IS OLDER THAN TODAY'S CHANGES and
-- was merely widened by one.
--
-- ⚠ NO PRODUCTION CALLER IS AFFECTED, and that is exactly why it survived: the
-- only caller is /api/cron/backfill-pack-rip-metadata, which passes 500. It is
-- fixed because a DB-invariant pin has to drive the function at a SMALL p_limit
-- to make one leg's share exactly one row and assert which row it picks -- and a
-- guard that cannot address a leg individually cannot pin an ORDER BY.
--
-- ⭐ Recorded as its own migration rather than folded back into 20260920204056,
-- because it is a different KIND of change: those two are about what the function
-- publishes, this one is about it not throwing at an input nobody sends. Keeping
-- them separate is what makes a bisect of either meaningful.
--
-- REVERT: re-apply the body from 20260920204056 (the ONLY difference is this
-- LIMIT expression). No data half.
--
-- anon-exec: intentional — CREATE OR REPLACE keeps the ACL of backfill_pack_rip_metadata, so a revoke here would CHANGE production rather than preserve it; verified live after the apply (anon false, service_role true, proconfig still search_path + statement_timeout=50s). Its only caller is /api/cron/backfill-pack-rip-metadata on the service role.
--
-- ⚠ The DDL below is BYTE-IDENTICAL to production: prosrc md5
-- 552d4ef49fc453a8d4bfa77fe9e28ec3, length 11199, read back after the apply.

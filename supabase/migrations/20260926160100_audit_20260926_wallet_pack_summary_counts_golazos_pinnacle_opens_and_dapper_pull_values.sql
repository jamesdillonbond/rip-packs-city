-- 2026-09-26 (PT) — get_wallet_pack_summary counts every pack the wallet OPENED
-- (Top Shot + All Day rips, and Golazos + Pinnacle opens, which pack_rips never
-- held) and Golazos marketplace buys/sells, and values each opened pack from
-- Dapper's list of the moments it yielded (pack_open_pull_values, same opener)
-- before the open row's own value. Sibling of 20260926160000 (the list); the two
-- must agree, so the hero totals and the Opened tab count the same packs.
-- Measured on 0xbd94cade097e50ac before: packs_ripped 504, 56 valued.
--
...
BEGIN
  ...
  SELECT o.collection_id, o.pack_nft_id, o.sealed_at,
         COALESCE(pov.pull_value_usd, o.pull_value_usd) AS pull_value_usd
  FROM (
    SELECT collection_id, pack_nft_id, sealed_at, pull_value_usd
    FROM public.pack_rips WHERE opener_address = v_wallet
    UNION ALL
    SELECT v_gz, pack_nft_id, opened_at, pull_value_usd
    FROM public.golazos_pack_opens WHERE opener_address = v_wallet
    UNION ALL
    SELECT v_pin, pack_nft_id, opened_at, pull_value_usd

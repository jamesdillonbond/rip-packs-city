-- 2026-09-26 (PT) — get_wallet_pack_summary v11: the history's two changes
-- (20260926200100): a Top Shot pack Dapper minted straight into this wallet
-- (pack_nft_mints within 2 s of the index's acquisition instant) is inferred at
-- its drop's retail however late it arrived; a Trade Ticket pack's retail is a
-- ticket price, read through pack_retail_usd(raw, title) as NULL -- so a recorded
-- Trade Ticket "buy" counts in primary_spend_unknown_count, not as $10 spent.

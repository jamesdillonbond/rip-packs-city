-- Wire the roadmap headline metric (per-collection HIGH/MEDIUM confidence SHARE)
-- into rpc_trust_health_precompute_refresh, so it is TRACKED automatically on the
-- existing cron instead of only being readable on demand via
-- rpc_fmv_confidence_share() (shipped 2026-08-08, migration 20260808143508).
--
-- Piggybacked on Legs 2-5's EXISTING latest-per-edition DISTINCT-ON scan over
-- fmv_snapshots at ~0 marginal cost: add fs.confidence to the `latest`/`elig`
-- CTEs and one count FILTER in `agg`; upsert 5 new <coll>_fmv_high_med_share_pct
-- metrics alongside the 5 stale-% metrics via a want_share UNION. No new scan, so
-- the ~569/600s budget is unaffected (the fn is load-bearing: the sentinel fails
-- BLIND on a timeout). Same canonical-edition scoping (Top Shot restricted to
-- setID:playID[::parallel] external_ids), so the share is measured over the same
-- population as the stale-% metric. TRACK-only: no breach arm is added to
-- v_rpc_trust_health -- a thin market is a fact, not an alert.
--
-- Every other leg is preserved byte-for-byte from the prior definition.
...
BEGIN
  ...
  -- Legs 2-5 (+ candy, 2026-08-01): per-collection FMV coverage staleness, AND
  -- (2026-08-08) per-collection HIGH/MEDIUM confidence SHARE -- the roadmap
  -- headline metric -- piggybacked on the SAME latest-per-edition scan at ~0
  -- marginal cost (one extra count FILTER on the confidence column already in the
  -- selected row). Standalone on-demand read is rpc_fmv_confidence_share(); this
  -- leg writes the tracked <coll>_fmv_high_med_share_pct metrics. TRACK-only.
  --
  ...
  -- every other leg's denominator. The share metric shares this exact scoping.
  t1 := clock_timestamp();
  ...
  --   panini_sale_price_capture_dry_days (NEW 2026-08-04) -- consecutive most-recent
  ...
  VALUES ('panini_sale_field_mapping_shortfall', v_short, now(),
          round(EXTRACT(epoch FROM clock_timestamp() - t1) * 1000)),
         ('panini_sale_price_capture_dry_days', COALESCE(v_panini_dry, 0), now(),
          round(EXTRACT(epoch FROM clock_timestamp() - t1) * 1000))
  ON CONFLICT (metric) DO UPDATE
    SET value = EXCLUDED.value,
        computed_at = EXCLUDED.computed_at,
  ...
  RETURN jsonb_build_object(
    ...
    'panini_sale_price_capture_dry_days', v_panini_dry,

-- Bug 4 (AllDay leg): get_allday_market_listings took ~66s (killed at service_role's
-- 30s cap) -> /nfl-all-day/market 504. Root cause: it was LANGUAGE sql, so p_sort_by
-- was never a plan-time constant; the CASE-per-sort ORDER BY always referenced the
-- fmv LATERAL, forcing fmv (and the per-row wmc lateral) to be evaluated for ALL
-- ~22k active listings before the LIMIT. Market's default sort is "recent"
-- (listed_desc) with min_discount=0 — a case that needs NEITHER lateral to order or
-- filter.
--
-- Fix: convert to plpgsql and build the ORDER BY from a hardcoded whitelist, and only
-- add the fmv-based discount filter when p_min_discount>0. For listed_desc/price with
-- min_discount=0 the WHERE + ORDER BY reference only cached_listings_v2 columns, so
-- the planner pushes the LIMIT below the laterals -> they run for ~p_limit rows, not
-- 22k. SELECT/FROM/WHERE/ORDER-BY semantics are byte-identical to the old function for
-- every input. Only caller is /api/market. No FMV math changed. Scalars interpolated
-- via format() %L/%s (text quoted, numerics digit-only) — no injection surface.
--
-- NOTE: superseded ~1 min later by 20260703235137 (adds ::text casts required by
-- plpgsql RETURN QUERY EXECUTE for varchar columns). Kept for migration-history parity.
...
BEGIN
  ...
  v_sql := format($fmt$
    SELECT
      ...
      'https://nflallday.com/listing/' || cl.listing_resource_id::text,
      e.thumbnail_url,
...
CREATE INDEX IF NOT EXISTS idx_cl_v2_collection_listed_active
  ON public.cached_listings_v2 (collection_id, listed_at DESC)

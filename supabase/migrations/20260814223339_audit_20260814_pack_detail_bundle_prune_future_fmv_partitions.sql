-- get_pack_detail_bundle: prune the empty future fmv_snapshots partition.
--
-- The hero-editions leg scores FMV with a correlated subquery per pool edition
-- (1,531 probes on dist 4184). Each probe walked an Append over EVERY
-- fmv_snapshots partition. Measured 2026-08-14: fmv_snapshots_2025 and _2027
-- hold ZERO rows and 0 bytes, yet the 2027 index root cost 2 buffers per probe
-- = 3,062 buffers, ~33% of the whole leg, for nothing.
--
-- `computed_at <= now()` gives the planner a partition-key predicate and it
-- prunes at RUNTIME ("Subplans Removed: 1"). Total buffers 9,131 -> 6,308 on
-- dist 4184, using the SAME index as before.
--
-- Semantically a no-op: an FMV snapshot cannot be computed in the future, and
-- there are 0 future-dated rows (checked). When 2027 begins the predicate stops
-- pruning and the cost simply returns to today's -- it degrades, never breaks.
--
-- ⚠ THIS IS NOT THE FIX THE INBOX FILING PROPOSED, and that proposal was
-- MEASURED AND REJECTED -- do not re-derive it. Adding
-- `fs.collection_id = p_collection_id` does flip the hot partition to an
-- "Index Only Scan" on fmv_snapshots_2026_coll_ed_ct_fmv_idx exactly as
-- predicted, and it is still WORSE:
--   * that index is 113 MB against the 61 MB one this lookup uses today, and it
--     has 15,621 lifetime scans against 38,906,280 -- i.e. it is cold, and on a
--     2 GB instance the change evicts the hottest index on the platform;
--   * "Index Only" is a misnomer here: Heap Fetches 1,292 of 1,531. The
--     delete-then-insert FMV write pattern keeps pages non-all-visible, so the
--     visibility map cannot pay off;
--   * measured cold it read 1,630 pages vs 137, and cold is precisely the case
--     that times out.
-- The planner's own cost estimate preferred it (2906 -> 1948). The estimate was
-- wrong because it assumes the visibility map works.
--
-- ⚠ This does NOT close JAVASCRIPT-NEXTJS-1Z. A 31% buffer cut does not save a
-- 19 s cold path whose real cause is cache residency on a 2 GB instance. The
-- real fix is to stop doing 1,531 probes per page view at all -- precompute the
-- five hero editions per (collection_id, dist_id) on the FMV cadence (4,694
-- pairs, ~23k rows) and read one row. Filed, not taken.
--
-- Rollback: re-apply the previous definition -- it differs ONLY by the absence
-- of `and fs.computed_at <= now()` in the hero-editions correlated subquery.

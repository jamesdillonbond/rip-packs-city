-- rpc_trust_health_precompute_refresh — re-key Leg 7 (`sales_serial_supply_worst_pct`)
-- off the FRESH cohort and onto an AGED one. ALARM RE-KEY ONLY: no writer, no
-- backfill, no sales data is touched by this migration.
--
-- WHY. The arm counted serial-less sales among rows `ingested_at >= now()-24h`.
-- But a NULL serial on a fresh row is normally IN-FLIGHT WORK, not a defect:
-- `sales-serial-backfill` resolves it within days. Measured live 2026-08-03,
-- NULL-serial rate by row age on TopShot `offer_fill`:
--
--   0-6h  10.0%    6-24h 26.7%    1-3d  8.0%    3-10d 1.1%    10d+ 0.8%
--
-- So the arm was watching the one window where the rate is HIGHEST BY DESIGN,
-- and would keep flapping on a perfectly healthy system. This is what produced
-- the "36% of one writer's rows" finding (ledger `4432c549`) — a fresh-cohort
-- snapshot misread as a permanent defect rate. The true permanent residue is
-- 0.8% (272 of 33,937 rows aged 10d+), and the backfill is healthy and draining
-- (728 rows on 08-03, 1,529 on 08-02, ok_count = runs).
--
-- Same arm, both keyings, measured together 2026-08-03 (sold within 30d):
--
--   collection      fresh(24h) n / pct     aged(3-10d) n / pct
--   nba_top_shot         3,304 / 6.54%          16,657 / 0.26%   <- BREACHing at 5
--   candy_mlb              456 / 0.00%           2,518 / 0.00%
--   nfl_all_day            165 / 0.00%           3,367 / 0.00%
--   laliga_golazos           6 / 0.00%              14 / 0.00%
--
-- The arm KEEPS its detection power. It exists to catch a sales writer silently
-- dropping serial_number while the indexer still reports ok=true; a genuinely
-- broken writer produces a sustained 10-30% rate that sails through the 5%
-- threshold in the aged cohort too — it is simply reported ~3 days later. That
-- matches the lag the arm already had in the validation recorded on it (the
-- 2026-07-13 AllDay regression "would have breached 2026-07-16"). In exchange it
-- stops crying wolf, which is what actually erodes an alarm's value.
--
-- The wider 7-day band also makes the arm MORE reliable, not less: the `HAVING
-- count(*) >= 200` minimum-sample gate now sees 16,657 TopShot rows instead of
-- 3,304, so a single quiet ingest hour can no longer swing the percentage.
--
-- breach_at stays 5 (unchanged) — the threshold was never the problem, the
-- window was. Do NOT "fix" this class by raising breach_at: that would blind the
-- arm to a real writer failure instead of pointing it at settled data.
--
-- Everything except Leg 7 is reproduced byte-for-byte from the live definition
-- (pg_get_functiondef, 2026-08-03).
--
-- Revert: re-apply this function with Leg 7's WHERE restored to
--   AND s.ingested_at >= now() - '24:00:00'::interval
--
-- The companion migration audit_20260803_serial_supply_arm_catches_text updates
-- the arm's self-documenting `catches` string in v_rpc_trust_health to match.

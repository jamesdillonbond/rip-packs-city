-- Fix: the floor-sweep detector's re-emission dedup compared
-- evidence_jsonb->>'last_buy' (jsonb ISO text, "2026-07-12T06:28:50.046+00:00")
-- against last_buy::text (Postgres text, "2026-07-12 06:28:50.046+00"). The two
-- formats never string-match, so the NOT EXISTS guard failed and the hourly cron
-- re-inserted the same ~18 alerts every run (36 rows / 18 dup groups observed).
-- Compare as timestamptz instead. Also de-dupes the rows already emitted.
-- Revert: CREATE OR REPLACE back to the 20260712191000 body (text compare).
...
-- Clean up the duplicate rows the format-mismatch already emitted: keep one per
-- (buyer_address, last_buy).
DELETE FROM public.topshot_insider_alerts
WHERE alert_type = 'floor_sweep'
  AND ctid NOT IN (
    SELECT MIN(ctid) FROM public.topshot_insider_alerts
    WHERE alert_type = 'floor_sweep'
    GROUP BY evidence_jsonb->>'buyer_address', evidence_jsonb->>'last_buy'
  );

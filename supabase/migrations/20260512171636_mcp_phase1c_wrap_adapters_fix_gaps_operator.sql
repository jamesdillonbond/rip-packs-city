-- ================================================================
-- mcp_phase1c_wrap_adapters_fix_gaps_operator
--
-- Hot-fix for version 20260512171327. The original migration used
-- `v_gaps := v_gaps || 'literal'` to append to a text[] gaps array.
-- That parses ambiguously in Postgres: the bare 'literal' is `unknown`
-- type, and operator resolution prefers `text[] || text[]`, attempting
-- to cast 'literal' as a malformed array literal and raising 22P02.
--
-- Replaces all four adapters with CREATE OR REPLACE using
-- array_append(v_gaps, 'literal') — unambiguous, no operator overload
-- ambiguity. Behaviour is otherwise identical to the original.
-- ================================================================
...
begin
  ...
  if v_pack_price is null then
    v_gaps := array_append(v_gaps, 'pack_price_missing_from_metadata');
  end if;
  if v_slots is null then
    v_gaps := array_append(v_gaps, 'slots_missing_from_metadata');
  end if;
  ...
  if v_pack_price is not null and v_slots is not null then
    ...
    exception when others then
      v_ev := null;
      v_gaps := array_append(v_gaps, 'compute_pack_ev_from_pool_raised_' || regexp_replace(sqlerrm, '[^a-z0-9_]+', '_', 'gi'));
    end;
  else
    v_ev := null;
    v_gaps := array_append(v_gaps, 'ev_skipped_missing_inputs');
  end if;

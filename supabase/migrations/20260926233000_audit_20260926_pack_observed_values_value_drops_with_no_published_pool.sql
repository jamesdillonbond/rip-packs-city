-- 2026-09-26 (PT) — a pack whose drop has no published pool is valued at what
-- that drop's OPENED packs actually yielded.
--
-- WHY (Trevor: "why do we have such low coverage of understanding the moments
-- inside the packs"). 212 of 0xbd94cade097e50ac's 434 sealed packs had no rip EV.
-- Not a moment-pricing gap: the EV model (mv_pack_ev_latest) needs a drop's
-- published pool and odds. 154 packs / 63 drops have no MV row at all -- reward
-- packs (Set Completion Rewards, Playbook "Free" packs, One to Remember rewards)
-- -- and 58 / 12 drops have a row the MV could not price (Chance Hit packs, Fast
-- Break rewards, Trade Ticket packs). Yet for 67 of those 75 drops we already
-- hold >= 5 opened packs with a known value (195 of the 212 packs covered):
-- the drop's own track record.
--
-- WHAT.
--   pack_observed_values   one row per (collection, drop): n_opened, n_valued,
--                          avg_value_usd -- the mean current value of its
--                          opened packs (pack_open_pull_values first, the rip
--                          row's value otherwise; unpriced opens counted, never
--                          averaged in as 0).
--   refresh_pack_observed_values()  daily rebuild (write first, delete only what
--                          it did not write); pg_cron rpc-pack-observed-values
--                          at 13 11 * * * (4:13 AM PT).
--   get_wallet_pack_history v14: pack_opened_avg_usd / pack_opened_n where the
--                          drop has >= 5 valued opens (beside the modelled EV,
--                          never replacing it).
--   wallet_held_pack_value: opened_avg_count / opened_avg_usd for held packs
--                          with no modelled EV -- reported apart from rip_ev.
-- anon-exec: unchanged (get_wallet_pack_history, wallet_held_pack_value) — CREATE OR REPLACE of existing fns; ACL preserved, has_function_privilege anon=false, authenticated=false (read 2026-09-26).
-- anon-exec: refresh_pack_observed_values() — new; REVOKE FROM PUBLIC, anon, authenticated below.
--
-- Revert: re-apply get_wallet_pack_history and wallet_held_pack_value from
--   supabase/migrations/20260926230000_audit_20260926_wallet_pack_history_gross_ev_and_held_pack_value.sql
-- and repoint their pins; then
--   SELECT cron.unschedule('rpc-pack-observed-values');
--   DROP FUNCTION public.refresh_pack_observed_values(); DROP TABLE public.pack_observed_values;

CREATE TABLE IF NOT EXISTS public.pack_observed_values (
  collection_id  uuid NOT NULL REFERENCES public.collections(id),
  dist_id        text NOT NULL,
  n_opened       int  NOT NULL,
  n_valued       int  NOT NULL,
  avg_value_usd  numeric(14,2),
...
COMMENT ON TABLE public.pack_observed_values IS
  'Per drop: how many opened packs we hold (n_opened), how many carry a value (n_valued), and their mean CURRENT value -- what the drop actually yielded. Readers use it only at n_valued >= 5. Written by refresh_pack_observed_values().';
ALTER TABLE public.pack_observed_values ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.pack_observed_values FROM anon, authenticated;
...
REVOKE ALL ON FUNCTION public.refresh_pack_observed_values() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.refresh_pack_observed_values() TO postgres, service_role;
...
SELECT cron.schedule('rpc-pack-observed-values', '13 11 * * *', 'SELECT public.refresh_pack_observed_values();');

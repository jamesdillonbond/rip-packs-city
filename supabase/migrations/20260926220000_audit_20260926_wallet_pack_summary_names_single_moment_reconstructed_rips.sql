-- 2026-09-26 (PT) — get_wallet_pack_summary v13: says how many reconstructed
-- rips are a single moment delivery.
--
-- WHY. 1,244 of 0xbd94cade097e50ac's 2,736 reconstructed Top Shot rips are ONE
-- moment each, every one a delivery the bulk seed labelled pack_pull
-- (acquisition_confidence heuristic / inferred_pre_flowty / verified). A lone
-- delivery may be a single-moment pack -- or a challenge reward or a gift --
-- and nothing we hold separates them (the drop-pool fit scored 88% for packs
-- and rewards alike). packs_ripped stays as it is; the new
-- packs_ripped_reconstructed_single lets the page say how much of the count
-- rests on single deliveries.

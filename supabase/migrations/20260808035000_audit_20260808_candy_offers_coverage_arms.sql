-- 2026-08-08 · Candy offer-book COVERAGE arms: candy_offers_unverified_pct +
-- candy_offers_oldest_active_hours. Snapshot of the LIVE v_rpc_trust_health.
--
-- ⚠ REPO CATCH-UP MIGRATION. Both arms were applied to production via MCP as
-- `audit_20260808_candy_offers_coverage_arms` with no committed file; this
-- commits the resulting definition verbatim from pg_get_viewdef, so it is a
-- NO-OP against prod and byte-identical to what runs. Same prod/repo drift class
-- as get_wallet_collection_stats on 08-06. v_rpc_trust_health is NOT in the
-- drift-guard PINS array, so no pin went stale — only the repo was behind.
--
-- WHY THESE ARMS EXIST — a three-day outage that every existing signal called green.
-- candy-offers-indexer went silent after 2026-08-05 00:50Z. The deactivation guard
-- correctly refuses to mark standing offers dead on a partial sweep, so it traded
-- wrongly-empty for wrongly-stale: `is_active` froze and the PUBLIC
-- candy_offer_spread_board kept quoting bids that were 3+ days unconfirmed —
-- 39 of 50 active offers (78.0%), oldest 79.7h, ~$528 of standing bids.
--
-- ⚠ THE ARM THAT WAS FIRST PROPOSED WOULD HAVE BEEN BLIND TO ALL OF IT.
-- `candy_offers_stale_hours` = hours since max(last_seen_at) read **1.67h — green**
-- at the peak of the outage, because the max was carried by 4 touched rows out of
-- 186, every one of them is_active = false. A MAX OVER A SET IS NOT COVERAGE OVER
-- THE SET. One touched row resets it. Same defect family as the
-- public_board_slow_count count(*) probe. Measure coverage, not the extremum.
--
--   candy_offers_unverified_pct      share of is_active offers whose last_seen_at is
--                                    older than 12h (2x the 6h sweep cadence).
--                                    breach_at 25. Returns 999 on an EMPTY active set,
--                                    so a vanished book breaches instead of dividing
--                                    to a comfortable zero.
--   candy_offers_oldest_active_hours hours since the oldest still-active offer was
--                                    re-confirmed. breach_at 36 (6 missed cycles).
--
-- TWO ARMS ON PURPOSE: they fail differently. A percentage dilutes away a narrow
-- tail in which one high-value bid rots; the oldest-active arm catches exactly that.
--
-- CALIBRATION, against the healthy baseline measured immediately after the fix
-- (batched mint resolution + serial/150ms ME throttle, verified 2026-08-08 03:15:33Z:
-- 75 active offers, 0 unverified >12h, oldest 2.3h): breach_at 25 is ~1/3 of the
-- observed outage value and far above a healthy 0, so one missed tick cannot fire it
-- but a sweep that quietly stops re-confirming will. During the outage the arms would
-- have read 78.0 and 79.7 against 25 and 36 — caught on day one, not day three.
--
-- NOTE ON CAUSATION: the upstream failure was Cloudflare Error 1015 / HTTP 429 from
-- Magic Eden (25 of 70 calls), and every fetch error suppresses deactivation. So an
-- upstream fault surfaces here as a RISING UNVERIFIED SHARE, never as a shrinking
-- book. Do not "fix" a rising percentage by relaxing the deactivation guard — the
-- guard is reporting an upstream problem accurately.
--
-- ⚠ THE ALTER VIEW BELOW IS LOAD-BEARING, NOT BOILERPLATE. CREATE OR REPLACE VIEW
-- WIPES reloptions, and this exact view was bitten by that on 2026-08-03 (it silently
-- lost security_invoker=on). The two statements must always ship together.
--
-- REVERT: re-splice the view without the two arms, then re-run the ALTER VIEW.
-- Nothing else was touched — no data writes, no grant changes, no other arm.

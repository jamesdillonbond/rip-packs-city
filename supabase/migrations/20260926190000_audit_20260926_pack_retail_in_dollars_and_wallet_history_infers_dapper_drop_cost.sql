-- 2026-09-26 (PT) — pack retail prices in DOLLARS, and the wallet pack history
-- prices a pack with no buy row at its drop's retail where that is knowable.
--
-- WHY (1). pack_distributions.metadata->>'retail_price_usd' carries 109 Top Shot
-- drops in UFix64 units (x1e8; e.g. 4,990,000,000 for $49.90). Most readers
-- normalise (lib/packs/normalize-retail-price.ts: >= 1,000,000 -> /1e8), but the
-- wallet history and summary did not: 371 recorded primary buys joined to those
-- drops, so their buyers saw a pack "bought" for tens of millions of dollars.
-- pack_retail_usd(text) is that rule in SQL -- except unknown stays NULL (the TS
-- helper returns 0) and 0 stays 0 (a reward pack's known price).
--
-- WHY (2) (Trevor: pack history "missing a lot"). 0xbd94cade097e50ac: 716 Top
-- Shot packs held / sold / opened with a known drop and NO buy row -- so no cost,
-- no P&L. The marketplace history covers every Top Shot PackNFT's whole life
-- (the NFTs start 2023-10, topshot_pack_sales_history 2023-09), so such a pack
-- came from Dapper -- a drop at retail, a reward at $0 -- or by a transfer this
-- cannot see. All Day the same, but only when Dapper's index says the wallet
-- acquired it on/after 2022-12-15 (allday_pack_sales_history's start); its drop
-- price is allday_pack_supply.pack_price, where 0 means "not known".
--
-- WHAT: get_wallet_pack_history v10 -- retail via pack_retail_usd / allday
-- supply; buy_price_source 'retail_inferred' for such packs (has_buy stays
-- false: an inference is not a buy row); sold-with-inferred-buy gets P&L =
-- sale - retail; coverage text says what the inference can and cannot see.
-- Reconstructed rips (no distribution) are never inferred.
-- anon-exec: revoked (pack_retail_usd) — REVOKE FROM PUBLIC, anon, authenticated below; SECURITY DEFINER callers run as owner.
...
--
-- Revert: re-apply the history body from
--   supabase/migrations/20260926170100_audit_20260926_wallet_pack_history_lists_reconstructed_rips.sql
-- repoint its pin, then (after 20260926190100 / 190200 are reverted too)
--   DROP FUNCTION public.pack_retail_usd(text);
...
REVOKE ALL ON FUNCTION public.pack_retail_usd(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pack_retail_usd(text) TO postgres, service_role;
...
BEGIN
  ...
  enriched AS (
    ...
    -- A pack with no buy row we hold is inferred to come from Dapper (a drop or
    -- a reward -- or a transfer, which this cannot see) only where our
    -- marketplace history covers its whole life: every Top Shot PackNFT (the
    -- NFTs start 2023-10, the history 2023-09); an All Day pack only when
    -- Dapper's index says this wallet acquired it on/after 2022-12-15. Never a
    -- reconstructed rip (no distribution).
    CROSS JOIN LATERAL (
      SELECT (r.dist_id IS NOT NULL
              AND r.rip_source IS DISTINCT FROM 'reconstructed'
              AND (r.collection_id = v_ts
                   OR (r.collection_id = v_ad AND r.identity_acquired_at >= timestamptz '2022-12-15'))) AS inferable
    ) inf
  ),
  classified AS (
    SELECT *,
  ...
  RETURN jsonb_build_object(
    ...
    'coverage', jsonb_build_object(
      ...
      'retail', 'buy_price_source = retail_inferred: a pack with no buy row we hold, priced at its drop''s retail -- only where the marketplace history covers the pack''s whole life (every Top Shot PackNFT; All Day acquired on/after 2022-12-15 per Dapper''s index). A transfer from another wallet would read the same. Retail is in dollars (Top Shot UFix64 values normalised; All Day from allday_pack_supply, 0 = unknown)',

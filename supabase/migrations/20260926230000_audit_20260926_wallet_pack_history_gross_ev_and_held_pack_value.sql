-- 2026-09-26 (PT) — the pack history names a pack's CONTENTS value, and the
-- sealed packs a wallet holds get a market value.
--
-- WHY. (1) The history row's pack_ev_usd is mv_pack_ev_latest.pack_ev =
-- gross_ev - pack_price (NET of the drop price); the page labelled it "EV", so
-- an Anthology Quick Rip whose moments are worth $2.33 read "EV -$6.67", and
-- 0xbd94cade097e50ac's 434 sealed packs summed to a negative "EV". A holder's
-- question is rip or sell: the contents' value (gross) against the floor ask.
-- (2) The summary's net P&L counts what the wallet PAID for packs it still holds
-- and nothing for them -- 434 sealed packs, 433 with a floor ask ($11,042 at the
-- asks we hold, some up to a month old).
--
-- WHAT.
--   get_wallet_pack_history v13: new row key pack_gross_ev_usd (the MV's
--     gross_ev under the same publish gates as pack_ev_usd); pack_ev_usd stays.
--   wallet_held_pack_value(wallet): pages get_wallet_pack_history(status
--     'held') -- the history's own definition of held -- and sums floor asks,
--     last sales and rip EV, each with the count it covers and the oldest ask
--     check; complete = false when more than 25 pages (5,000 packs) are held.
--   (20260926230100: the summary publishes it as `held`.) Nothing is folded into
--   net P&L: it is unrealized, reported beside it.
...
-- anon-exec: wallet_held_pack_value(text) — new; REVOKE FROM PUBLIC, anon, authenticated below.
--
...
-- and repoint its pin; then (after 20260926230100 is reverted)
--   DROP FUNCTION public.wallet_held_pack_value(text);
...
REVOKE ALL ON FUNCTION public.wallet_held_pack_value(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.wallet_held_pack_value(text) TO postgres, service_role;

-- 2026-09-26 (PT) — get_wallet_pack_history v12: All Day packs are judged
-- against their drop's real sale window, and a REWARD drop is $0.
--
-- WHY. 400 of 0xbd94cade097e50ac's 468 All Day packs with no buy row had no
-- drop start to test the sale-window rule against: pack_distributions carried
-- start_time for 151 of 3,228 All Day distributions. 20260926210000 records
-- every distribution's window from Dapper's searchDistributions
-- (allday_drop_windows; 3,214 of 3,228 with a start). And All Day's metadata
-- types 1,131 distributions REWARD -- priced 0, read as "unknown" -- so a reward
-- pack had no cost at all instead of $0.
--
-- WHAT. drop_start = pack_distributions start_time, else allday_drop_windows
-- (All Day). All Day retail = 0 when metadata type = REWARD, else the supply
-- price (0 stays unknown: internal holds carry 0 too). The sale-window rule,
-- its 2022-12-16 All Day coverage floor and everything else are unchanged.

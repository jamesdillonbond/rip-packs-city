...
-- 73cfdc158434860d1827db469e815699). Applying it is a no-op against prod.
--
-- What it does: the per-challenge DRILL-DOWN behind /topshot/challenge-plan.
-- For a wallet + challenge it resolves each slot: eligible editions + costs
-- (badge floor low_ask / catalog fmv), whether the wallet fills the slot, the
-- cheapest-eligible 'pick' (the BUY RECOMMENDATION, with an /edition/ deep link),
-- cost-to-complete over unfilled slots, completion %, and the packs-per-user
-- netEv / worthIt verdict.

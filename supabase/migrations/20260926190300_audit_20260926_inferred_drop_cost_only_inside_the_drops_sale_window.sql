-- 2026-09-26 (PT) — get_wallet_pack_history: an inferred drop cost only when the
-- pack was acquired INSIDE its drop's sale window.
--
-- WHY. 20260926190000 inferred retail for any pack with no buy row whose life the
-- marketplace history covers. Checked against 0xbd94cade097e50ac right after
-- apply: All Day "Premium (Series 1, Week 18)" -- a 2022-03 drop -- priced at its
-- $249 drop price for a pack the wallet received 2023-02, with NO marketplace
-- sale of that pack on record; ten more Series 1 / Series 2 packs the same.
-- Those are rewards / redistributions, not retail buys. Coverage alone was not
-- enough: the acquisition must also fall where a retail purchase could happen.
--
-- WHAT. inferable additionally requires the drop's start_time (pack_
-- distributions.metadata) and an acquisition between start_time - 1 day and
-- start_time + 30 days -- Dapper's index acquired_at, else bounded above by the
-- wallet's own sale / open of the pack. All Day drops must start on/after
-- 2022-12-16 (its marketplace history's start). No start_time -> NULL.
...
--   supabase/migrations/20260926190000_audit_20260926_pack_retail_in_dollars_and_wallet_history_infers_dapper_drop_cost.sql
-- and repoint its pin.

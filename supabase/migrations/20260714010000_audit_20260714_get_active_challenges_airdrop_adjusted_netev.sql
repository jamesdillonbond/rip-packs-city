-- netEv/worthIt/sort now airdrop-adjusted: reward pack value x expected packs per completer
-- (allocation/completions, floored at the 1 guaranteed pack) - cost. Valuing only the single
-- guaranteed pack understated the verdict (Hoop Vision read -$34 "skip" at 1 pack, +$8 at ~2.3 packs).
-- Only netEv/worthIt/ORDER BY change; all other fields (incl rewardValue, packsPerUser) identical.

-- audit_20260806_get_wallet_moments_series_topshot_convention
--
-- SERIES 0-vs-1 CONVENTION COLLISION. base_other did:
--     COALESCE(wmc.series_number, e.series) AS series_number
-- which unions TWO INCOMPATIBLE ENCODINGS:
--   * wallet_moments_cache.series_number is the ON-CHAIN number. Top Shot has NO
--     on-chain series 1 — its Series 1 is 0 (see collection_series: nba_top_shot
--     row series_number=0, display_label='Series 1'; there is no series_number=1).
--   * editions.series is the DISPLAY number, where 1 = Series 1.
-- 385,734 Top Shot wmc rows have a NULL series_number, so they fall through to the
-- editions arm and emit a DISPLAY 1 into an ON-CHAIN column. Two symptoms:
--
--   (a) DISPLAY — the column renders a bare "1". lib/collection/helpers.ts
--       seriesDisplayLabel() has no map entry and no fallback for 1 (the fallback
--       map is 0,2,3,4,5,6,7,8), so it hits `return seriesRaw` and prints the raw
--       DB value. Trevor spotted this on two Damian Lillard "Cosmic" moments of the
--       SAME edition (8:145): serial 38 read "Series 1 · 2019-20", serial 12 "1".
--
--   (b) FILTER, silent — the SAME column backs `AND (p_series IS NULL OR
--       series_number = p_series)` and the client sends the ON-CHAIN number, so
--       filtering "Series 1" sends p_series=0 and DROPS every one of those rows.
--       The collector sees an incomplete collection with no error. This is the
--       worse half: (a) looks broken, (b) lies.
--
-- ⚠ THE TRAP — the fix MUST be scoped to Top Shot. A blanket `series = 1 -> 0`
-- would corrupt FOUR collections. Measured live against collection_series:
--     nba_top_shot   series 1 does NOT exist  (0 = 'Series 1')   <- normalise
--     nfl_all_day    series 1 = 'Series 1'                       <- legitimate
--     laliga_golazos series 1 = 'Series 1'                       <- legitimate
--     disney_pinnacle series 1 = '2023'                          <- legitimate
--     ufc_strike     has BOTH 0 ('Series 0') and 1 ('Series 1')  <- legitimate
--
-- Fix: normalise only when p_collection_id is the Top Shot uuid. Fixes display AND
-- the filter for all 385,734 rows with no bulk UPDATE. wmc.series_number, when
-- present, still wins — this only changes the editions FALLBACK arm.
--
...
--     COALESCE(wmc.series_number, e.series) AS series_number,
-- in base_other (and re-point the PINS entry + verbatim test copy back).

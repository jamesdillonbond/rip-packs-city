-- Per-slot challenge plan: each required lock, whether the wallet already owns an eligible
-- moment for it, and the cheapest eligible moment to buy if not (the actionable pick).
-- Revert: restore from audit_20260712_challenge_unify_cowork_roi.
...
SELECT jsonb_build_object(
  ...
  'netEv', CASE WHEN ch.cached_reward_value IS NULL OR a.cost_to_complete IS NULL THEN NULL ELSE ROUND(ch.cached_reward_value - a.cost_to_complete, 2) END,
  'worthIt', CASE WHEN ch.cached_reward_value IS NULL OR a.cost_to_complete IS NULL THEN NULL ELSE (ch.cached_reward_value - a.cost_to_complete) > 0 END,
  'slots', COALESCE((

CREATE OR REPLACE FUNCTION public.rpc_search_catalog_v4(p_q text, p_collection_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 20)
...
BEGIN
  ...
  -- beater" should not return nothing merely because the prose says "buzzer"
  -- and never says "beater". Requiring every token (LIKE ALL) made ONE absent
  -- word annihilate an otherwise perfect query -- measured 2026-08-14, that is
  -- the whole narrative-search defect, not the ranking.
  --
  ...
  -- Token coverage is then applied as a refinement, not as an index predicate.
  edition_cand AS (
  ...
  SELECT u.k, u.lbl::text, u.sub::text, u.sl::text, u.cid, c.slug::text, u.thumb::text, u.n,
    (coalesce(u.sim, 0)
      ...
      -- this is a constant offset there and existing rankings are unmoved.
      + (u.cov * 0.60)
...
REVOKE EXECUTE ON FUNCTION public.rpc_search_catalog_v4(text, uuid, integer) FROM PUBLIC, anon, authenticated;
-- 2026-08-04 · topshot_fmv_pct_stale_30d: scope to canonical, and record the disproof.
--
-- TWO changes, matched pair:
--   1. rpc_trust_health_precompute_refresh legs 2-5: the TopShot leg now counts only
--      CANONICAL editions (external_id ~ '^[0-9]+:[0-9]+(::[0-9]+)?$'). Other collections
--      unaffected -- the UUID-dupe class is TopShot-only, and no other collection uses
--      that external_id form (canonical count for allday/golazos/candy/ufc is 0 under
--      this predicate, which is why the filter is scoped by collection_id rather than
--      applied globally).
--   2. v_rpc_trust_health: the `catches` text on that arm, and short markers on the
--      allday/golazos/candy legs, now state what was measured rather than what was hoped.
--
-- breach_at is DELIBERATELY NOT CHANGED (stays 50). See the catches text: the healthy
-- steady state is one day old and re-baselining off it would repeat the exact error of
-- the 2026-07-25 baseline of 32.3%, which was captured while the sweep was already stuck.
--
-- MEASURED, 2026-08-04:
--   TopShot all-editions pct_stale_30d 32.2% ; canonical 0.0%
--   stale editions: 6,263 of 6,263 non-canonical. Canonical worst age 7.0d, p99 6.9d.
--   Back-test as-of during the 2026-07-20..08-03 sweep stall (the outage this arm claims
--   to catch, 74% of catalogue never recomputed): canonical form read 0.00% on 07-20,
--   07-25, 07-29, 08-02 and 08-03. It could not have fired.
--   Peak-stall (08-02) canonical age profile: 33.7% >3d, 11.7% >5d, 0.1% >7d, 0.0% >10d.
--   Family ceiling, max latest-FMV age: AllDay 7.00d, Golazos 6.96d, Candy 0.38d.
--
-- COST: the added editions join measured 486.9ms for the whole coverage leg
-- (Hash Left Join, Rows Removed by Filter 6426) vs 2,750ms for the current leg at the
-- 18:58Z tick. Not a regression.
--
-- REVERT: re-apply the prior function body (drop the elig CTE, point agg back at latest)
-- and restore the prior catches literals. No data is written or destroyed by this
-- migration; the only effect is the value written to
-- rpc_trust_health_precompute at the next 6-hourly tick (jobid 222).
...
BEGIN
  ...
  --
  -- 2026-08-04: the TopShot leg is now CANONICAL-ONLY. Before this change it read 32.2%

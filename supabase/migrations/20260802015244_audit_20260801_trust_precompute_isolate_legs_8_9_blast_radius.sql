-- audit_20260801_trust_precompute_isolate_legs_8_9_blast_radius
--
-- Legs 1-7 byte-identical to audit_20260801_trust_precompute_legs_8_10_*.
-- Two changes, both about BLAST RADIUS:
--
--  1. Legs 8 (board liveness) and 9 (pack-EV publish shortfall) are each wrapped
--     in BEGIN/EXCEPTION. Before this, a throw in the board probe aborted the
--     whole refresher, which would age ALL 11 precomputed metrics past 24h and
--     mass-breach them at 999 -- one slow public board taking down every
--     precomputed trust arm. Observed for real while testing: a client-side
--     cancel inside the probe propagated out of the function. On failure the arm
--     writes 999 to ITS OWN metrics (loud, correctly attributed) and the other
--     legs keep their values.
--
--  2. Leg ORDER swapped so the expensive board sweep runs LAST. Defence in depth:
--     even if something escapes the handler, everything cheaper has already been
--     written.
--
-- Also: budget_exhausted from the probe is treated as 999, not as a partial
-- count. An incomplete liveness sweep is INCONCLUSIVE, and a monitor that cannot
-- finish must be loud rather than silently green.
--
-- Revert: re-apply audit_20260801_trust_precompute_legs_8_10_board_liveness_packev_candy.

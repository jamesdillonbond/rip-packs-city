...
--
-- ── THE DEFECT, MEASURED ─────────────────────────────────────────────────────
-- 20260913211500 (2:00 PM PT) gave the hourly saved-wallet sweep a size gate on
-- the (user, wallet) PAIR's summed cached_moment_count, and excluded "the rows
-- the gate skips" from oldest_cache_h — but tested each ROW's own count there,
-- and called the gap "a bounded, stated approximation". It bit within two ticks.
-- At the 3:44 PM tick every queued wallet had just been refreshed (ok=true,
-- 4 of 4, untruncated) and the run row STILL said oldest_cache_h = 17.0: the
-- whale pair 0xf77bf547… (44,646 moments, skipped) also owns a 4,580-row All
-- Day row and three rows of 25/32/61, each under 20,000, so those four rows
-- stayed in the figure at 17.5 h, and the Portfolio Cache Drain arm warned
-- "Oldest portfolio cache >= 17.3h" at 4:04 PM about a wallet the hourly sweep
-- will never attempt — exactly the pinned-metric trap the 08-28 change to this
-- procedure records, in the place the 2:00 PM change said it could not matter.
-- Read from saved_wallets at 4:2x PM: the only rows under the gate and older
-- than 6 h were the two skipped pairs' small rows; no user_id-NULL rows exist.
--
-- ── THE CHANGE ───────────────────────────────────────────────────────────────
-- oldest_cache_h and oldest_big_cache_h now split on the PAIR's sum — the
-- queue's own test — via an EXISTS on saved_wallets grouped by (user_id,
-- wallet_addr). A pair above the gate contributes ALL its rows to
-- oldest_big_cache_h and none to oldest_cache_h. Signature, queue, gate, zero
-- pass, per-wallet COMMITs and the log are byte-identical; no DROP is needed
-- (same four-argument signature, so CREATE OR REPLACE is in place and pg_cron's
-- two CALLs — jobid 259 hourly, 497 nightly — are unaffected).
--
-- The pin gains a second row for the whale fixture (10 moments, 30 h stale) on
-- another collection; the existing assertion that oldest_cache_h reads the
-- 10 h of the stalest in-gate wallet is what fails without this change (it
-- read 30) and passes with it — run both ways locally before this shipped.
--

-- audit_20260810_wmc_selfheal_procedure_per_collection_d8
--
-- D8 attempt (SUPERSEDED — see 20260810154333, which drops this): a procedure that
-- heals each editions-backed collection in its OWN transaction (COMMIT per
-- collection), smallest-first, so a too-slow Top Shot leg drops without rolling
-- back the collections already healed.
--
-- ⚠ THIS FAILED via pg_cron: "invalid transaction termination". pg_cron wraps every
-- job command in a transaction, so a procedure cannot COMMIT here. v2
-- (20260810152627) tried moving the budget in-body; that failed identically.
-- Both were dropped by 20260810154333. Kept as a committed record of what was
-- applied (the procedure no longer exists in prod).

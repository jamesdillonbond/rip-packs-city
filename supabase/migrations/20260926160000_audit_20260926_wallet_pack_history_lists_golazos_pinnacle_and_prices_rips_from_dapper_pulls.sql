-- 2026-09-26 (PT) — the wallet pack history goes WIDER (Golazos + Pinnacle) and
-- DEEPER (every rip priced from the moments Dapper says it yielded).
--
-- WHY (Trevor: "feels like we're missing a lot of pack transactions … pull value
-- should be filled for every ripped pack"). Measured on 0xbd94cade097e50ac:
--   * get_wallet_pack_history read opens ONLY from pack_rips, which holds no
--     Golazos or Pinnacle rows -- his 14 Golazos opens rendered as index-only
--     "ripped" rows with no open time, no moment count and no value -- and read
--     marketplace buys/sells only from the Top Shot and All Day history tables,
--     so his 7 Golazos marketplace pack buys were absent (the transaction
--     history gained all of these on 2026-09-25; this list and the summary
--     did not).
--   * 462 of 518 rips had pull value NULL. The previous migration's lane
--     (pack_open_pull_values) now prices 507 of them from Dapper's own pull
--     lists; these readers did not look there.
--
-- WHAT. Two bodies, both pinned (supabase/tests/get_wallet_pack_{history,summary}.sql
-- carry them verbatim):
--   get_wallet_pack_history v8
--     + buys/sells: golazos_pack_sales_history (same shape as the TS/AD arms)
--     + opens: golazos_pack_opens, pinnacle_pack_opens as rips (rip_id NULL)
--     + pull value = COALESCE(pack_open_pull_values [same opener], rip row);
--       new fields pull_value_source ('dapper_pulls' | 'rip_record' | NULL),
--       pulls_total / pulls_identified / pulls_priced
--     + coverage text names the new sources
--   get_wallet_pack_summary
--     + Golazos marketplace buys/sells
--     + packs_ripped / ripped_value over pack_rips + Golazos + Pinnacle opens,
--       each valued pack_open_pull_values-first (same opener), else the row's own
-- Unknown stays NULL everywhere: a partly priced pull list is NULL, never a
-- partial sum, and never overrides a rip row that IS valued.
--
-- Headers preserved from pg_proc (STABLE SECURITY DEFINER / SECURITY DEFINER,
-- search_path=public, statement_timeout 20s / 15s); ACLs unchanged.
...
-- (get_wallet_pack_summary: the sibling migration 20260926160100, applied separately.)
--

...
// the Golazos copy of the UFC edges suite; the walkers are structural twins, so
// a defect found in one is a defect in all of them.
//
...
const req = (qs = "?range=250") =>
  new NextRequest(`https://t/api/cron/golazos-sales-history-backfill${qs}`, {
    method: "POST",
    headers: new Headers({ authorization: "Bearer golazos-history-token" }),
  })
...
/** Two mapped Golazos moments so both sales resolve to real editions. */
function mappedFixtures(over: Fixtures = {}): Fixtures {
  return {
    event_cursor: cursorFixture,
    wallet_moments_cache: {
      data: [
        { moment_id: "555", edition_key: "GOL-EDITION-1", serial_number: 12 },
        { moment_id: "777", edition_key: "GOL-EDITION-2", serial_number: 3 },
      ],
      error: null,
    },
    editions: {
      data: [
        { id: "uuid-gol-1", external_id: "GOL-EDITION-1" },
        { id: "uuid-gol-2", external_id: "GOL-EDITION-2" },
      ],
      error: null,
    },
    ...over,
...
describe("golazos-sales-history-backfill — batch-insert dedupe retry", () => {
...
describe("golazos-sales-history-backfill — the V2 Dapper venue", () => {
  it("ingests a purchased V2 Dapper sale, decodes its counterparties, and filters the rest", async () => {
    ...
    fetchMock = installFetchMock(
      flowStubs({
        v2d: [
          eventBlock({ height: CEILING - 50, txId: tx, eventType: V2_DAPPER_LISTING_COMPLETED, payload: v2DapperSalePayload("555", "9.00000000", GOLAZOS_NFT) }),
          ...
          eventBlock({ height: CEILING - 52, txId: "f".repeat(64), eventType: V2_DAPPER_LISTING_COMPLETED, payload: v2DapperSalePayload("999", "3.00000000", "A.329feb3ab062d289.UFC_NFT.NFT") }),
        ],
      }),
    )
...
describe("golazos-sales-history-backfill — dryRun + range failures", () => {
  it("?dryRun=true decodes a sample and writes NOTHING — no sales, no unmapped, no cursor move", async () => {
    ...
    fetchMock = installFetchMock(
      flowStubs({
        v1: [eventBlock({ height: CEILING - 200, txId: tx, eventType: V1_LISTING_COMPLETED, payload: v1SalePayload("555", "901", true, GOLAZOS_NFT) })],
      }),
    )
    const spy = install(mappedFixtures())
  ...
  it("reports belowFloor on the spork-floor 404 and treats any other status as an empty range", async () => {
    ...
    const res = await POST(req())
    expect(res.status).toBe(200)
    expect((terminalLog(spy2.rpcCalls)!.p_extra as Record<string, unknown>).below_floor).toBe(false)
    expect(terminalLog(spy2.rpcCalls)!.p_rows_found).toBe(0)
  })
})

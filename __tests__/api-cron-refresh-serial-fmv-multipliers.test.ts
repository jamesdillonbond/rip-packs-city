...
// Route-integration test for /api/cron/refresh-serial-fmv-multipliers.
...
//
// Success path: auth is synchronous; the compute_serial_fmv_multipliers pass +
// logging are deferred into after() and the route returns an immediate 202 accept.
// after() is stubbed to a no-op so the accept is observable without a request scope
// or any DB I/O; we assert the accept envelope (accepted + pipeline).
...
let mod: any
beforeAll(async () => {
  mod = await import("@/app/api/cron/refresh-serial-fmv-multipliers/route")
})

describe("POST /api/cron/refresh-serial-fmv-multipliers", () => {
...
describe("POST /api/cron/refresh-serial-fmv-multipliers — success path", () => {
  it("202-accepts with the correct bearer token (compute deferred to after())", async () => {
    const res = await mod.POST(makeReq({ method: "POST", auth: "Bearer test-ingest-token" }))
    expect(res.status).toBe(202)
    const body = await res.json()
    expect(body.ok).toBe(true)
    expect(body.accepted).toBe(true)
    expect(body.pipeline).toBe("refresh-serial-fmv-multipliers")
  })

...
// Route integration test for /api/ufc-listings-indexer (POST run + GET alias).
...
const url = "https://t/api/ufc-listings-indexer"

describe("POST /api/ufc-listings-indexer — no secret configured (fail-closed)", () => {
  let POST: (req: any) => Promise<Response>
  let GET: (req: any) => Promise<Response>
  beforeAll(async () => {
    vi.resetModules()
    delete process.env.INGEST_SECRET_TOKEN
    const mod = await import("@/app/api/ufc-listings-indexer/route")
...
describe("POST /api/ufc-listings-indexer — secret configured (success path)", () => {
  const TOKEN = "ufc-listings-token"
  let POST: (req: any) => Promise<Response>
  let GET: (req: any) => Promise<Response>
  beforeAll(async () => {
    vi.resetModules()

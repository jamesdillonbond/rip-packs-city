...
// Route integration test for GET /api/topshot-listing-cache (GET only export).
...
const url = "https://t/api/topshot-listing-cache"

describe("GET /api/topshot-listing-cache — no secret configured (fail-closed)", () => {
  let GET: (req: any) => Promise<Response>
  beforeAll(async () => {
    vi.resetModules()
    delete process.env.INGEST_SECRET_TOKEN
    process.env.FLOWTY_PROXY_TOKEN = "test-flowty-token"
    const mod = await import("@/app/api/topshot-listing-cache/route")
    GET = mod.GET as any
  })
...
describe("GET /api/topshot-listing-cache — secret configured (accept path)", () => {
  const TOKEN = "topshot-listing-cache-token"
  let GET: (req: any) => Promise<Response>
  beforeAll(async () => {
    vi.resetModules()
  ...
  it("200s and reports the background-accept with the correct bearer token", async () => {
    ...
    expect(body.message).toContain("topshot-listing-cache")
  })

...
const payload = {
  ...
  timeline: [{ d: "2026-08-16", acquisitions: 5000, priced_acquisitions: 0, spend_usd: null }],
}
...
describe("BuybackDashboard", () => {
  it("renders the unpriced wallet WITHOUT claiming it spent $0", async () => {
    ...
    expect(container.textContent).toMatch(/no price is recorded on-chain/i)
  ...
  it("states the spend-coverage caveat beside the headline spend figure", async () => {
    fetchMock.mockReturnValue(jsonResp(payload))
    const { container } = render(<BuybackDashboard />)
    await waitFor(() => expect(container.textContent).toContain("Spend is known for"))
    expect(container.textContent).toContain("99.7%")
    expect(container.textContent).toMatch(/unknown cost/i)
  })
  ...
  it("renders the acquisition leaderboard, which IS fully known", async () => {
    fetchMock.mockReturnValue(jsonResp(payload))
    const { container } = render(<BuybackDashboard />)
    await waitFor(() => expect(screen.getByText(/Grant Williams/)).toBeTruthy())
    expect(container.textContent).toContain("1,014")
  })
  ...
  it("a failed read says so and does NOT render an empty-board claim", async () => {
    ...
    expect(container.textContent).not.toMatch(/No buyback activity in this window/i)
  ...
  it("a thrown fetch is handled the same as a 5xx", async () => {
    ...
    expect(container.textContent).not.toMatch(/No buyback activity/i)
  })
  ...
  it("a genuinely empty window reads as an honest zero, not as a failure", async () => {
    fetchMock.mockReturnValue(
      jsonResp({
        ...
        coverage: { ...payload.coverage, unpriced_acquisitions: 0, unpriced_share_pct: null },
        wallets: [],
        top_editions_by_count: [],
        top_editions_by_spend: [],
        top_sellers_by_spend: [],
    ...
    const { container } = render(<BuybackDashboard />)
    await waitFor(() =>
      expect(container.textContent).toMatch(/No buyback activity in this window/i)
    )
    // The failure banner must NOT appear for a real empty result...
    expect(container.textContent).not.toMatch(/could not be loaded/i)
    // ...and the coverage caveat is suppressed when nothing is unpriced, rather

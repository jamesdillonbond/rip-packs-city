...
// Route integration test for POST /api/cron/allday-sales-history-backfill.
// Fail-closed auth: the handler accepts Bearer INGEST_SECRET_TOKEN / CRON_SECRET
// (or ?token=) and 401s on a missing/wrong credential before any backfill. We
// pin that guard.

const req = (auth?: string) =>
  ({
    headers: new Headers(auth ? { authorization: auth } : {}),
    nextUrl: new URL("https://t/api/cron/allday-sales-history-backfill"),
  }) as any

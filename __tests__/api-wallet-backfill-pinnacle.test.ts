...
// Route integration test for POST /api/wallet-backfill-pinnacle. Bearer
// INGEST_SECRET_TOKEN gated → fail-closed 401. The Pinnacle Cadence walk runs
// in after() (stubbed no-op) so the fire-and-forget accept (202) is observable;
// resolveWalletInput short-circuits for a valid 0x address. Sync-mode is left
// uncovered (it drives the real runPinnacleDetailsBackfill inline). Mocks
...
describe("POST /api/wallet-backfill-pinnacle — fail-closed auth", () => {
...
describe("POST /api/wallet-backfill-pinnacle — secret configured (success + body guards)", () => {

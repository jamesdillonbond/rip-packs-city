...
// Route integration test for POST /api/wallet-backfill-allday. Bearer INGEST_SECRET_TOKEN
// gated → fail-closed 401, then the body guards. Beyond that this drives BOTH
// execution modes by mocking runAllDayDetailsBackfill (an earlier revision of this
...
vi.mock("@/lib/chains/flow/wallet-backfill-helpers", async (importOriginal) => ({
  ...
  runAllDayDetailsBackfill: async (args: any) => { st.runArgs.push(args); return st.result },
}))
...
describe("POST /api/wallet-backfill-allday — auth + body guards", () => {
...
describe("POST /api/wallet-backfill-allday — default fire-and-forget", () => {
...
describe("POST /api/wallet-backfill-allday — ?sync=true checkpoint mode", () => {
  const sync = (qs = "") =>

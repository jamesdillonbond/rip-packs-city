...
// Route integration test for POST /api/wallet-backfill-golazos. Bearer
// INGEST_SECRET_TOKEN gated → fail-closed 401. The Golazos Cadence walk runs in
// after() (stubbed no-op) so the fire-and-forget accept (202) is observable;
// resolveWalletInput short-circuits for a valid 0x address. Mocks supabaseAdmin
// + wallet-backfill-helpers.
...
const url = "https://t/api/wallet-backfill-golazos"
const req = (headers: Record<string, string> = {}, body: any = {}) =>
  ({ headers: new Headers(headers), json: async () => body }) as any

describe("POST /api/wallet-backfill-golazos — fail-closed auth", () => {
...
describe("POST /api/wallet-backfill-golazos — secret configured (success + body guards)", () => {
  ...
  it("202-accepts with the correct bearer token + a valid wallet (walk deferred to after())", async () => {
    ...
    expect(body.collection).toBe("laliga_golazos")

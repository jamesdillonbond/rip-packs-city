  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Packs tracked</div>
          <div className="text-2xl font-bold text-slate-50 tabular-nums">{formatNumber(tracked)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Sellable</div>
          <div className="text-2xl font-bold text-slate-50 tabular-nums">{formatNumber(sellable)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Positive EV</div>
        ...
        <div>
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Avg ratio</div>
          <div className="text-2xl font-bold text-slate-50 tabular-nums">{formatRatio(stats.avg_value_ratio)}</div>
        </div>
      </div>

      <div className="mt-4 flex flex-wrap items-center gap-3 text-[11px] text-slate-400">
        <span>
          Median price <span className="text-slate-200">{formatUsd(stats.median_pack_price)}</span>
        </span>
        <span className="text-slate-700">·</span>
        <span>
          Unopened <span className="text-slate-200">{formatNumber(stats.total_unopened)}</span>
        </span>
      </div>
    </div>
  )
...
function MutedPanel({ label }: { label: string }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/30 p-5 opacity-70">
      <h3 className="text-base font-semibold text-slate-300">{label}</h3>
      <p className="mt-2 text-sm text-slate-500">Pack analytics not yet available for this collection.</p>
    </div>
  )
}
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      <div className="flex flex-col gap-2">
        <div>
          <h1 className="text-2xl font-bold text-slate-50 tracking-tight">Pack Analytics</h1>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Packs Tracked</div>
          <div className="mt-1 text-2xl font-bold text-slate-50 tabular-nums">{formatNumber(kpis.tracked)}</div>
          <div className="text-[11px] text-slate-500 mt-1">across {summaryCollectionKeys.length} collections</div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Positive-EV Packs</div>
          <div className="mt-1 text-2xl font-bold tabular-nums" style={{ color: "var(--rpc-success)" }}>
            {formatNumber(kpis.positive)}
          </div>
          <div className="text-[11px] text-slate-500 mt-1">{formatPct(kpis.pctPositive, 1)} of tracked</div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Median Pack Price</div>
          <div className="mt-1 text-2xl font-bold text-slate-50 tabular-nums">{formatUsd(kpis.medianPrice)}</div>
          <div className="text-[11px] text-slate-500 mt-1">across collections</div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Avg Value Ratio</div>
          <div className="mt-1 text-2xl font-bold text-slate-50 tabular-nums">{formatRatio(kpis.avgRatio)}</div>
          <div className="text-[11px] text-slate-500 mt-1">weighted by packs</div>
        </div>
      </div>
      ...
      <section>
        <div className="mb-4">
          <h2 className="text-lg font-semibold text-slate-100">Per-collection breakdown</h2>
          <p className="text-xs text-slate-500">Pack inventory, EV, and depletion by ecosystem</p>
        </div>
        {loading && !summary ? (
      ...
      <section>
        <div className="mb-4 flex items-baseline justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">Highest value-ratio packs currently listed</p>
          </div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
          {loading && !topEv ? (
            ...
            <div className="p-8 text-center text-sm text-slate-500">No positive-EV packs in this filter.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {topEv.map((r) => (
                    <tr key={r.pack_listing_id} className="border-b border-slate-800/60 hover:bg-slate-900/40 transition-colors">
                      <td className="py-3 px-4 text-slate-500 tabular-nums">{r.rank}</td>
                      ...
                      <td className="py-3 px-3 text-slate-100 font-medium">{r.pack_name ?? "—"}</td>
                      <td className="py-3 px-3 text-right text-slate-300 tabular-nums">{formatUsd(r.pack_price)}</td>
                      <td className="py-3 px-3 text-right text-slate-100 font-semibold tabular-nums">{formatUsd(r.pack_ev)}</td>
                      <td
                      ...
                      <td className="py-3 px-3 text-right text-slate-300 tabular-nums">{formatPct(r.fmv_coverage_pct, 0)}</td>
                      <td className="py-3 px-3 text-right text-slate-300 tabular-nums">{formatNumber(r.total_unopened)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section>
        <div className="mb-4">
          <h2 className="text-lg font-semibold text-slate-100">Fresh drops</h2>
          <p className="text-xs text-slate-500">Packs first seen in the last 24h</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 overflow-hidden">
          {loading && !fresh ? (
            <div className="h-32 animate-pulse bg-slate-900/60" />
          ) : !fresh || fresh.length === 0 ? (
            <div className="p-8 text-center text-sm text-slate-500">No new pack listings in the last 24 hours.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {fresh.map((r) => (
                    <tr key={r.pack_listing_id} className="border-b border-slate-800/60 hover:bg-slate-900/40 transition-colors">
                      ...
                      <td className="py-3 px-3 text-right text-slate-300 tabular-nums">{formatNumber(r.total_unopened)}</td>
                      <td className="py-3 px-3 text-right text-slate-500 tabular-nums">{relativeTime(r.first_seen_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

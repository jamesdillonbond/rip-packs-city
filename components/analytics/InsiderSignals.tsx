  ...
  return (
    <section className="rounded-xl border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-5">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-[color:var(--rpc-text-primary)]">Insider Signals</h2>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {/* Alerts */}
        <div>
          <h3 className="mb-2 text-[10px] uppercase tracking-widest text-[color:var(--rpc-text-muted)] font-semibold">
            Alerts
          </h3>
          {alerts.length === 0 ? (
            <p className="text-[12px] text-[color:var(--rpc-text-muted)]">No active alerts.</p>
          ) : (
            <ul className="space-y-2">
              {alerts.map((a) => (
                <li key={a.id} className="rounded border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-2.5">
                  <div className="flex items-center gap-1.5">
                    <span
                      ...
                      aria-hidden
                    />
                    <span className="text-sm font-semibold text-[color:var(--rpc-text-primary)] truncate">
                      {a.title ?? "Insider alert"}
                    </span>
                  </div>
                  {a.summary ? (
                    <p className="mt-1 text-[11px] text-[color:var(--rpc-text-secondary)] leading-relaxed">{a.summary}</p>
                  ) : null}
                  <div className="mt-1 text-[10px] text-[color:var(--rpc-text-muted)]">{fmtRelative(a.generated_at)}</div>
                </li>
              ))}
            </ul>
          )}
            ...
            <ul className="space-y-2">
              {visibleBuybacks.map((b) => (
                <li key={b.id} className="rounded border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-2.5">
                  <div className="text-sm font-semibold text-[color:var(--rpc-text-primary)]">Insider buyback detected</div>
                  <div className="mt-1 truncate text-[11px] text-[color:var(--rpc-text-secondary)]">
                    {b.player_name ?? "Unknown moment"}
                    {b.set_name ? <span className="text-[color:var(--rpc-text-muted)]"> · {b.set_name}</span> : null}
                    {b.serial_number ? <span className="text-[color:var(--rpc-text-muted)]"> · #{b.serial_number}</span> : null}
                  </div>
                  <div className="mt-1 flex items-center justify-between">
                    <span className="text-[12px] font-bold text-[color:var(--rpc-text-primary)] tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>
                      {fmtUsd(b.price_usd)}
                    </span>
                    <span className="text-[10px] text-[color:var(--rpc-text-muted)]">{fmtRelative(b.sold_at)}</span>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div>
          ...
          {announcements.length === 0 ? (
            <p className="text-[12px] text-[color:var(--rpc-text-muted)]">No recent announcements.</p>
          ) : (
            <ul className="space-y-2">
              {announcements.map((a) => (
                <li key={a.id} className="rounded border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-2.5">
                  <div className="flex items-center gap-1.5">
                    {a.source ? (
                      <span className="rounded border border-[color:var(--rpc-border)] bg-[color:var(--rpc-surface-raised)] px-1.5 py-0.5 text-[9px] uppercase tracking-wider font-semibold text-[color:var(--rpc-text-secondary)]">
                        {a.source}
                      </span>
                    ) : null}
                    <span className="text-sm font-semibold text-[color:var(--rpc-text-primary)] truncate">
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    <span className="text-[10px] text-[color:var(--rpc-text-muted)]">{fmtRelative(a.posted_at)}</span>
                    {a.source_url ? (
                      <a
                        href={a.source_url}
                        target="_blank"

  ...
  return (
    <div ref={wrapperRef} className="relative inline-block">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-2 rounded-md border border-slate-800 bg-slate-900/60 px-2.5 py-1.5 text-xs hover:border-slate-700 transition-colors"
        aria-expanded={open}
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-xl border border-slate-800 bg-slate-950/95 backdrop-blur shadow-xl shadow-black/50 p-3">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-xs font-semibold text-slate-200">Pipeline status</h4>
            {data?.as_of ? (
            ...
            <ul className="space-y-2">
              {PIPELINE_ORDER.map((key) => {
                ...
                return (
                  <li
                    key={key}
                    className="flex items-center justify-between gap-2 rounded-md border border-slate-800/60 bg-slate-900/40 px-2.5 py-2"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px] text-slate-500">{row.cadence}</div>
                      </div>
                    </div>

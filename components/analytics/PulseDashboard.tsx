...
const KIND_CONFIG: Record<PulseActivityKind, KindConfig> = {
  ...
  sale: {
    label: "Sale",
    icon: ShoppingCart,
    className: "border-[color:var(--rpc-border)] bg-[color:var(--rpc-surface-raised)]",
    iconClassName: "text-[color:var(--rpc-text-secondary)]",
  },
}
...
function HourlyTooltip({
  ...
  return (
    <div className="rounded-md border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] px-3 py-2 text-xs text-[color:var(--rpc-text-secondary)]">
...
function HourlySparkline({ rows }: { rows: PulseHourlyRow[] }) {
  ...
  if (!points || points.length === 0) {
    return (
      <div className="flex h-32 items-center justify-center rounded-lg border border-dashed border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] text-xs text-[color:var(--rpc-text-muted)]">
...
function ActivityRow({
  ...
  return (
    <article
      className={
        "flex items-start gap-3 rounded-lg border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-3 transition-colors " +
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
          <p className="text-sm text-[color:var(--rpc-text-primary)] font-medium">{summary}</p>
          {anon ? (
            <span className="rounded border border-[color:var(--rpc-border)] px-1.5 py-px text-[9px] uppercase tracking-wider font-semibold text-[color:var(--rpc-text-secondary)]">
              Centralized · anon
            </span>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[color:var(--rpc-text-muted)] mt-1">
          <span>{relativeFromNow(row.occurred_at)}</span>
          {!anon && row.primary_addr ? (
            <span className="flex items-center gap-1">
              <span className="text-[color:var(--rpc-text-ghost)]">addr</span>
              {isLinkableAddr(row.primary_addr) ? (
                ...
                <span className="font-mono text-[color:var(--rpc-text-secondary)]">{truncateAddr(row.primary_addr)}</span>
              )}
            </span>
          ) : null}
          {!anon && row.counterparty ? (
            <span className="flex items-center gap-1">
              <span className="text-[color:var(--rpc-text-ghost)]">cp</span>
              {isLinkableAddr(row.counterparty) ? (
                ...
                <span className="font-mono text-[color:var(--rpc-text-secondary)]">{truncateAddr(row.counterparty)}</span>
              )}
            </span>
          ) : null}
          <button
            type="button"
            onClick={() => setOpen((o) => !o)}
            className="ml-auto inline-flex items-center gap-0.5 text-[color:var(--rpc-text-muted)] hover:text-emerald-400 transition-colors"
          >
        ...
        {open ? (
          <pre className="mt-2 overflow-x-auto rounded border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-2 text-[10px] text-[color:var(--rpc-text-secondary)] font-mono">
...
export default function PulseDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* "as_of" caption */}
      <div className="flex flex-wrap items-center gap-2 text-xs text-[color:var(--rpc-text-muted)]">
      ...
      <section className="rounded-xl border border-[color:var(--rpc-border)] bg-[var(--rpc-surface)] p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-sm font-semibold text-[color:var(--rpc-text-primary)]">Last 24 hours</h2>
            <p className="text-xs text-[color:var(--rpc-text-muted)]">Hourly buckets · sales vs loans</p>
          </div>
        </div>
      ...
      <section>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
          ...
          <label className="flex items-center gap-2 text-xs text-[color:var(--rpc-text-secondary)]">
            ...
            <input
              ...
              placeholder="$"
              className="w-20 rounded-md border border-[color:var(--rpc-border)] bg-[color:var(--rpc-surface-raised)] px-2 py-1 text-xs text-[color:var(--rpc-text-secondary)] placeholder:text-[color:var(--rpc-text-ghost)] focus:border-emerald-500/50 focus:outline-none"
            />
          </label>
        </div>

        <div className="rounded-lg border border-[color:var(--rpc-border)] bg-amber-950/10 border-amber-900/30 p-3 flex items-start gap-2 mb-3">

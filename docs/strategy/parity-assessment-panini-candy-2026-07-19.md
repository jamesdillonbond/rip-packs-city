...
>
> 📌 **What IS still a genuine Candy exclusion, and it is an explicit `IN`-list rather than a `CASE`** — so "add the `CASE` arms" would not have touched any of it: `get_market_pulse_all` and `get_market_pulse_windows` (`WHERE c.slug IN ('nba_top_shot','nfl_all_day','laliga_golazos','ufc_strike')` — excludes Pinnacle too, so it is a wider question than Candy), `capture_institutional_wallet_snapshot` (5-slug list) and `claim_sales_counterparty_batch` (`IN ('nba_top_shot','nfl_all_day','ufc_strike')` — excludes Golazos and Pinnacle as well). Open, named, not yet done.
>

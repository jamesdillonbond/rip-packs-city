...
**2. Match the queries that already show up — PARTLY SHIPPED 09-07.** `/insights/account-value` is now titled *NBA Top Shot Account Value Calculator — Free Value Tracker*; pack-dist pages (where "wnba gold slot" → the WNBA Metallic Gold LE packs, and "premiumchance" → the Premium Chance packs, both land) are titled *<Pack> — <Tier> Pack · Odds, Pulls & EV* and no longer publish `Retail $0.00` for an unrecorded price. Still open: a WNBA Gold explainer and a premium-chance explainer — these need Trevor's Top Shot knowledge of the pack "slot" mechanic (two honest paragraphs; I will not invent the mechanic). **Original finding:** 224 queries in 3 months, most with 0 clicks. They cluster into four intents the site has no dedicated, title-matched page for:
...
**3. Fewer, stronger pages — PARTLY SHIPPED 09-07.** Pack distributions with a known mint of 1–49 packs (All Day's median is 25 — reward/promo shells) or no title left the sitemap: 2,065 of 5,529. Unknown mints (Golazos) stay. Still open: editions with no sales and no holders; single-edition long-tail sets. **Original finding:** 34K URLs with zero authority is worse than 2K. Google's "Discovered – not indexed" (2,827) and "Crawled – not indexed" (4,448) are it saying the marginal page adds nothing. Candidates to drop from the sitemap (keep the pages, stop advertising them): pack-dist pages for closed markets and sold-out drops, editions with no sales and no holders, series pages, WNBA/UFC long-tail sets with a single edition. Keep: overview pages, insights boards, player and team pages, editions with activity. Rule of thumb: if a page would never be the best answer to any query, it should not be in the sitemap.

**4. Internal linking is inverted — PARTLY SHIPPED 09-07.** Shipped: `/pricing` out of the footer (+ sitemap, + noindex); the edition page's new *More from this player / set* block (`get_edition_related`, up to 6 sibling editions — it had linked to zero other editions); the *Same Play · Other Sets* and moment-page *Parallels* cards link straight to the canonical edition URL instead of `/moment/<edition uuid>` (a redirect hop for every reader and the source of the duplicate-URL family). Overview → editions turned out to already exist (18 edition / 12 player / 12 set links per overview, measured 09-07) — struck. **Original finding:** Top internal-link targets are `/pricing` (27,566 links — from the footer on every page, to a page that says the product is free), `/ufc/overview`, the insights boards — all nav/footer. The entity pages that carry the long-tail get one link each from their parent. Add "related" blocks: player page → that player's top 5 editions and team; edition page → player, set, and the 3 nearest editions by FMV; overview pages → the 20 most-traded editions this week. Take `/pricing` out of the footer (or noindex it) until there is a price.
...
> ⚠ **Owed measurement (09-07):** the other side of this call is that a price in a SERP title is the click magnet on price-comparison results. Compare edition-page CTR for the 30 days before vs after `9909fb8`; if it fell, the figure goes back in a stable form (rounded, e.g. "~$250").

...
**2. Match the queries that already show up — PARTLY SHIPPED 09-07.** `/insights/account-value` is now titled *NBA Top Shot Account Value Calculator — Free Value Tracker*; pack-dist pages (where "wnba gold slot" → the WNBA Metallic Gold LE packs, and "premiumchance" → the Premium Chance packs, both land) are titled *<Pack> — <Tier> Pack · Odds, Pulls & EV* and no longer publish `Retail $0.00` for an unrecorded price. Still open: a WNBA Gold explainer, a premium-chance explainer. **Original finding:** 224 queries in 3 months, most with 0 clicks. They cluster into four intents the site has no dedicated, title-matched page for:

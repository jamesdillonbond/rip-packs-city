...
## Sixteenth pass (6:52–7:35 AM PT) — R118, the biggest finding of the night

- ↩ **leg 324 at 6:52 AM (new hours): killed at 602 s too.** Four slots, four kills ⇒ the read is the lever: 4,477 parallel editions × ~385 sales each across seven partitions, `serial_number` fetched from the heap. Not moving it again. **Trevor's call:** covering `(edition_id) INCLUDE (serial_number)` on the seven `sales_*` partitions, or a watermark (baseline + incremental over `sold_at`, weekly full recount).
- 🚨 **R118 (P1):** `EXCEPTION WHEN OTHERS` does NOT catch `query_canceled` — PL/pgSQL excludes it by design. Every "a killed run still logs / writes 999" wrapper (49 use `WHEN OTHERS`, 1 names `query_canceled`, 12 claim timeout handling, all 8 trust legs) has been blind to statement-timeout kills. Proven with a 1.5 s budget + `pg_sleep(3)`; explains the missing 999s and terminal rows all night. **`20260920140611`** fixes the five unpinned pg_cron wrappers (`WHEN query_canceled OR OTHERS`, md5-gated regexp rewrite). **Control:** a deliberately killed leg (3 s budget) now writes `thp-leg-impossible-parallel` ok=false `57014: …`, 3,024 ms; before, only the heartbeat.
- 📝 **Owed:** the 8 pinned legs + 34 other functions (one deliberate migration, three-file discipline for the pinned ones) and a repo guard against new `WHEN OTHERS` in timeout-claiming functions. → **Both done by 8:10 AM PT (seventeenth pass).**
- ⚠ 7:00–7:13 AM PT: 27 `job startup timeout` — a `topshot_pack_sales_history` autovacuum (0.1 trigger, unthrottled) + wmc's 07:08Z pass; same launcher-blackout mechanism as 1:14 AM. `allday_pack_sales_history`'s map reads **13 %** ten hours after its last pass — churn re-dirties it faster than any cadence restores it; the 0.02→0.1 back-off was right and the map on that table should not be a target at all.

## Seventeenth pass (7:50–8:35 AM PT) — the R118 sweep, and leg 324's read re-shaped

- ✅ **R118 swept:** `20260920143546` (16 unpinned record-and-exit handlers, md5-gated regexp), `20260920143959` (the 14 pinned functions incl. all 8 legs as literal DDL from their pins), `20260920144120` `check_when_others_timeout_blind()` → `[]` (its first apply caught `collect_pack_nft_identity`, which the hand sweep missed; excluded with reason). 12 loop handlers left bare on purpose. The eight leg pins re-pointed to assert the 999 IS reachable on a 57014 (`89f9aa773`). **A concurrent Claude Code session shipped the repo guard** (`__tests__/new-plpgsql-recording-handlers-catch-query-canceled.test.ts`, forward-only over migrations after `20260920144120`) and rewrote `database.md`'s old "do not widen the clause" ban into the conditional rule — read that section before touching a handler; the rule is about the handler's TAIL being bounded, because the timer is not re-armed after the catch.
- ✅ **Leg 324's read re-shaped** (`20260920150414` / `151857` / `152137`): a stored per-MONTH baseline (`rpc_impossible_parallel_baseline`, 80 months seeded at 0) refreshed stalest-first by jobid 575 `rpc-impossible-parallel-baseline-rotate` (`22 19 * * *`, 12:22 PM PT, cron_heavy, soft 420 s budget) + a live-month probe (`rpc_impossible_parallel_count`). **Control 8:21 AM PT: 61.5 s, value 0, the arm's `computed_at` refreshed** for the first time since 5:48 PM. The live YEAR alone was 480 s (rejected); a covering CIC died at 120 s on the smallest partition (invalid index dropped). Rotation control (budget 300): 61 months in 320.6 s — the first month of each year pays the partition's cold read (2024-01 45.9 s), the rest 30–40 ms to 27 s; 19 months (2025-02 → 2026-08) remain for the first scheduled runs. **Bug caught by the pin before any tick:** stamping `now()` made stalest-first re-pick the same month 9,074 times; `clock_timestamp()` fixes it. `cron_heavy` has no table SELECT — controls go through the SECDEF function.
- ✅ **`/api/fmv` + `fetchFmvBatch` on `get_editions_latest_fmv_wide`** (`29ea0e4d9`; mutation-checked, 11 tests fail on a misspelled name). Twelve `fmv_current` readers remain on the view, listed in `database.md`.
- ✅ **Skills:** Trevor's `rpc-cron-ops` save confirmed (installed md5 == repo, 4:48 AM PT). `rpc-migration`, `rpc-cron-ops`, `rpc-audit-drain` and `rpc-nightly-autonomous-pass` sources extended with this thread's rules and re-packed — **save the three `propose_skills` cards in this thread** (rpc-migration / rpc-cron-ops / rpc-audit-drain); the nightly one is a one-line change, save it from the repo source when convenient. ⚠ Until saved, the installed `rpc-migration`, `rpc-handoff`, `rpc-data` and `rpc-artifact-ops` are BEHIND the repo (the 08-24 additions never reached the account).
- 📝 **CLAUDE.md:** R118, the hour-set rule and the per-id probe shape added; the "Windows / Git Bash" section displaced verbatim to `tooling-gotchas.md` (39,858 chars).

**Watches still open (PT):** 09-20 12:22 PM rotation (`thp-impossible-parallel-baseline` ok=true, `months_refreshed` ≥ 7) · 12:52 PM leg 324 (`succeeded` < 120 s, arm advances — the first real tick of the new read) · 09-21 2:36 AM reconcile (< 200 s) · 09-21 11:46 PM portfolio first real write · 09-27 2:16 AM VACUUM FULL · Sunday 8:23 / 8:43 PM reindex 477/478 + 9:03 PM verify.

## Eighteenth pass (11:20 AM–1:00 PM PT) — R123 drained on both halves, the two leg-324 watches read, and a live 403 attributed

- ✅ **R123 app/api half (`f5318f0f2`):** ten routes read the error of every Supabase write — Stripe entitlement failures answer 503 (Stripe retries); `edition-floor` and `allday-pack-listings` insert first then delete the replaced rows; the three offers indexers throw on a rejected write so the cursor never advances past it; four run rows learn `ok=false` + `write_errors`. Every fix mutation-checked; five defect-pinning tests inverted. Full vitest 1569/17,823 green.
- ✅ **R123 edge half (`6f094cc0d`):** eleven functions fixed in the repo (`deno check` clean — deno 2.9.6 installs from npm in the sandbox); redacted drift check per function; three live ones redeployed via MCP. `compute-allday-pack-ev`'s first tick on v52 proves upsert-then-prune (39 dists, 1,565 rows, 0 stale). Three functions with a hardcoded gate key still in production are repo-only (R21).
- ✅ **Leg 324, 12:52 PM PT: `succeeded` in 1.2 s** (four 600 s kills before the re-shape); arm 0, `computed_at` 12:52. **Rotation 12:22 PM: 80 months in 21.5 s** (the Large resize at 10:39:57 AM is the other half of that number). Both exits met by two orders of magnitude.
- 🚨 **`ingest-pinnacle-mints` 403 since 11:40 AM PT** — another session deployed v22→26 (env-gated) at 11:30–11:40 with no matching `PINNACLE_MINTS_GATE_KEY`; the sentinel's `pg_net_http_403` critical row is this. **Trevor: one `supabase secrets set`** (ledger entry has the exact recipe). Not caused by this thread; attributed by it.
- 📌 Main went red once today on a concurrent session's migration (`20260920185051`, the new R118 guard) and was greened by its author; not this thread's.

## Final close (1:35 PM PT) — nothing left that this thread can execute

- ✅ CLAUDE.md carries the R123 write-order rule (39,985 chars; two clauses displaced verbatim to `tooling-gotchas.md`); `key-files-and-honesty.md` has the three write-side sub-shapes; `rpc-edge-fn-deploy` has today's precondition and bundle facts; **#130** files the live Pinnacle-mints 403. Pushed `e8bbae06f`.
- ✅ Skills: `rpc-migration`, `rpc-cron-ops`, `rpc-audit-drain` saved by Trevor (installed == repo). Two more cards in the thread's last message: `rpc-edge-fn-deploy`, `rpc-nightly-autonomous-pass`.
- ✅ The Project's `trevor-queue` doc is re-cut: **one item, now urgent — `supabase secrets set PINNACLE_MINTS_GATE_KEY=<what jobids 83/84 send>`** (verified this thread cannot do it: no MCP secrets verb, no `SUPABASE_ACCESS_TOKEN` on the laptop VM). The other five gate-key secrets unblock the three repo-only R123 edge deploys.
- ⏳ Watches that outlive the thread: 09-21 2:36 AM reconcile · 09-21 11:00 AM the tier-upgrade verdict task · 09-21 11:46 PM portfolio write · 09-27 2:16 AM VACUUM FULL · Sunday 8:23/8:43 PM reindex 477/478.

## Close-out (7:40 AM PT) — this session's totals

Sixteen passes, ~14 h. **Shipped (DB):** 20260920044216 (wide helper) · 054402 (drift sample) · 064608/064646/121349 (leg 324 slots) · 095145 (reconcile probe shape) · 095221 (VACUUM FULL slot) · 140611 (R118 five wrappers). **Shipped (code):** `fa8ca839c` (wallet-search ×2 + cache-refresh on the wide helper, mutation-checked) · `cd30c3e32` (memory-doc self-link). **Verified exits:** jobid 506 (314 ms / 69 s), portfolio primary + route + retry, matcher pg_cron + edge, pack_rips pass (map 98.1 %, pack-detail errors 363 → 0), R118 control. **Failed exits, fixed forward:** reconcile (cold 600 s → probe 51 s), VACUUM FULL collision, leg 324 (four kills — the read is the lever, filed). **Needs Trevor:** R118 sweep (42 functions), leg 324's read (index vs watermark), R117 lever, the `rpc-cron-ops` card (#32), #22, #55, jobid 303, `portfolios` retirement, Golazos threshold. **Watches still open:** 12:52 PM PT leg 324 · 09-21 2:36 AM reconcile · 09-21 11:46 PM portfolio first real write · 09-27 2:16 AM VACUUM FULL · Sunday 8:23/8:43 PM reindex 477/478 + 9:03 PM verify.

...
## 5. Production packs page now shows TRUE supply (write-through, done live)

Since CC's seeder now preserves `total_minted`/`total_opened` on conflict, `apply_topshot_supply` was extended to write-through into `pack_distributions.total_minted`/`total_opened` (generated `total_sealed`/`depletion_pct` follow), and a one-time sync (`audit_20260628_sync_supply_to_pack_distributions`) populated all 1,990 dists. **Effect: `pack_table_rows`, `get_pack_detail`, `get_pack_for_simulator`, `get_pack_lifecycle`, `analytics_packs_*` now return TRUE minted/opened/sealed/depletion with no route/view change** — the packs page, dist page, and simulator light up automatically. `topshot_pack_supply` stays the durable source; this is the seeder-safe mirror into the columns the UI already reads. CC's "Observed pack lifecycle" strip (reads `v_topshot_pack_lifecycle` directly) is a richer per-dist surface and can still optionally add `realized_pull_value_usd`/`minted_true`, but the core supply numbers are now live on the page regardless.

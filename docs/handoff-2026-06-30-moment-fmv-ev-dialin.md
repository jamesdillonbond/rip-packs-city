...
### Feature-prior — stress-tested + CORRECTION (2026-06-30)

Built `v_topshot_fmv_feature_prior` (read-only/INERT) — comparable-cohort median FMV (tier×series×circ_band×rookie, cell>=5) + divergence flag. Validated: **39% median APE / 98% coverage** on held-out HIGH/MED editions (slightly beats the regression; SQL-native, refits trivially).

**CORRECTION to the earlier "divergence flags stale pack-EV FMV" claim — it does NOT.** Inspecting the 1,003 `over_3x` editions, they are almost entirely LEGIT STARS, not mispriced: Cooper Flagg Rookie Debut $1,876 (prior $6), LeBron Base Set $819 (prior $7), Jokić Hardware $8,999, a LaMelo 1/1 Holo Icon $9,000. The cohort prior is **player-blind** (no player-identity feature), so it massively under-prices stars and its divergence flag is dominated by star false-positives. So the "323 pool editions >3× prior = stale-chase" read was WRONG — most are correctly-priced stars.

Tried the fix (player-aware prior): a player-median prior is WORSE — 80% APE and only ~119/fold covered (most players have <3 priced editions, and within-player edition value varies hugely). player×circ-band: 66% APE, 57 covered.

**Honest bottom line:** there is no good feature-only substitute for sales on the high-value/star tail. The cohort prior is a usable rough fallback ONLY for the NON-STAR thin tail (and only ~31 of 188 NO_DATA editions even have comparable cells). It CANNOT price thin star editions or detect mispricing. This REINFORCES the core thesis: **edition FMV (built from sales, which already encode stardom) is the near-sufficient statistic; features can't shortcut it for the editions that matter most.** The thin tail is genuinely hard because those editions lack market signal — the real lever there remains more sales density (the ongoing backfills), not a feature prior. The view is kept as an honest cohort reference (its over_3x set is effectively a "star-premium" detector), not a pricing tool. Revert: `DROP VIEW public.v_topshot_fmv_feature_prior;`

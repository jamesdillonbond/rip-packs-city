...
- **Dist attribution is still grinding.** `resolve-allday-pack-dist` (cron `rpc-allday-resolve-pack-dist`, `*/2`) is sweeping the mint era (cursor ~152M → floor 148.9M, ~1-2h left). `pack_rips.dist_id` fills as it finds `PackNFT.Mint{distId}`. So lifecycle/realized-EV per-dist rows are sparse now and grow. The global aggregate is already meaningful.

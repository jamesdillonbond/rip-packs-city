...
⚠ **CORRECTION 2026-07-31 (later session) — the "hydrator gates AllDay attribution" chain recorded here was WRONG on both links. No Cloudflare deploy is required for AllDay pull attribution.**

1. **`topshot-moments-hydrator` is Top Shot only and can never attribute an AllDay pull.** `TOPSHOT_COLLECTION_ID` is a hardcoded constant at [index.ts:84](workers/topshot-moments-hydrator/index.ts#L84) and every read is `.eq("collection_id", TOPSHOT_COLLECTION_ID)` (lines 122, 238). Its backlog is a Top Shot pull-provenance / realized-EV-calibration issue only.
2. **AllDay pull attribution is not blocked — it runs at 100%.** Every AllDay pull opened in the last 10 days is attributed (10/10 days, 100.0%). The NULLs have a **clean cutover at 2026-01-01**: Jan 2026 onward is 0.0% NULL; May–Dec 2025 is 66–88% NULL. That is an un-backfilled historical tail, not a capability gap.

**The actual gap, and it is self-replenishing.** `allday-pack-opens-backfill` is walking 2025 right now, inserting 44k–61k pull rows/day while attributing only **6.2–22.1%** of them (event dates marched 2025-08-24 → 2025-05-14 over four days). It digs the hole faster than local backfills can fill it — the 2026-07-31 backfill cleared 72,143 rows and the NULL count still rose ~1,500 net. **Do not chase this with more local backfills; it is bailing water.**

**The drain exists and was never wired.** `get_allday_unresolved_pulls(int)` returns a full queue on demand (`SELECT count(*) FROM get_allday_unresolved_pulls(500)` → 500) and has **zero callers anywhere in the repo** (verified across `.ts/.tsx/.sql/.mjs/.json`, migrations excluded). It returns `(moment_nft_id text, opener_address text, block_height bigint)` — i.e. inputs for an **on-chain Cadence lookup**, not the CF-1009-blocked AllDay consumer GraphQL. The transport already exists and is already in use: `allday-pack-opens-backfill` reports `routed: "spork"`, and `allday-edition-resolver` carries a `cadence_fallback_max` path.

**CORRECTED AGAIN (same day) — the cause is STRUCTURAL, and the drain is healthy.** `get_allday_unresolved_pulls` IS called, by the deployed-but-ungitted edge fn `resolve-allday-pull-editions` via **pg_cron jobid 22 (`9,39 * * * *`, active; 1,055 succeeded)** — a repo grep cannot see a pg_cron→HTTP caller, so check `cron.job` before calling a function dead. `allday-edition-resolver` is likewise fine: it is a synthetic pipeline row from the AllDay **sales** indexer's edition-**catalog**-fill stage, and `0 hydrated` is healthy on a complete catalog. It has no bearing on pull attribution.

The real limit: resolution borrows each moment **at its open block** via Flow REST `/v1/scripts`. Forward opens are inside Flow's execution-state window → 100%. Historical opens are not — rest-mainnet prunes execution state and `spork-proxy` fronts only `/v1/events` + `/v1/transactions`, **not `/v1/scripts`** — so they resolve only where the moment is still borrowable today, giving the observed 6–22%. Local mining is exhausted (of 293,274 NULL moments: `moments` 0, `wmc` 95, `nft_edition_map` 114).

**Plan around this, not against it:** an opens-derived AllDay remaining pool is buildable for **2026-01-01 onward only**. Pre-2026 pull attribution is largely unrecoverable — scope the model to 2026+ opens or ship an explicit coverage disclosure. "Finish the AllDay attribution backfill" is not an achievable task.

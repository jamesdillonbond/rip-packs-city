...
**And that resolver is silently no-op'ing:** `allday-edition-resolver` runs 257×/14d, `ok=true`, 0 fails — with `editions_hydrated_from_chain: 0` and `editions_hydrated_from_relay: 0` on every recent tick. Classic ok=true-with-zero-rows silent degradation. **Start here, not with a worker deploy:** find out why the resolver hydrates nothing, and wire the unresolved-pulls drain to the same Cadence/spork path the opens backfill already uses.

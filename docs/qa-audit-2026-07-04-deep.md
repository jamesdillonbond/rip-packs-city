...
## F1 extension — DONE via authoritative TopShot GQL sweep (1,514 more editions)
The `gross − Σ(children)` heuristic was correctly **rejected** (the gross=4099 group has an edition — Jalen Duren 218:8061 — with an owned serial **7,999** that's a mis-keyed wmc row, disproving the heuristic). Instead, did it right: swept TopShot GQL `searchMarketplaceEditions` for the per-parallel `circulationCount` (parallelID 0 = Standard), mapping GQL set UUID → `sets.set_id_onchain` (set.flowId is a `0` sentinel; play.flowID is reliable). **Established `gql_circ` = the GROSS Standard mint** — it equals all 284 already-fixed editions (1000) and exceeds max owned serial for illiquid editions.
- **Shipped** (`audit_20260704_fix_topshot_circulation_from_gql_authoritative`): corrected **1,514 Standard editions across 40 sets** to the authoritative GQL circulation (1,511 over-counts corrected down + 3 null populated). Verified e.g. Jarrett Allen 245:8436 654→464, Anthony Edwards 245:8438 1026→836.
- **Safety guard:** only applied where `gql_circ >= max owned serial` — so the corrected mint never drops below a real serial, and mis-key-contaminated editions are skipped (Jalen Duren 218:8061 left at 4099, flagged). Reversible via the shared backup table (now 1,798 rows total).
- Post-fix: security invariants 0, fmv_sanity 0, editions flat (17,490); serial-owners MV refreshed. **Total F1 across both phases: 1,798 editions corrected.**
- *Residual:* 1 mis-key edition (Jalen Duren — stray wmc serial 7999 vs gql 4000) needs the bad wmc row cleaned before its circ can be set to 4000; and ~2,400 canonical editions weren't in the marketplace sweep (retired/inactive) so weren't re-verified — a future full-catalog (incl. retired) sweep would close those.

...
## F1 extension — INVESTIGATED, DECLINED (would corrupt data)
The same `gross − Σ(children)` shape appears on other groups (4099→4000 ≈213 eds, 284→249, 234→199, 164→129…), but on-chain evidence **disproves** the heuristic there: in the gross=4099 group, an edition has an **owned serial 7,999** (> the 4,000 candidate), i.e. RPC's circulation is an *undercount* for that edition, not the clean parallel-*over*count of the 1,149 family. So `gross − Σ(children)` is unreliable for the older sets (mixed over/undercounts; gross ≠ Standard+parallels). Blindly applying it would set wrong values. **Correct path (larger, deferred):** an authoritative per-edition circulation backfill from topshot-proxy GQL `searchEditions` (parallelID-0 `circulationCount`) with validation — an ingest task, not a DB heuristic. The confirmed, airtight 1,149→1,000 family (284 eds) remains the shipped scope.

## F2 — needs a product decision (NOT auto-fixed, by design)
`get_edition_badges_unified` already curates via an allowlist (`topshotdebut, rookieyear, rookiemint, rookiepremiere, mvpyear, championshipyear, rookieoftheyear, allstar, threestarrookie`) and excludes gameplay tags — so it is *not* dumping raw tags. The residual mismatch (RPC shows `Rookie Year`/`Rookie Premiere`/`Rookie Mint` where TS shows fewer) is because **TopShot awards those badges by specific criteria, not merely "tag present."** Replicating TS's exact rules would need reverse-engineering; simply removing those titles from the allowlist would **under-award** on the many moments where they ARE correct. This is a genuine parity-vs-enrichment product call for Trevor — deliberately left unchanged rather than guessed. (Recommendation: if parity is the goal, derive TS's per-badge criteria from a sample of TS moment pages before touching the allowlist; if enrichment is the goal, visually label flag-sourced "TopShot badges" distinctly from tag-sourced ones.)

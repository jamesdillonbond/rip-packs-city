...
- **Durable fix — `audit_20260704_trg_normalize_circulation_v3_per_family`:** a `BEFORE INSERT OR UPDATE OF circulation_count` trigger on `editions` normalizes at write time, per family:
  - **Base Set / WNBA Base Set** (Club Collection `::16` /99 only; **variable-mint** standards e.g. Jalen Duren 218:8061 = 8,500): subtract exactly 99 when `circ % 100 = 99`. Safe for variable mints — a genuine round std (`mod100=0`) is untouched; `8,599→8,500`, `4,099→4,000` both correct. (A floor-to-1000 rule — my discarded v2 — would have destroyed the variable mints; caught and reverted.)
...
- **Separate pre-existing issue surfaced — variable-mint undercount (getMinted follow-up).** A few Base Set variable-mint editions are *undercounted* by the writer's `searchMarketplaceEditions.circulationCount` (Jalen Duren reported 4,000 but serials run past 4,800; the authoritative value is `getMintedMoment.edition.circulationCount` = 8,500). Duren was **restored to 8,500** (from `badge_editions`/the morning backup). **Vince Williams Jr. 218:8240** is the same class (serials to 7,959, all sources report 4,099) but has no captured getMinted value, so it's left flagged — this is the morning's documented "F1 tail: authoritative getMinted circulation backfill," and the durable fix is to make the **writer** use `getMintedMoment` for circulation (not `searchMarketplaceEditions`), after which the trigger's −99 keeps the club fold off.

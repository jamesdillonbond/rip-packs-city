...
**F7 — FIXED.** Migrations `audit_20260704_fix_base_set_club16_circulation` (+`_part2`) corrected `editions.circulation_count` from `X,099 → X,000` on **419 Series 2025-26 Base Set + WNBA Base Set editions** (the Club Collection `::16` /99 parallel had been folded into the base). Signature-gated to canonical integer editions with `circulation_count % 100 = 99` (all resolved to a clean `/1,000` or `/4,000`); floor-safe (0 owned serials exceed the corrected standard); inert UUID-dupe editions excluded (`external_id ~ '^[0-9]+:[0-9]+$'`). Part 1 hit the 220 with a catalogued `::16` child; Part 2 the remaining 199 whose `::16` child RPC never catalogued (incl. the verified Obi Toppin / Dalton Knecht / Kuminga). Old values in `public.audit_20260704_editions_circ_club16_fix` (419 rows, RLS-on). Then `refresh_topshot_special_serial_owners_mv()`. Verified: spot-checks now `218:8775=4000`, `218:8757=4000`, `218:8339=1000`, `258:8679=4000`; 0 inflated canonical editions remain; security invariants 0, `secdef_anon` [], fmv_sanity 0, editions flat (17,490). Edition pages self-correct on the 600s ISR cache. **Revert:** `UPDATE editions e SET circulation_count = b.old_circulation FROM audit_20260704_editions_circ_club16_fix b WHERE e.id=b.id AND e.circulation_count = b.old_circulation-99;` then `refresh_topshot_special_serial_owners_mv()`; `DROP TABLE audit_20260704_editions_circ_club16_fix`.

...
### Item B — AllDay V1-Dapper price-recovery drain [LOW, the unmapped_sales drift]
unmapped_sales open crept to 247, **all AllDay V1-Dapper** (`source=onchain_dapper_v1`): 199 already edition-mapped but `price_usd` 0/null/$1 (price-uncertain — `promote_unmapped_sales` correctly won't promote; `get_unmapped_resolver_targets` correctly skips them) + 47 with no edition map (retry-retired in unmapped_sales_resolution_failures). These are V1 sales whose DUC price extraction failed. The recovery path exists — `/api/admin/recover-v1-budget-exhausted` re-decodes the tx — but isn't on a schedule, so they accumulate ~9/day net. Fix: wire a low-frequency cron to call the recovery route (Bearer INGEST_SECRET_TOKEN) and re-promote; investigate the 47 unresolvable nft_ids (likely genuinely-uncertain V1 txs). Not user-critical (AllDay secondary), but it's the source of the drift + likely feeds Item C.

...
- **`ufc_fmv_stale_hours` — ⚠ MY "lagging consequence of the wedge, should self-clear" READING WAS WRONG. Cowork caught it; the mechanism is now traced and the arm is RED PERMANENTLY BY DESIGN.**
  - **The disqualifying fact was available when I wrote it:** UFC's last FMV write is **2026-08-04 06:31:44Z**, ~22h *before* the wedge window opened. A wedge on 08-05 cannot explain a writer that stopped on 08-04. I adopted a causal story without checking whether the timeline permitted it — one `max(computed_at)` away.
  - **Mechanism (traced to the line, `app/api/fmv-recalc/route.ts:1548`):** the only writer keeping UFC fresh was **Step 6**, the `force_stale` liveness touch, which selects `WHERE l.confidence IN ('HIGH','MEDIUM') AND rt.edition_id IS NULL` — editions whose latest snapshot is HIGH/MEDIUM with no in-window sales. UFC's 15 editions (2 HIGH + 13 MEDIUM, zero sales since 2026-05-13) were exactly that set, re-stamped daily by the `computed_at < now() - 24h` gate — **which also explains the metronomic +20 min/day drift.** On **2026-08-03 20:11:52Z** Step 6 wrote them a final time and the same-day `fmv_snapshots_cap_closed_market_confidence` trigger rewrote HIGH/MEDIUM → **STALE** on insert. STALE fails Step 6's own predicate. **Self-terminating loop — the writer switched itself off, permanently, as a correct consequence of the closure work.**
  - **Verified:** UFC now holds **zero** HIGH/MEDIUM latest snapshots (314 NO_DATA / 149 STALE), and the trigger caps any future one on insert. The value grows without bound; **no threshold can make it green.**
  - ⚠ **Also corrects Cowork's own caveat** that "this arm is the only instrument covering that writer." It isn't — Step 6 is collection-agnostic by its own design comment, excludes only Pinnacle and ULTIMATE, and runs daily for Top Shot, All Day, Golazos and Candy, each with its own freshness arm. **Retiring or re-basing loses no coverage.**
  - **Decision owed (Trevor):** retire the arm, or re-point it at something that can still move. Precedent: the `ufc_sales` CRITICAL arm was suppressed 2026-08-02 for exactly this reason. **Do not merely raise `breach_at`** — an unbounded value defers the crossing without making the reading honest. The mechanism is now recorded in the arm's own `catches` text so nobody re-investigates.

...
### R123 — the twelfth shape swept fleet-wide (2026-09-20): three sub-shapes, and the fix each one takes

The sweep that followed R120 found the same four ingredients in **10 `app/api` routes and 11 edge functions** (register R123). Every instance is shipped (`f5318f0f2`, `6f094cc0d`); what generalises is that the write side has three sub-shapes with three different fixes, and "log the error" is the right fix for none of them.

1. **The MONEY / ENTITLEMENT write** (`stripe/webhook`): the caller has a retry machine and the write's failure must reach it. `pro_users` was upserted with no binding and the handler returned 200 to Stripe regardless — *paid, not granted Pro, never retried*. The fix is the status code: a rejected write (or a rejected LOOKUP — "no row" and "the read failed" are not the same fact) answers the same retryable **503** the invoice path already used. ⛔ **A failed lookup that falls into the "no row → no-op" branch is the read-side class hiding inside the write side.**
2. **The CURSOR / STATE write** (offers indexers, pack-opens ingests, pinnacle mints, the parallel probe, the seed walker): a rejected write here is unrecoverable progress, because the next tick starts from the cursor the run *reported*. Three files carried a long comment fixing this class on the READ ("returning [] is indistinguishable from an empty window… that is how the AllDay backfill lost 31.4M blocks") **six lines above a `setCursor` that dropped its own error** — the lesson was learned, written down, and applied to one half of the function. The fix: **a failed cursor write fails the RUN and reports the cursor where it IS** (never the value that did not land); a failed cursor READ throws (a re-seed from 0 / 1 is the 31.4M-block shape). Where the writes are keyed, the re-scan is a cost, not a corruption — say so in the code.
3. **The DELETE-THEN-INSERT window** (`edition-floor`, `allday-pack-listings`, both pack-EV computes): the delete commits, the insert is rejected, the lane is now EMPTY and the run row says ok. supabase-js has no transaction, so the fix is **ORDER**: write first (INSERT beside the old rows when the PK is a fresh id — `fmv_snapshots (id, computed_at)`; UPSERT on the PK when it is natural — `pack_drop_pool (collection_id, dist_id, edition_id, slot_name)`, `pack_listings_cache (id)`), then **delete only the rows this run did not write** (`.not("id","in","(…)")`, ids double-quoted for PostgREST). A rejected write **skips the delete** — a stale board beats an empty one — and fails the run. Readers keyed on newest `computed_at` never see the overlap.

**And the run-row form** (candy ingest, candy wallet backfill, candy listings): collect `writeErrors[]` per walk, `ok = writeErrors.length === 0`, `p_error` names the table and message, `extra.write_errors` counts; the walk still continues past a rejected chunk — that half of "non-fatal" was right, the literal `true, null` was not. ⚠ **An `UPDATE … .select()` destructured as `{ data }` only reads a FAILED update as `[]` = "nothing to update"** — candy-listings published `deactivated: 0` for a rejected deactivation. Bind the error there too.

**Harness facts that made the fixes testable:** supabase-js RETURNS errors — a `try/catch` around the call cannot see them, so a test that injects `{ error }` on the write is the only arm that proves the guard; the shared fixture (`makeInstrumentedSupabaseFixture`) takes an ARRAY of payloads per table key to drive read-then-write sequences (`[{read}, {write}]`), and `failWrites` throws rather than returns, which is a different shape. **Every guard was mutation-checked by deleting the guard line** — the arm goes red on `ok` / `status` / the cursor, never on a message string. ⚠ **Five tests pinned the swallow as the contract** ("tolerates upsert errors (run still ok)", "all four write errors are swallowed: ok=true"); per the standing rule they were INVERTED in place, never deleted, with the old assertion quoted in the new test's comment.
...
- **A number the data cannot support must not be manufactured — three shapes, all found on live public surfaces (2026-08-12/13).** Distinct from the "failed read renders as an answer" class above: here the READ SUCCEEDED and the arithmetic invented the answer.
  - ⚠ **`|| 1` as a divide-by-zero guard.** `app/profile/[username]/ProfileClient.tsx` computed its 30-day change as `((last - first) / (first || 1)) * 100`, so a **$0 baseline silently became $1** and a rise to $500 rendered as **"↑ 50000.0% / 30D"** on a page collectors SHARE. A ratio against zero is UNDEFINED, not enormous. Omit it and keep the chart — the *shape* is real even when the ratio is not. ⚠ That forces a second half people miss: anything deriving DIRECTION from the now-null ratio (`change >= 0 ? green : red`) must derive it from the series instead, or a genuine gain paints as a loss.
    ...
    🚨 **AND THE BAN WAS BLIND TO A FOURTH SPELLING FOR AS LONG AS IT EXISTED — 2026-09-07.** Anchoring on the `/` is what makes the parse-fallback carve-out possible, and it is also what made the guard unable to see the substitution when the denominator is **NAMED FIRST**: `const total = stats?.total_principal_usd || 1` on one line, `part / total` on the next, and the pattern matches neither. That is not hypothetical — **`components/analytics/WalletProfile.tsx` carried exactly it, inside the guard's own roots, under a green CI**, publishing a measured **"0%"** collection-mix share for every wallet whose funded loans all carry a NULL `principal_usd` (the RPC `COALESCE`s that `SUM` to 0, so the total and every part go to 0 together — **2 of 16 borrower wallets**, 2026-09-07). ⭐ **And the same sweep found the copy-paste pair AGAIN, one expression down from the ratio that started this entry:** both `Sparkline`s used `const range = max - min || 1`, sending every point of a FLAT series to `(v - min) / 1 === 0` — the line drawn hard along the **bottom** of the box, visually identical to a portfolio that fell to the low of its window and stayed there. A dormant wallet is *unchanged*, not *at its low*. Both now centre a flat series, and **both are pinned**, each with a no-change control on a varying series. ⚠ **The transferable rule is not about division:** *a guard anchored on an OPERATOR is blind to the hoisted spelling of its own class* — assign the fabricated value to a name and every operator-anchored ban stops seeing it. Ask it of every such guard in this repo, not just this one.
    ✅ **Widened the same day, and the widening is itself instructive.** Three additions: the **clamp** spelling (`x / Math.max(y, 1)` — a value substitution wearing a divide-by-zero guard's clothes, added at population zero); the **hoisted** spelling above; and the roots, from a curated four to a walk that also covers **`scripts`** and **`supabase/functions`** — that exclusion rested on nothing, since edge functions sit outside the coverage gates entirely, so no other instrument was looking. ⚠ **The hoisted detector only enrolls a fallback to a NON-ZERO literal**: `?? 0` is a different class, because dividing by an explicit zero yields `Infinity`/`NaN`, which is loud, not a plausible-looking measurement. ⚠ **And its first cut had two FALSE POSITIVES that are worth remembering — both were the identifier appearing inside a URL PATH** (`"/api/fmv/demo"` matching a `fmv` constant, `"/serial-premiums"` matching a `serial` one). A `/` is not an operator just because it precedes the name. ⚠ **The 3-line opt-out window then bit a second time, in the opposite direction from the first:** *both* suppressions written that day were silently ignored, because a justification that states the measured population, why the fix is elsewhere, and the exit condition **does not fit in three lines**. The window is now the fixed 3 lines **UNIONed with the contiguous comment block immediately above** — which stops at the first non-comment line, so unlike a larger fixed number it can never reach across code. **An escape hatch that quietly does nothing is worse than none, because the author believes it took.** All three new spellings were proved by injection into real files in all three newly-relevant places, then reverted.
...
- `app/api/support-chat/route.ts` — AI concierge (**38 tools**, counted from the tool array 2026-09-13 — it was 33 on 08-16 and 35 on 09-02, so re-count with `[...src.matchAll(/^\s+name: "([a-z_]+)",/gm)]`; Claude Sonnet — model `claude-sonnet-4-6`, verified 2026-07-16). ⚠ **The prompt's FMV methodology is `FMV_METHODOLOGY_BLOCK`, a module constant interpolating `lib/fmv-confidence.ts` — never hand-type a threshold there** (it had drifted to "HIGH = 5+ sales" against a ≥7 gate; concierge.md "2026-09-13 pass").
- `lib/concierge/edition-metadata.ts` — `fetchEditionMetadata()` / `metadataFieldsFor()`: the badges (Top Shot moment tags), team, series, parallel and supply block (circulation / burned / locked / `squeeze_pct`) every priced edition the concierge hands back now carries. ⚠ **`badges_status` is THREE-state** — `ok` (an empty list is a real "none"), `unavailable` (the read FAILED), `not_tracked` (no moment tags for that collection / no row) — and the prompt forbids "this has no badges" on anything but `ok`. Fail-soft, never throws; the honesty shape is that a timeout must not render as an unbadged moment.
- `lib/concierge/fmv-coverage.ts` — live per-collection FMV coverage for the prompt (head counts on `edition_fmv_current` + `pinnacle_catalog`, 30-min module cache, 2.5 s budget). A failed count is `null`, rendered as "coverage read failed — do not quote", never 0. Replaced hardcoded percentages that had sat months stale. See the AI Concierge section for the 2026-08-13/14 additions (`search_catalog`, `get_price_history`, `find_quirky_serials`), the newest one (`get_edition_listings`), and the honesty constraints that travel with them.
...
The route paged `wallet_moments_cache` by OFFSET, 1000 at a time, with `if (offset > 50_000) break`, then answered 200 as if complete. **3 live wallets exceed 50k moments in one collection** (largest 153,544), so every edition past the cap read "Owned: 0" on Market, Sniper and the player-page tiles. The tell is the same control-flow keyword as the `/sitemap/3.xml` case above: a `break` inside a paging loop with no `complete:false`. **Fixed by moving the aggregate into SQL** (`get_wallet_edition_counts`, migration `20260926050443`): one jsonb value, so no PostgREST row cap applies. On the largest wallet it takes 2.0 s and ~103k buffers, counting owned from the covering `(wallet, collection, edition_key)` index and locked from the `is_locked` partial index. The obvious single `GROUP BY` heap-fetched `is_locked` for all 153k rows: 23.6 s, ~581k buffers. Equivalence: a 0-row set diff against a direct count on a 678-edition wallet. A malformed result is now a 502, never `{}`. ⚠ **2.0 s is a WARM number:** verified live ~10:29 PM PT, the first production call on that wallet was a COLD read and hit the route's 8 s `boundedRead` budget, returning an honest retryable 503 (the grid then shows "Couldn't load your owned & locked counts", no numbers). The retry a minute later returned 200 with all 8,168 editions / 153,544 owned / 93,244 locked. So the largest wallets may need a second load on a cold cache. That is a latency cost, not a wrong number.

## A LIVE FIGURE HARDCODED INTO STATIC COPY is a claim with no read behind it (2026-09-26, PT)

Every other face of the honesty canon has a READ that can fail. This one has none: a string written on the day it was true, then left to drift. Nothing fails, no instrument sees it, and it is usually the most-published text on the site (SEO descriptions, OG cards, share tweets, the Concierge prompt). Found in one sweep, each checked against live data:

- Pack Reality OG subtitle "Median pull value $0." — live **$0.91**; metadata "Over 145,000 rips / ~41% delivered nothing / Under 1% over $100" — live **69,956 · 31.6% · 1.47%**; the share-tweet repeated the stale three under the collector's own name.
- The Concierge prompt: UFC "only ~20% of editions have FMV" — live **518 of 518**; "beyond ~4.5 months" of snapshots — a span that grows daily. The prompt's own rule said coverage comes from the live section, never from memory; the rule was obeyed everywhere but in the prompt itself.
- `/ufc` SEO "247 NFTs indexed … sniper deals" (UFC has no Sniper tab), "sub-$1" floor, "days-old market" two months on, "~58x", "575 editions and 23 sets", "~94% of volume", "~92% of active buyers".

**Rules.** (1) Static copy (metadata, layout descriptions, share text, prompts) carries NO figure describing current data — describe WHAT the page measures, and let the page, OG card or share text print the live value from its read. (2) Where a figure must appear, build it from the read at render time and OMIT it when the read is unavailable — never a default. (3) A cumulative floor that can only grow ("4M+ sales") stays true and may stay; a dated statement framed as dated ("as of Sept 2026") may stay. (4) A test that pins the literal ("expect(note).toMatch(/92%/)") pins the decaying snapshot — re-pin to the property ("discloses partial coverage, states no percentage"). Guards: `__tests__/api-og-insights-pack-reality-honesty.test.ts` (static metadata carries no figure), `__tests__/support-chat-prompt-carries-no-remembered-live-figure.test.ts`.

## Two cases from 2026-09-26 (Claude Code, web)

### A guard on ONE writer of a column is not a guard on the column — the Candy ask-ceiling

The Candy FMV ask-ceiling (09-25) capped an FMV only when fmv-recalc PRICED the edition in that run. Two other paths wrote the same `fmv_snapshots.fmv_usd` and never passed the cap: the Step 6 re-stamp (HIGH/MEDIUM editions with no 30-day sale, once per 24 h) carried the pre-ceiling value forward dated NOW, and LOW rows are never re-stamped at all. The morning after, 10 of 123 Candy editions still sat above their live ask; one LOW edition ($175.45 off sales all older than 30 days, against a $66.83 ask) was on the Deals board as "62% off". Fixed at both writers (`56702caa5`, then a per-tick Candy sweep `a3091fa79` that writes only where the cap binds) → 0 over ask. **This is CLAUDE.md's "grep the column's WRITERS" rule, missed by the author of the guard within a day.** Before calling a value-rule shipped, list every path that INSERTS the column (here: main loop, historical fallback, Step 6 re-stamp) and prove each one applies it, or name the one that cannot and cover it separately.

### A headless crawler's beacon report can name a site that is already handled — read the FRAME before attributing

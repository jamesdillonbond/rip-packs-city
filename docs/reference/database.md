...
Accent and case variants fold (`unaccent` + `lower`). Two different names for one person ("Steph" / "Stephen Curry") cannot fold, and merging the rows alone does not hold: every writer resolves on the name slug, so the next "Stephen Curry" from wallet-search or the daily `ensure_players_from_edition_names` mints the row again. **Register the second spelling in `public.player_name_aliases` (collection_id, alias_slug → player_id).** `resolve_canonical_player`, `ensure_players_from_edition_names`, `link_editions_to_players_by_name` and `get_player_alias_target` (the player layout's 308) read it. The FK to players has no cascade, so a later merge that deletes an alias target fails loudly. Leave the edition labels alone: search matches tokens as substrings, so rewriting "Stephen" to "Steph" would make "stephen curry" find nothing.

...
⭐⭐ **ROOT CAUSE, SHARPER THAN "BEWARE STALE ANCHORS" — THE FAILING SCRIPT GUARDED ONE ANCHOR AND NOT THE OTHER.** It held two: one wrapped in an occurrence assertion, and a second used through a bare `.replace()` with **no assertion at all**. ⛔ **The correct pattern was in the same file, twenty lines above, applied inconsistently.** ⇒ **THE RULE IS: IN A MULTI-ANCHOR EDIT, ASSERT *EVERY* ANCHOR — the unguarded one is the one that fails**, and "I used the safe form" is a claim about a script, not about each anchor in it. ⭐ Reading anchors only AFTER all prior mutations is the other half: together they explain how the anchor went stale *and* why nothing caught it.

⚠ **AND THE OBVIOUS FIX FOR THE CASE TRAP IS WRONG: DO NOT REACH FOR `grep -i`.** A case-insensitive survival check would also match a section that had been genuinely corrupted or half-deleted, **so it trades a false negative for a blind spot** — it stops the check crying wolf by stopping it from working. ⭐ **The real fix is the same one this whole family keeps arriving at: lift the fragment OUT OF THE FILE so case cannot drift in the first place.** Remove the failure mode; do not soften the detector.
...
- `curl` fails silently in Git Bash for Vercel REST calls — always use PowerShell `Invoke-WebRequest`.
  - ⚠ **AND THE INVERSE, measured 2026-09-18 during the #122 outage — do NOT generalise the rule above into "PowerShell is the trustworthy one".** Reading a **non-2xx response BODY** is the case where `Invoke-WebRequest` is the worse tool: it throws, and the body has to be dug out of `$_.Exception.Response.GetResponseStream()`, which **returned `BODY_LEN=0` on a real Cloudflare 522 that `curl` printed in full**. Had I stopped there, the open question three ledger entries called *"the discriminator to chase first"* — what the `<!DOCTYPE html>` error bodies actually were — would have stayed open; `curl -s` on the same URL returned `<title>supabase.co | 522: Connection timed out</title>` and closed it. ⭐ **Split by what you are reading, not by tool loyalty: Vercel REST → PowerShell; an ERROR BODY → `curl`.** ⛔ **And never conclude "the body was empty" from one client** — an empty body and a body your HTTP client discarded are the same reading, which is the ordinary failed-read-rendered-as-an-answer shape aimed at your own instruments.
- 🚨 **`$` AND BACKTICK ARE INTERPRETED BY MORE LAYERS THAN YOU ARE THINKING ABOUT — three DIFFERENT ones bit in a single session (2026-08-26), each in a different tool, each silent.**
  ...
  **The rules that actually hold:**
  ...
  - 🔁 **A FOURTH instance, 2026-09-07, and it is the one that shows a count assertion is not enough either.** Adding a cross-reference to known-issues #50 via `node -e "…"`, six backticked terms — `` `refresh_atlas_pack_ev()` ``, `` `total_unopened = 0` ``, `` `pack_ev_latest` ``, a migration id and two more — were **command-substituted away by bash before node ever saw them**, leaving *"the +EV board:  hardcoded  and  reads that as SOLD OUT"*. ⚠ **The script printed `cross-reference added` and exited 0**, and a line-count assertion would have passed: **exactly one line was inserted, as intended — it was its CONTENTS that were gutted.** The `command not found` lines went to stderr among successful output. ⭐ **The assertion that works for this class is a CONTENT one: after writing, re-read the line and assert each expected substring is present.** The repair script did exactly that (`all 6 backticked terms present`) and is the pattern to copy — a count proves the shape of an edit, only a read-back proves its text.
  - 🔁 **A FIFTH instance, 2026-09-13 — and the point of recording it is WHERE THE SYMPTOM APPEARED, not that the rule was broken again.** A three-file DB re-pin swapped a pinned function body with `s.replace('$function$' + oldBody + '$function$', '$function$' + newBody + '$function$')`. ⚠ **Every function body in this repo's `sales`-side pins ends its tx-hash guard with `~ '^[0-9a-f]{64}$'` — that trailing `$'` is the `$'` replacement token, "everything after the match"** — so the new body spliced the file's entire tail back into itself, twice. **The corrupting call reported success.** What failed was a LATER, unrelated `rep()` in the same script, which printed `OCC=3` for an anchor that occurs exactly once on disk — so the diagnosis began three edits downstream of the damage, against a file that had never been written. (Nothing was written: the script exits non-zero before `writeFileSync`, which is the only reason this cost minutes instead of a red `main`.) ⭐ **The generalisable bit: a `$`-token corruption does not fail where it happens, it fails wherever the duplicated text next collides with a uniqueness assumption.** If an occurrence count comes back higher than the file can justify, suspect the PREVIOUS replacement before you re-read the anchor. **And the standing rule already covers it** — replacement FUNCTION (`() => neu`) plus a script FILE, never `node -e` — so this is a fifth demonstration that the rule needs applying, not amending. **SQL pins are the acute case: dollar-quoting (`$function$`) and regex anchors (`{64}$'`) mean a pin body is nearly guaranteed to contain a live replacement token.**

  - 🚨 **A SIXTH instance, 2026-09-13 (PT) — and it is the first that REACHED `main` and turned CI red, which retires the fifth entry's consolation that "nothing was written".** Extending register #113 in `docs/reference/known-issues.md` with a two-`rep()` script, replacement passed as a STRING. The replacement prose contained a markdown code span quoting a regex: \`^[0-9]+:[0-9]+$\` — **the trailing `$` immediately followed by the closing BACKTICK is \`$\` + backtick, the "everything BEFORE the match" token**, so ~950 KB of preceding file was injected mid-document. `known-issues.md` went **1,782 → 3,526 lines**: every item, both section headings and the status index duplicated. It was committed and pushed (`b5af77544`), CI run 5460 went red, and the repair was `a3837ec19`.

    ⭐ **THREE NEW THINGS, none of which the first five instances show.**
    1. **A DIFFERENT TOKEN with a different blast direction.** The fifth instance was `$` + single-quote ("everything AFTER"); this is `$` + backtick ("everything BEFORE"). Both are produced by ordinary content, and they corrupt in opposite directions, so "check the tail" is not a sufficient inspection.
    2. ⛔ **PROSE IS AS ACUTE AS SQL PINS, and that widens the rule.** The fifth entry concluded "SQL pins are the acute case". This edit touched no SQL at all — a **register entry quoting a regex in backticks** is enough, and this repo's ledger, register and session logs quote regexes in backticks constantly. **Any markdown replacement text ending a code span with `$` carries the token.**
    3. 🚨 **EVERY CHEAP CHECK PASSED AND THE EXPENSIVE ONE CAUGHT IT.** The corrupting `.replace()` returned normally and the file WAS written. The pre-replace occurrence assertions (`count === 1`) passed — they run BEFORE the replace and cannot see it. `npm run docs:issues-index` regenerated the index happily. The guard's **"byte-identical to a fresh regeneration" test PASSED**, because the generator rewrote the FIRST index block and the injected copy was identical to it. **The only thing that failed was the guard's POSITIVE CONTROL** — "tamper the index, the check must catch it" — which failed because after removing one row the regenerated index still matched the *second* copy. ⭐ **This is the "a NULL result needs a positive control" rule paying for itself: the tamper test is the sole reason a doubled register was found in minutes rather than living in the tree.**

    **The mechanical fix is unchanged and now has six demonstrations:** pass a replacement **FUNCTION** — `s.replace(old, () => neu)` — from a script **FILE**. A function replacer disables `$`-interpretation entirely. **And add a post-write SIZE assertion** to any script that rewrites a whole document: this one would have been caught before `git add` by a single `lines after ≈ lines before` check, which is cheaper than every guard that missed it.

  - ⛔ **THE SAME PUSH ALSO BROKE THE 2026-09-04 GATE RULE AT THE TOP OF THIS FILE — second instance, and the failure mode is identical in shape.** The command was `npx vitest … | grep -E 'Tests |FAIL'; echo "EXIT=${PIPESTATUS[0]}"; git add … && git commit … && git push`. It **printed** `EXIT=1` and pushed anyway: the `&&` chain hangs off `git add`, not off the test. **Printing an exit code is not branching on it.** The correct shape, used for the repair: run the suite redirected to a log, capture `$?` into a variable, `if [ "$EXIT" -ne 0 ]; then echo REFUSING; exit 1; fi`, and only then commit. ⚠ **Both instances of this rule were broken by a session that had just READ the rule** — so treat "I know about the pipe trap" as no protection; the protection is the `if`.
...
**The path that works today** (proven 2026-09-12 12:45–12:50 PT):

- `push-dryrun.cmd` — fresh clone at `%USERPROFILE%\rpcwork-cowork`, then `git push --dry-run`: **exit 0, credential helper `manager`**. A dry-run push still authenticates GitHub's `git-receive-pack` advertisement (exactly the step the cloud proxy 403s on), so exit 0 IS proof of push auth — not a read.
- `apply-and-push.cmd` — applies every `queue\*.patch` in name order with `git am -3` onto a FRESH clone of `origin/main`, pushes `HEAD:refs/heads/main`, verifies `ls-remote == HEAD`, moves the patches to `applied\`, logs to `apply-and-push.log`. Aborts BEFORE the push on a dirty clone or an `am` conflict and leaves the queue untouched.
- Recipe from a cloud session: commit in the cloud clone (ledger BEFORE code, run the three ledger guards) → `git format-patch origin/main -o out/` → `device_commit_files` the patches into `…\Rip Packs City\cowork-push\queue\` → double-click `apply-and-push.cmd` — Trevor, or computer use (File Explorer is granted at **click-only** tier, which is enough to navigate `rip-packs-city → Rip Packs City → cowork-push` and double-click; terminals cannot be typed into) → `device_stage_files` the log and **READ it: success is the literal line `[PUSH CONFIRMED]` with matching shas.** The cmd window closing proves nothing.
- The clone lives OUTSIDE the connected folders on purpose and `Rip Packs City/` is gitignored at the repo root, so the live working tree (Claude Code's) never sees any of it. Never point this tool at `C:\Users\TDill\rip-packs-city` itself.
- ⚠ `computer_open_application("File Explorer")` is needed before each click batch when VS Code or Chrome is frontmost — a non-allowlisted app in front aborts the whole batch.
...
- 🚨 **CORRECTED 2026-09-19 — THE DISCRIMINATOR IS THE *SHELL*, NOT THE BOX, AND THE OLD WORDING SENDS AN AGENT ON TREVOR'S BOX STRAIGHT INTO THE TRAP.** This bullet used to read *"on Trevor's Windows box `LANG=en_US.UTF-8` is already set, so bare `wc -m` is correct; in the Claude Code sandbox `LANG` is empty"*. **The first half is false for every agent session.** The locale is exported from `~/.bashrc`, which an interactive login shell loads and **the Claude Code Bash tool's non-login shell does not** — so on the SAME machine, in the same minute, measured 2026-09-19 against CLAUDE.md: `bash -lc` reports `LANG=en_US.UTF-8` and `wc -m` = **39,992** ✅, while the agent's own shell reports `LANG` / `LC_ALL` / `LC_CTYPE` all EMPTY and bare `wc -m` = **40,641** ❌ — byte-identical to `wc -c`. ⛔ **So bare `wc -m` is correct when TREVOR types it and wrong when an agent runs it on his behalf, which is the worst possible split: the doc says "safe here" and it is not safe for the reader of the doc.** ✅ **`LC_ALL=C.UTF-8 wc -m` = 39,992, re-confirmed in the agent shell tonight** — it remains correct in every environment, and it is the only `wc` form worth typing. ⭐ **Prefer Node `.length` regardless: the limit is enforced by a JS harness, which counts UTF-16 units.**
- 🚨 **AND THE STAKES ARE NOT THEORETICAL RIGHT NOW: CLAUDE.md measured 39,992 on 2026-09-19 — EIGHT characters of headroom.** A session reaching for bare `wc -m` in an agent shell reads **40,641**, concludes the file is **641 OVER** a limit it is 8 UNDER, and starts deleting rules to fix an emergency that does not exist. ⚠ **Eight characters is a tripwire, not headroom** — the next edit to CLAUDE.md by anyone trips a blocking CI gate, so a rule added there must DISPLACE one, and choosing what leaves the user's core context file is **Trevor's call, not a session's**. (A dated sample: re-measure with Node, never quote this number.)
...
> 🚨 **THE `python3 splice.py` BELOW CANNOT RUN ON TREVOR'S WINDOWS BOX — USE THE NODE EXECUTABLE THAT SUPERSEDES IT (measured 2026-09-19).** `python` and `python3` both RESOLVE there, to the Windows Store **App Execution Alias stub** at `/c/Users/TDill/AppData/Local/Microsoft/WindowsApps/python`, and **neither EXECUTES ANYTHING** — every form (`-c`, `--version`, a stdin heredoc, a script file) returns **rc=49** with an empty stdout and a 177-byte *"Python was not found; run without arguments to install from the Microsoft Store"* on **stderr**. ✅ **It fails LOUDLY, so this snippet fails CLOSED** (the `if` is false, `git add` never runs) — the cost is a wasted turn mid-rebase, not a bad resolution. ⛔ **But `command -v python3` SUCCEEDS, so a presence check protects you from nothing; the only real test is running it.** 👉 **The successor already exists and enforces the same three traps: `node scripts/resolve-ledger-rebase-conflict.mjs && git add docs/overnight/ledger.md`** — documented at [ledger-discipline.md](ledger-discipline.md) (2026-09-01), used successfully against a live three-way ledger conflict on 2026-09-19. **Reach for `node` for any scripted edit on this box; the snippet below is kept for its reasoning, not as a runnable recipe.**
>
> ⚠ **A CORRECTION RECORDED WITH IT, because it nearly became a gotcha of its own:** a session first measured this stub as *"prints its failure to STDOUT and exits 0"* — which would defeat a presence check AND an `&&` guard simultaneously. **All three claims were its own instrumentation:** `2>&1` merged stderr into stdout by hand, and `| head -3` made `$?` report **head's** status (`${PIPESTATUS[0]}` = 49, `$?` = 0). Isolated re-run with no pipe reproduced rc=49 / empty stdout / stderr exactly. ⭐ **Two rules, both already in this file and both broken by someone documenting them: never read `$?` through a PIPE, and when a finding says a tool behaves IMPOSSIBLY, suspect the HARNESS before the tool.**
...
## 🚨 The full `/sessions` disk can be cleared FROM A SESSION — and the two traps on the way (2026-09-07)

`/sessions` is `sessiondata.vhdx` (~9.8 GB) under `%LOCALAPPDATA%\Packages\Claude_*\LocalCache\Roaming\Claude\vm_bundles\claudevm.bundle\`; the leak is un-GC'd session dirs owned by uid 65534 that no session can delete (`CapBnd 0`, `sudo` deliberately broken). Until 2026-09-07 the fix was Trevor's keyboard. Now:

1. **Relief without a reset:** the rootfs (`/`, ~4 GB free) is a different disk. The thing that actually blocks `playwright install` / `npm i` on a full `/sessions` is **`TMPDIR=$HOME/tmp`**, not the target path — `export TMPDIR=/tmp/rpc-$(id -u)/tmp PLAYWRIGHT_BROWSERS_PATH=/tmp/rpc-$(id -u)/pw` and the download lands. `libxdamage1` still comes from `apt-get download` + `dpkg -x` + `LD_LIBRARY_PATH`.
...
🚨 **A SECOND, DIFFERENT FAILURE ON THE SAME SUBJECT — 2026-09-11, TWICE INSIDE ONE HOUR: a time that was never READ.** The rule above is about the ZONE. This one is not a conversion error at all — it is **fabrication**. Filing an inbox document at 06:34 PT, I headed an addendum *"07:20 PT"* and, twenty minutes after committing the correction for that, headed the next one *"06:55 PT"*. The clock read **06:42** and **06:46**. Both numbers were plausible, neither was read; I estimated elapsed time from how much work had happened and typed the result.

⛔ **THE RULE: never write a clock time you did not read in the same action that writes it.** `TZ=America/Los_Angeles date` costs nothing. ⭐ **The reason this is worse than a zone slip:** a wrong zone is a systematic error a reader can decode once and correct everywhere, while an invented time is **indistinguishable from a measured one** and silently corrupts the ordering of a document whose entire value is that its numbers were measured. Both of mine landed in filings that argue, at length, against exactly that. ⚠ **And note where it happened — not in chat, but in the ARTEFACT**, which is the half the 09-10 rewrite was expanding *from*. The two failures are complements: one quoted the right number in the wrong zone to a person, the other wrote an unread number into a document. **Cheapest defence: paste the command's output, never a recollection of it, and if a precise time is not worth one command, drop the precision instead of inventing it.**

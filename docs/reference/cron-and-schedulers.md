...
| `vercel.json` cron entries | **36** | 35 (before 09-02), 36 (08-22), 37 before that |
...
✅ **2026-09-03 — jobid 219 moved AGAIN, to `43 0,6,12,18 * * *`, because the 08-17 ground "its only consumer is already 6-hourly" missed that the consumer READ FIRST.** Leg 324 reads `topshot_impossible_parallel_serials` at `:48`; the healer ran at `:52`, so every inflow was published as a six-hour BREACH after it had been raised away. Now heal-then-read. Same hours, same function, jobid + `cron_heavy` owner preserved via `SET LOCAL ROLE cron_heavy; cron.schedule(...)`. Migration `20260903221540`; the arm's new meaning is in [trust-board-and-safety.md](trust-board-and-safety.md). Also new the same day: **jobid 436 `rpc-refresh-candy-scarcity-board` `26 * * * *`** (300 s), the hourly refresh behind the now-materialised `candy_scarcity_board` (migration `20260903221326`).

✅ **2026-09-03 — the wmc REINDEX is WEEKLY MAINTENANCE now, not a campaign: jobids 438–441 `rpc-weekly-wmc-reindex-1..4` (`3 2` / `23 2` / `43 2` / `3 3 * * 0`, `cron_heavy`, one bare `REINDEX INDEX CONCURRENTLY` each — a `SET` in front would make an implicit transaction block) + 442 `rpc-weekly-wmc-reindex-verify` (`23 3 * * 0`).** Sized off the 08-31 second wave (33–50 s per index in the 02Z–03Z band), so the standing 600 s budget holds with ~12× headroom and no ALTER ROLE window; regrowth measured +64 → +38 → +24 MB/day on `idx_wmc_cohort_cover` (decelerating). ⚠ A `failed` on 438–441 is not "work not done" (the reindex commits its phases outside the block) — read the Sunday `wmc-reindex-verify` row; `invalid_left` there means a `<index>_ccnew` to drop by hand. Migration `20260903223235`; register #56.

📏 **2026-09-03 (late PT) — jobid 443 `rpc-pipeline-gap-hourly-rollup` (`7 * * * *`, postgres) → `rollup_pipeline_gaps(3)` → `pipeline_gap_hourly`.** A CALIBRATION SERIES for the correlated-skip arm sketched in inbox `2026-09-03T0300Z`, not an alarm: per hour, the derived scheduled population (≥ 20 gaps in the trailing 73 h, p90/p10 ≤ 1.25), ticks vs expected, and skipped ticks (round(gap/median) − 1 above 1.5× median), with a `_all_` row every hour and per-pipeline rows only where something happened. Idempotent over the last 3 complete hours; its own terminal row is `pipeline-gap-hourly-rollup` (watchlist: silent 180 / no-success 360, info). Migration `20260904024810` + `20260904024921`. ⚠ Nothing reads the table yet — the arm waits for weeks of data and at least one real correlated dip.

⏳ **2026-09-08 (PT) — jobid 475 `rpc-dune-free-tier-sunset` (`0 12 23 9 *`, postgres) — a CALENDAR-DATED kill switch for the whole Dune integration.** Dune converts pre-2026-07-21 free accounts to **view-only on 2026-09-10**; after that every Dune lane's `execute` is refused. The job runs `update public.dune_budget_state set paused = true … where id = 1 and paused = false` (idempotent, and it appends its own reason to `note`). ⭐ **Why `paused` and not a schedule removal:** `readDuneBudget` reports a paused budget as a **CONFIGURED** stop, so `logDuneBudgetStop` writes `ok: true` with `budget_stopped` — the hourly `sync-sales-seller-recovery-dune` Vercel cron (`47 * * * *`) then costs ~400 ms and pages nobody, instead of throwing `ok: false` forever. ⚠ **THE DATE IS 09-23, NOT 09-10, AND THE FIRST VERSION OF THIS JOB HAD IT WRONG IN BOTH DIRECTIONS.** (1) A 09-11 pause was **redundant**: the cycle's 1,000,000 datapoints were already spent, so the lane's own gate refuses in ~400 ms **without ever reaching Dune** — measured, five consecutive ticks. (2) It was **harmful**: Dune hands affected accounts a **14-day Plus trial** from 09-10, and a 09-11 pause would have silently killed that window. **The real risk opens 2026-09-24**, when `dune_budget_state`'s cycle rolls (`cycle_anchor_day = 24`) and the cap REFILLS — that is the first moment the lane would call a dead API hourly. ⚠ **Transferable: before adding a guard, check whether an EXISTING gate already covers the window you are guarding** — the August budget ledger was already doing this job, and the new one only mattered for a date three weeks later than the one it was set to. ⚠ It is an **annual** cron (fires every 23 Sep); re-firing is a no-op by the `paused = false` predicate. **Revert:** `select cron.unschedule('rpc-dune-free-tier-sunset'); update public.dune_budget_state set paused = false where id = 1;`. Context + the full spend record: [apis-and-cadence.md](apis-and-cadence.md) counterparty source floors, and the 2026-09-07/08 ledger entries.

✅ **2026-09-04 (PT) — `topshot-catalog-backfill` is UNSCHEDULED (36 → 35 crons), and the "Atlas decision" it was waiting on is DECIDED, not deferred.** Its upstream `public-api.nbatopshot.com` has answered 530/1033 since 2026-08-28: **last success 08-28, then 7 of 7 daily ticks failed.** ⚠ **It was retired for REDUNDANCY, not for being broken** — the difference matters, because "it is failing" is not on its own a reason to remove an arm. All three jobs it owned were checked against live data first: **circulation** → `topshot-circulation-onchain` (on-chain, 09-03); **tier + badges** → the Atlas edition walk already running on pg_cron (0 of 13,436 canonical Top Shot editions have a NULL tier; 13,312 carry an Atlas map row); **prose + media** → the same Atlas walk as of migration `20260905024630`, which splices an `editions` enrichment into `atlas_editions_drain` — Atlas serves the identical text at `editionTemplate.metadata.Description` (measured on a live 100-row page of set 90: **64 rows would be FILLED, 0 CHANGED**, so the field is byte-identical where we already hold it and the write is near-zero in steady state) and the CDN media at `assets[]`, filled **only where NULL** so an on-chain-resolved IPFS CID is never overwritten. It rides inside the page loop the walk already pays for — **no new HTTP call, no new cron, no new lambda** — and its own `BEGIN/EXCEPTION` keeps a bug in it from ever taking down the badge lane it shares. First live tick: `editions_enriched: 141`, 0 errors; `editions.description` NULLs 4,237 → 4,189 within a minute. ⚠ **One job nothing inherited, stated rather than buried:** creating editions rows Atlas knows and we do not (16 of 100 on that same page, all parallels such as `90:4046::1 "Explosion"`). That is not a regression — the walker has created nothing since 08-28 — and it is left open on purpose, because new-edition creation ripples into circulation, the sitemap and every entity surface. Guard: `__tests__/topshot-catalog-backfill-schedule-is-retired.test.ts` (pins the absence, the surviving route, the one-line restore, the named inheritors, and that the on-chain lane is still scheduled). **Restore:** re-add `{"path": "/api/cron/topshot-catalog-backfill", "schedule": "12 2 * * *"}` to `vercel.json`.

✅ **2026-09-03 (late PT) — Vercel cron `/api/cron/topshot-circulation-onchain` (`5 4 * * *`, 04:05Z) replaces the CIRCULATION half of the dead `topshot-catalog-backfill`.** Reads `TopShot.getNumMomentsInEdition` for every Top Shot BASE edition (250 pairs per Flow REST script, ~40 calls) and applies changes through `apply_topshot_onchain_circulation()`, which compares against the series-8 normaliser (`topshot_normalize_circulation()`, now shared with the write trigger) so only rows the chain moved are written. Base rows ONLY — parallels (`::`) stay with `backfill-topshot-subedition-circulation`. Any Flow REST / RPC failure is `ok=false`; watchlist silent 4320 / no-success 8640. ⚠ ~~`topshot-catalog-backfill` itself still runs daily and still 530s~~ — **SUPERSEDED 2026-09-04 (see the entry below): it is unscheduled, and its remaining job is answered.** Migrations `20260904031944` + `20260904032601`.

✅ **2026-09-04 — `pipeline_cadence_watchlist` has a SECOND, opt-in arm: `max_minutes_without_success` (NULL = not armed), read by `detect_pipelines_without_success()` and the sentinel check "Pipeline Success".** The incumbent `detect_stalled_pipelines()` still reads `max(started_at)` with no `ok` filter (a pipeline that runs and fails stays green there — by design, do NOT add `AND ok`: 21 of 83 arms would flap). Seeded from each pipeline's own ok-gap over retention (`GREATEST(3× max ok-gap, 2× max_silent)`); 86 of 87 armed, `ingest` deliberately NULL. Raise a threshold WITH A NOTE from `pipeline_runs_daily` when a weekly rhythm the 73 h retention cannot see trips it; never NULL it by taste. Migration `20260904012510`; inbox 2026-08-29T0130Z.
...
## 🚨 A QUEUE WALK THAT STARTS AT THE TOP OF WHAT IT RESOLVES COMPOUNDS — three instances in one day (2026-09-07), each caught by the tick's own `duration_ms`

The shape: a pg_cron lane selects "the next N rows that still need work" from the top of an ordered queue. Every row it resolves stays at the top of that order, so every later tick re-examines everything it has already done to reach the first row that still needs work. The cost grows with the lane's own success and is invisible at ship (the top is all unresolved).

1. **The Atlas edition-verify dispatcher** (`20260907024130` → fix `20260907055104`): a `NOT EXISTS` on a computed key against the never-pruned requests table planned as a per-candidate Seq Scan — 13,884 scans / 2.5 s / 306K buffers per tick after four hours. Fix: read the in-flight set ONCE into a `MATERIALIZED` CTE + a partial index on the in-flight rows.
2. **The chain hydrator's dispatcher** (`20260907153117` → fix `20260907211915`): "newest pack pulls without a `moments` row" re-examined the 11,135 newest queue rows every tick six hours in (8,656 already named, 1,839 already asked) — 69K buffers from 4.6K at ship. Fix: a `<acquired_date>|<nft_id>` cursor in `backfill_state`, a page of 3,000 INDEX rows per tick, the filters applied to the page, wrap at the end.
3. **The wmc hydrator's page** (`20260907051909` → fix `20260907213736`): "the next 5,000 rows WITHOUT a `moments` row" — fine with a backlog, unbounded at convergence (a pass from the top walks the whole 640K-row index probing `moments` to find fewer than 5,000, wraps, repeats next tick). Fix: the page is the next 15,000 index rows named or not; the `moments` check is applied to the page.
...
✅ **VERIFIED ON AN UNFORCED WAVE TOO (2026-09-20 ~6:20–6:40 PM PT).** The 09-20 backlog clear used a `workflow_dispatch` of the GHA backstop (`?force=1`) — the only secret-free lever, since the route needs `INGEST_SECRET_TOKEN` and harvesting it would print a live secret — so "a primary behaves identically" was an INFERENCE. It is now a measurement. All four ordinary cohorts ran (`forced: false`, 00:45 / 00:59 / 01:13 / 01:27 UTC) with `saved_sweep_state=ok`, no error, and candidates **1 / 0 / 3 / 1 = 5** — the exact saved-only population, each wallet in exactly one cohort, and the SAME split the forced run produced hours earlier. ⭐ **The hash partition is therefore stable across invocations hours apart, which is the property that stops a 4× fan-out** — asserted live, not just in the unit test.

⭐ **`picked: 0` on every cohort is the RIGHT answer and the more valuable arm.** Those wallets had been walked ~6.8 h earlier, inside the 24 h threshold, so the sweep declined to re-walk them: `fresh_skipped` **1 / 0 / 3 / 1**. The forced run exercised DISPATCH; this one exercises the **staleness gate, which is what actually bounds the steady-state cost**. Both arms are now observed in production rather than argued.

⚠ **ONE THIN GAP REMAINS, stated rather than dropped: the DISPATCH arm has not yet fired on an unforced wave** (every fire so far came from the forced backstop). `forceWave` gates only the cadence check and the backstop-freshness check, neither of which touches this sweep — so the gap is narrow, and it closes on its own the first time a wallet crosses 24 h with a wave in front of it.

📏 **AND THE COST FIGURE ABOVE IS A CONSERVATIVE UPPER BOUND, now that the cadence is observed.** A 24 h threshold against a 12 h wave cadence refreshes a wallet every **~24–36 h**, not every 24 h, so the real marginal load is **~20 walks/day, not ~25** — still ~+1 % and in the safe direction. ⚠ Re-derive it rather than quoting either number.
...
## A watch cannot watch a lane whose PERIOD is longer than the table it reads (2026-09-24, PT)

`detect_stalled_pipelines()` and `detect_pipelines_without_success()` read `pipeline_runs`, which keeps about 73 h. A weekly lane therefore had `last_run IS NULL` about 4 days in 7, and the stalled check reports NULL as STALLED. A watchlist row for a weekly job was a false alarm by construction. That is why the weekly `wmc-reindex-verify` failed on 09-19 PT with nobody told.

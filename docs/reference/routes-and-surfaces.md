...
- **`market` + `packs`:** all except UFC (Pinnacle gained both in the IA reorg). **Candy MLB `packs` since 2026-09-25** on its NATIVE pack plane (`/api/candy-pack-market` → `candy_pack_market`/`_ev_model`/`_listings`/`_sales`/`candy_packs`, rendered by `components/packs/CandyPackMarket.tsx`), not `pack_distributions` (0 Candy rows). ⚠ Its headline floor is over asks CONFIRMED in the last 12 h — `candy_pack_market.floor_ask_usd` is over every `is_active` row and the indexer never retires on absence.
- **`sets`:** ✅ **ALL SIX published collections since 2026-09-25** (Candy MLB joined with `/api/candy-set-progress`, below). Earlier: ✅ **ALL FIVE since 2026-09-20** — Disney Pinnacle was the last gap and now ships the tab. ⚠ It is NOT served by `/api/sets-db` like Golazos: Pinnacle has **0 rows in `editions` and 0 in `sets`** (it is catalogued render-keyed in `pinnacle_catalog`), so `sets-db` answered every Pinnacle wallet `{ totalSets: 0 }` — a confident "no sets" out of a join that matched nothing. Its `disney-pinnacle` entry was REMOVED and the tab reads **`/api/pinnacle-set-progress`** (169 `set_render_id` checklists × `wallet_moments_cache.render_id`). ⛔ The set NAME is not the key — several carry leading/trailing whitespace and would split a set in two. ⚠ And Pinnacle set rows do **not** link to `/[collection]/set/<slug>`: `get_set_detail` returns NULL for every Pinnacle slug, so `setEntityHref()` (lib/entity-href.ts) returns `null` and the heading renders as text.
- **`pack-sniper`:** Top Shot + AllDay only.
- **`challenges` + `hot-floors` + `play`:** Top Shot only.

**Panini more-boards (2026-09-25, Trevor: "All four"):** `/insights/panini-squeeze` carries four tabs under the squeeze board — Deals (`panini_deal_board`), Pack EV (`panini_pack_ev_board`), Special serials (`panini_special_serials_board`), Players (`panini_player_board`) — from ONE snapshot `panini-boards` (`lib/insights/panini-more-boards.ts`), warmed **hourly** (`warmEveryMs` in `WARM_BOARDS`; the views cost ~270k buffers per warm) with a matching reader window (`freshMsFor`). The listing-gated coverage disclosure sits above every tab; deals lead with sale-backed rows; no Panini username is selected. Held back 2026-07-19 on coverage grounds; the gap is disclosed, not resolved.

**Player page `/[collection]/player/[slug]` (2026-09-25):** section order is header → stats → **Top Sales** (two-line rows, no fixed min-width; phone-safe) → **Editions** → Top Collectors → Season Stats → Sets. The Editions grid (`EditionsGridPaginated` with `showFilters` + `showOwnership`, player page ONLY) has a filter bar (search · Team · Set · Series · Rarity · Parallel · **Badge** · Ownership) over the LOADED rows (it says "N of M loaded · more not loaded yet"), and "Owned: X  Locked: Y" under FMV from `/api/wallet/edition-counts` for the per-chain `rpc_owner_key` wallet. Badges come from `GET /api/entity/edition-badges` → `get_edition_badge_titles` (runs the canonical `get_edition_badges_unified` per edition). Unknown ownership or badges never render as zero or "none"; Pinnacle gets neither.

**Candy MLB entity pages (2026-09-25):** `/candy-mlb/player/*`, `/team/*`, `/set/*`, `/edition/*` and `/moment/<mint>` all render (verified live). The team page's **checklist + "Sets featuring…" panel take a Solana wallet** through `lib/entity/checklist-wallet.ts` (verbatim base58, own localStorage slot `rpc_checklist_wallet_solana`, wrong-chain key refused; `/api/entity/team-checklist`, `-progress`, `team-sets` parse the same way). `/moment/<mint>` resolves via `get_moment_detail` and, since `d592eec61`, carries the Candy collection slug (team + breadcrumb links, "View on Magic Eden" CTA) and reads its asks from Candy's own plane (`fetchCandyAsks`: `candy_listing_floor.confirmed_floor_usd` + this mint's ask seen in the last 12 h, labelled "Magic Eden ask" / "Listed"); its Similar editions tiles name the Rainbow colour (`candyParallelFromExternalId`). ⚠ The team/player "Floor" cells are NOT a floor ask — see known-issues #143.
...
**Per-collection Set Tracker backends** (five now, one per data model — `CollectionSetsClient` dispatches on the slug): Top Shot `/api/sets`, All Day `/api/allday-set-progress`, UFC `/api/ufc-set-progress`, Pinnacle `/api/pinnacle-set-progress`, **Candy MLB `/api/candy-set-progress` (2026-09-25)**, everything else (Golazos) the generic `/api/sets-db`. ⚠ Candy cannot use `sets-db`: it `.toLowerCase()`s the wallet (base58 then matches nothing) and counts editions (125) where the checklist is the 100 PLAYERS — the five-colour Rainbow cards are parallels (`totalPrintings`), exactly as Top Shot's `::subID` and Pinnacle's variants. Measured at ship: 72 of 425 Candy wallets hold all 100 players; only 2 hold all 125 printings, so edition grain would have hidden 70 real completions. ⚠ Only the first two expose a per-set DETAIL endpoint (`?set=`); the other three carry their piece lists inline on the list response.
...
**Disney Pinnacle's per-render page is `/disney-pinnacle/edition/<render_id>` (moved 2026-09-20).** It used to be `/pinnacle/moment/<render_id>` — outside `(collections)`, under a slug that is not the collection's URL slug, using a noun the page never used for itself (its own metadata said "Pinnacle edition") — while the house-shaped URL existed and **308'd away to it**. A render IS the edition-grain object, so the shared `[collection]/edition/[slug]` route now renders it via `PinnacleEditionDetail.tsx` beside the route. Consequences worth knowing:
- ⛔ **`/pinnacle/moment/<id>` is a PERMANENT REDIRECT and must never be deleted** — ~2,600 of its URLs were in the sitemap and it was the site's **11th busiest route** (125 visitors / 173 pageviews in the 30 d to 2026-09-20). A 308 transfers those signals; a 404 throws them away. `next.config.ts`'s `/pinnacle/:page(...)` allowlists still deliberately EXCLUDE `moment` so the redirect page is reachable in one hop.
- **`/disney-pinnacle/moment/<id>`** (the guessable shape) now 308s to `/disney-pinnacle/edition/<id>` — in-namespace; it used to bounce out.
- ⚠ **The Pinnacle arm `await connection()`s** so it keeps rendering per request. The segment exports `revalidate = 600` and the old route had no segment config; without that call the move would have put a page whose failed-read branch says "this pin didn't load" behind a 10-minute ISR cache. Whether Pinnacle *should* be on ISR is open and needs its own measurement.
- ⚠ **The key space is still `pinnacle_catalog.render_id` (or a legacy `edition_key` → disambiguation), NOT `get_set_detail`/`get_edition_detail`'s.** That is why the segment layout's existence gate skips Pinnacle; the body 404s for itself.
- ⭐ **Guard: `every-collection-entity-url-is-in-the-collection-namespace.test.ts`** bans any source outside the redirect stub from building the retired URL, and pins the redirect + the `connection()` call.
...
**Outbound marketplace links — MOMENT grain vs EDITION grain (`lib/collections.ts`).** The native
marketplaces are deep-linkable at **moment grain only**, which is why `marketplaceMomentUrl` declines to
build an edition URL and why the canonical edition page carried **no outbound CTA at all** until 2026-08-22.
dapper.market publishes a real per-EDITION page keyed by the numeric on-chain editionID — exactly what
`editions.external_id` already holds — so `dapperMarketEditionUrl` builds one with no new data.

- ⚠ **GOLAZOS ONLY, and that is the design, not a gap.** `nba-top-shot` can NEVER join from
  `external_id`: it is a set+play composite (`<setUUID>:<playUUID>`, or `258:9004::16` for a subedition) —
  **0 of 19,838 TS external_ids are numeric**. `nfl-all-day` DOES store a numeric editionID (6,190 of
  6,190) so the URL shape is plausible, but **it is UNVERIFIED and therefore deliberately unmapped** — an
  unverified outbound CTA is a 404 pointed at a collector. ⚠ **This is a filed decision NOT to act, the
...
- The CTA is suppressed on a **closed market** (`isMarketClosed`), so a future map entry cannot resurrect a
  "go buy this" link for a venue that stopped trading (UFC's Flow market, frozen 2026-05-13).
- 🚨 **Its ONLY instrument is `e2e/entity-smoke.spec.ts`'s `edition_golazos` arm** (`expectText:
  /View edition on Dapper/i` on its `ENTITY_SPEC` entry). The coverage gates include `app/**/route.ts` but
  **not `page.tsx`**, and jsdom renders no page at all — so nothing else in the repo can see whether this
  CTA renders. **A red on that arm means the CTA stopped rendering, NOT that the page is down.** Verified
  live 2026-08-26 (dispatched run 32928760902): `✓ entity · edition_golazos detail page renders (7.9s)`
  with the assertion in place.

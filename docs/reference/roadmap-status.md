...
1. 🔔 **UPDATE 2026-09-25 (Trevor): Flowty will turn its API endpoints off "soon" — the 07-07 "API is alive, do NOT tear down" note below becomes false on that day.** Live Flowty consumers re-measured 09-25: `topshot-/allday-/golazos-listing-cache` (216 runs / 72 h each, ~7k rows/day each, via the `flowty-proxy` edge fn) → legacy `cached_listings` (`source='flowty'`). `pinnacle-sync` no longer touches Flowty (since 06-08); the Pinnacle sniper's Flowty leg is dormant. **Already safe for the cutover:** the routes preserve the prior cache on a failed sweep, `purge-stale-listings` deletes rows > 48 h daily (04:00 UTC), and `fmv_from_cached_listings` prices only listings ≤ 2 h old (`20260925152928`), so nothing is re-stamped as fresh. **On the day it goes dark:** (a) confirm by the routes' own `pageErrors`, not one probe; (b) retire the Flowty sweeps in the three listing-cache routes + their schedulers and `flowty-proxy`, rather than letting failure alarms fire; (c) check every reader of `cached_listings` Flowty rows renders an honest "no listing source" state once they purge — **Golazos is covered — ⚠ CORRECTED the same day:** an earlier line here said its listings had no other source and that its `cached_listings_v2` rows were frozen at 08-01. **Both wrong** — that date was `collections.updated_at` leaking through a join (`cached_listings_v2` has no `updated_at`), and a storefront walk found **1,391 live Golazos listings across 442 editions** on Dapper's NFTStorefrontV2, far more than Flowty's 67 moments. `golazos-storefront-reconcile` (Vercel cron, every 2 h, shipped 09-25) now keeps them in `cached_listings_v2`; and since 2026-09-25 ~4:00 PM PT Golazos ASK_ONLY is priced from it (`refresh_golazos_ask_fmv_from_listings`, pg_cron 614, 6 h `verified_at` bound; `fmv_from_cached_listings` prices no Golazos) — so Flowty going dark costs Golazos nothing; (d) Top Shot's 51k `source='topshot'` rows are Atlas-fed and unaffected. ⭐ **Also resolved 2026-09-25: All Day.** Flowty's storefront FORK has purchases disabled on-chain, and its 27,406 "open" All Day listings had been setting All Day's ask floor (the FMV ceiling) for 2,724 editions; closed `unpurchasable` (`20260925230134`). The storefront reconciler now covers All Day too, so after the cutover All Day and Golazos asks both come from Dapper's own storefronts. What is left for the day Flowty goes dark is (b) retiring the Flowty sweeps and `flowty-proxy` — no data dependency remains.

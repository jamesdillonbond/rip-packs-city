...
- ⚠ **Leg 1 (`topshot_impossible_parallel_serials`) costs 225s to return a structurally-near-zero count.** It is `editions JOIN sales WHERE serial_number > circulation_count` over TopShot. Splitting makes it survivable but not cheap; it wants its own index/predicate review. Its generous 300s budget avoids a false 999 in the meantime.

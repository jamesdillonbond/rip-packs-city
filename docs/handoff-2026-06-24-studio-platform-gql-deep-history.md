...
3. **Golazos + Pinnacle** next. **Per-collection filter key (Cowork-derived from our schema; Chrome was down so the GQL introspection itself is CC's last step):**
   ...
   - **UFC — NO LONGER GATED (Cowork 2026-06-24).** The earlier "gated, needs a (set_id, edition_num) map" conclusion is overturned. `searchUFCMarketplaceHistory` takes **no edition filter** (only optional `set_id`), but each row carries `nft.set.metadata.{athlete_name, edition_size}` + `nft.edition_num` (serial) **inline** — so resolve to our edition per-row, no pre-built map. Our 518 UFC editions have `player_name` + `circulation_count` on 518/518 and `(player_name, circulation_count)` is **unique (0 collisions)** ⇒ clean 1:1 to studio `(athlete_name, edition_size)`; **verified 40/40 exact** on a live sample (accented names included). **Cowork shipped the resolver** `audit_20260624_ufc_studio_history_resolver_and_targets`: `resolve_ufc_edition_by_studio_meta(athlete text, edition_size bigint) → editions.id` (service_role-only; returns NULL for packs/`set_id=0`/uncataloged → CC skips) + queue view `ufc_studio_sales_history_backfill_targets`. **UFC build = walk `searchUFCMarketplaceHistory` unfiltered (paginate `{first:200, after}`, `purchased=true`), per row call the resolver on `(set.metadata.athlete_name, set.metadata.edition_size)`, write matched rows to `sales` with `source='ufc_studio_history_v1'`, serial=`nft.edition_num`, price=`sales_price`/1e8, tx/block from `created_at`, dedup by `transaction_hash`.** Note: 860,676 total UFC sales exist studio-side (most for editions we don't catalog), so either low-cadence-cron the unfiltered walk, or (efficiency) on first encounter store the resolved `set.id → editions.id` and populate `editions.set_id_onchain` (currently 0 for UFC) to enable per-`set_id`-filtered drains thereafter. Edge case to guard: if studio ever has two sets at the same `(athlete, edition_size)` for different events, both map to our single edition — rare given our catalog; acceptable for the thinnest market.

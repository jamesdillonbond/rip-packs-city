...
7. ⛔ **`app/api/recent-sales` line ~55 is DONE — do not re-open it from inbox `2026-09-05T0555Z`.** That filing names it "the one worth doing next"; the edition lookup is already `boundedRead` + `apiErrorResponse` on error + an explicit empty on no-match, and its header documents the defect it replaced.

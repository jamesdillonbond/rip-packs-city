...
14. ✅ **#54 decided: `match-topshot-players` gates its walk behind its inputs** (migration `20260904043939`, edge fn v30). Expect `extra.gated: true` + `gate_reason` on the daily 08:00Z tick and a full run again on/after 2026-09-11. ⛔ Do NOT suppress `check_pipelines_running_but_not_succeeding()` for it — a gated tick has rows_written 0 by design; read `extra.gated` before calling it a stall.

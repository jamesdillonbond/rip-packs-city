...
9. ⓘ **`pg_stat_statements` is at 99.3% of its 5,000-entry cap with `dealloc = 36`** since the 2026-08-12 reset, and **972 entries (19.6%) are MCP-tagged, 817 of them with `calls = 1`** — one-shot agent queries, each carrying a unique `-- date:` comment and so a unique queryid. ⛔ **Filed as context, NOT as an action item, and one hypothesis about it is already REFUTED:** `ops_pgss_delta` handles eviction correctly — it flags `counter_reset` when live calls fall below the baseline and reports live counters whole rather than emitting a negative. Raising `pg_stat_statements.max` needs a restart and is Trevor's call, not a night-pass change.

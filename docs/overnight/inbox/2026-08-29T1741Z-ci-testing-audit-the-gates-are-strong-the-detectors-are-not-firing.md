...
- ⛔ **`npm run lint` never ran — and MEASURING it corrected the finding. It was a RULE-SET MISMATCH, not neglect.** ci.yml said so in a comment (*"do not cite it as coverage anywhere"*), and I filed it as a plain gap. Measured 2026-08-29 over 2,857 files: **6,474 violations across 20 rules, of which 5,757 (89%) are `@typescript-eslint/no-explicit-any`** — which is a **documented convention here** (*"Supabase client typed `any` in API routes"*). Turning eslint on as a gate would be a 6,000-error wall that gets switched off within a day. ⚠ **And I misread the report first:** I classified 24 messages as *files eslint could not parse*, which would have been far more alarming. They are **unused `eslint-disable` directives** — messages with no `ruleId`. **Zero files fail to parse.**

  ✅ **SHIPPED: an eslint RATCHET, not a gate** (`eslint-ratchet` job + `scripts/check-eslint-ratchet.mjs` + committed `eslint-ratchet.json`). It excludes `no-explicit-any` **at the rule's granularity, not the plugin's** — muting `@typescript-eslint` wholesale would also silence `no-unused-expressions`, `no-require-imports` and `no-empty-object-type`, which are not conventional here — and bounds the residual **717 violations across 19 rules**, several of them correctness rather than style: `react-hooks/set-state-in-effect` (59), `@next/next/no-html-link-for-pages` (114), `react-hooks/static-components` (15), `react-hooks/purity` (6). Counts may only shrink; a regression is loud on the commit that introduces it. ⭐ **A rule NOT in the baseline that appears is a FAILURE**, because an unmeasured rule is not a pass — that is how a ratchet goes blind after a plugin upgrade. ⚠ **And it refuses to report an all-clear on a missing or empty report**, because *"eslint found nothing"* and *"eslint did not run"* produce the same zero. Verified end-to-end all three ways (clean → 0, an injected `var` → 1 naming both grown rules, missing report → 2) and mutation-checked five ways.
...
🚨 **SIZED, GUARDED, NOT FIXED — closing the passthrough immediately surfaced a second escape nobody had recorded.** `next/og` fetches emoji SVGs from **`https://cdn.jsdelivr.net/gh/twitter/twemoji/...` at RENDER time**, so four cards (`og/collection`, `og/deal`, `og/pack`, `og/pack/lifecycle`) reached a third-party CDN on every run of the suite — **and do so in production on every uncached render.** That is the same unbounded-external-fetch shape as the fonts, on a host we do not control, in the path X's crawler is waiting on. ⛔ Not fixed here: it needs satori's `loadAdditionalAsset` wired to local assets, which is a real change to how every emoji-bearing card renders and wants its own measurement of which cards actually carry emoji. Stubbed in the sweep so the suite is hermetic.

**Sized properly (2026-08-29, tree walk over all 44 OG routes, comments stripped):** **6 render a hardcoded emoji** — `collection`, `deal`, `insights/serial-premiums`, `pack`, `pack/lifecycle`, `profile/[username]` — and **none sets an explicit `maxDuration`** (5 are `runtime=edge`, 1 `nodejs`). The three codepoints observed escaping were 🎯 `1f3af`, 🎴 `1f3b4`, 📦 `1f4e6`.

⚠ **Two of the six never appeared in the sweep's escapes** (`insights/serial-premiums`, `profile/[username]`) because their emoji sit on branches its fixtures do not take — **so the render sweep is a LOWER BOUND, and the source scan is a DIFFERENT lower bound. Neither is a census**, because `og/collection` renders `collection?.icon ?? "🎴"`: a collection whose icon is an emoji reaches the CDN through DATA, which no source scan can see.

⛔ **There is no central fix, and this is why it is filed rather than done.** `ImageResponseOptions` exposes exactly one knob — `emoji?: EmojiType` — and every preset (`twemoji`, `openmoji`, `blobmoji`, `noto`) is a remote URL. **There is no `loadAdditionalAsset` hook on the public API**, so the dependency cannot be moved to local assets without leaving `ImageResponse` entirely. Removing the emoji from six cards is a design call, not a cleanup, and I am not making it unasked.

✅ **What IS shipped: `og-cards-do-not-silently-acquire-a-cdn-dependency`** — a tree walk that records the six and fails when a SEVENTH appears, so adding an emoji to a crawler-facing card becomes a visible decision instead of a silent CDN dependency. The recorded set may shrink freely and only grows deliberately; a second case fails if the list names a route that no longer renders one, so it cannot rot into a description of the past. Mutation-checked three ways, including a **false-positive control** that an emoji in a COMMENT does not trip it.

⭐ **A third existing guard caught me while writing it:** `guards-use-the-shared-comment-stripper` reddened because I had rolled my own stripper. Re-pointed at `scripts/lib/strip-comments.mjs` — and since CLAUDE.md records that *using* it is not proof it stripped (blind three times), the comment-only case is kept as a control on the stripping rather than as a formality.

...
## ✅ DRAINED ~01:15Z — item 4 and the prose are shipped too. Nothing here is open.

- **(4) `/api/best-offers` — SHIPPED** (`edb727ff1`), and the measurement moved it UP, not down. The guess above ("probably fresher via three other indexers") was refuted twice over: `topshot-offers-indexer` writes `public.offers`, **not** `edition_offers`, and the route's *ordering* was wrong as well as its labelling. It read `edition_offers` first and `badge_editions` only for missed keys, on the premise — in its own header — that the sweep is fresher. **True for Top Shot (33.0 h vs 111.7 h); false for All Day (168.5 h vs 1.0 h).** Of 2,052 All Day keys in both, **1,925 (94%) had a badge row a day+ fresher and 137 (6.7%) disagreed on the value.** ⭐ Both numbers matter: the 94% is why the rule was wrong, the 137 is how many editions actually showed a wrong bid — quoting only the first overstates it 15×. Fixed as a RULE (prefer the most recently confirmed row), not a per-collection switch. ⚠ Two of the four legs stay **unmarked** because they cannot be dated: `get_serial_offers` returns no timestamp, and `marketplace_offers.created_at` is when the offer was MADE — labelling that "unconfirmed 90d" would repeat the All Day `floor_ask_listed_at` trap.
- **The methodology prose — SHIPPED** (`bd15c3bfc`). It paired "Live Top Shot ask data comes from `edition_offers`" with `refresh: "Every 5 minutes (page revalidate)"` — both true, and together saying the asks are five minutes old. ⭐⭐ **My first guard for it was too blunt and fired on an ACCURATE sentence** (the Packs entry bounds its "live ask" in the same breath, and that feed was 282/282 ok). *A guard that reds on a correct qualified statement teaches people to delete the qualifier.* Narrowed to paragraphs naming the feed that actually broke.

⛔ **The only ask-staleness work left is not code: the upstream itself.**

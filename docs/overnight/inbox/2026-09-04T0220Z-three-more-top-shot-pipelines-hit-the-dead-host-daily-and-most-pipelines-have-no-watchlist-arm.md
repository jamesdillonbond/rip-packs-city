...
> ✅ **§1 RESOLVED 2026-09-04 (PT) — decided, not deferred. Do not re-file it.** All three are settled and each disposition is recorded where the code lives, not only here. **`topshot-badge-set-backfill`** — unscheduled (route kept; the Atlas walk covers 266 of 266 sets and keeps 13,891 of 13,915 badge rows fresh within 6 h; the one gap, set 152 *2023-24 Honors (Diced)*, returns `total_count = 0` from Atlas and this route could not serve it either). **`topshot-catalog-backfill`** — unscheduled (`vercel.json` 36 → 35) only after all three of its jobs had live owners: circulation on-chain (09-03), tier + badges from the Atlas walk (**0 of 13,436** canonical editions have a NULL tier), and prose + media from that same walk as of migration `20260905024630`. ⚠ **The one job nothing inherited is new-edition creation** (Atlas knows parallels we do not carry — 16 of 100 on a sampled page); left open on purpose, because it ripples into circulation, the sitemap and every entity surface. **`topshot-misattrib-drain`** — the 215-row measurement below stands: a one-off script when the residue matters, not a pipeline. §2 (the ~80 unwatched pipelines) is separately answered by the 40 seeded arms in migration `20260904044417`; the ⛔ against auto-seeding the rest still holds.
...
> 🟠 **PARTLY RESOLVED, same night (ledger 2026-09-03, `topshot-circulation-onchain`):** the CIRCULATION half of `topshot-catalog-backfill` is ported to the chain (`TopShot.getNumMomentsInEdition`, base rows only, normalised compare). What the walker still did that nothing does now: tier, media URLs, new-edition creation — those are not on chain and remain the Atlas decision. `topshot-badge-set-backfill` and `topshot-misattrib-drain` are unchanged (badges are not on chain; the drain's holder-then-borrow port is the same shape as the serial lane and is the next candidate). **Measured 03:3xZ: `topshot_misattrib_drain_targets(2000)` returns only 215 rows** (152 with a wmc holder, 209 with any buyer; the on-chain map already holds 49,206) — a port would drain once and idle, so it is a one-off script when the residue matters, not a pipeline.
...
> ✅ **RESOLVED 2026-09-03 (late PT), under Trevor's delegation — ledger 2026-09-03, migration `20260904044417`:** 40 arms seeded at `info` from a derived population (≥ 12 active days, ran yesterday/today, not a heartbeat, not DB-internal, not already watched, not `sync-nba-projections`), each sized from its OWN 73 h profile (silent = 3× max gap, floor 60; no-success = 3× max ok-gap, NULL for the four that never succeeded in the window). `info` because the sentinel pages only on `high` — this is the visibility half; promotion stays a human read of the first weeks, exactly as the paragraph below asks.

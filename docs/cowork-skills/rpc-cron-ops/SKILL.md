...
- 🚨 A `javascript_tool` result is BLOCKED (`Cookie/query string data`) if it returns any URL carrying a query string — and the edge-function entries' URLs carry `?key=` gate keys, so a list-page sweep must strip everything from `?` on before returning, or return titles/status only. That block is the secret guard working; do not try to route around it.

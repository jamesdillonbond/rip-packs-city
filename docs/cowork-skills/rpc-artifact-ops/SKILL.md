...
- 🚨 **`extractRows` MUST THROW ON A FAILED READ — it used to return `[]`.** Measured 2026-09-14: the live Supabase MCP server answers a missing relation with `{error:{name,message}}`, which fell straight through the object branch to `return [raw]`; the MCP `{isError:true, content:[{text}]}` shape returned `[]` outright. Neither threw, so the honest `catch` branch every artifact already has NEVER FIRED, and `rpc-insights-health` rendered the banner **"7 surface(s) have an EMPTY backing view — investigate."** off a query that never ran. A renamed view, a revoked grant or a statement timeout is published as total data loss. The estate helper now carries, as the FIRST line of its object branch:
  `if(raw.isError===true||(raw.error&&typeof raw.error==="object")) throw new Error(...)`, plus `throw` (not `return []`) when a non-empty string parses to nothing. **Only a genuinely empty result set — `null`, `""`, or a payload that parses to `[]` — may return `[]`.** Copy the helper from `docs/cowork-skills/rpc-insights-health.html`; a hand-written one will reproduce the defect.
- 🚨 **THE LIVE ESTATE IS NOT FIXED YET, AND IT IS 15 ARTIFACTS, NOT THE "five" #114 first said.** Only the `docs/cowork-skills/*.html` repo copies carry the corrected helper. Re-derived **2026-09-18** (unchanged since 09-14) by walking Claude Desktop's artifact store: **15 carry an `extractRows` helper and 15 of 15 still fail to throw on `{error:{…}}`.** ⚠ **Eleven of them DO check `isError` at the call site** — which reads as error handling, survives review for exactly that reason, and is pointed at a shape the server does not send. **So do NOT scope this job by grepping for a missing `isError` check: that returns 4 and looks small.** The property is *does the helper throw on `{error:{…}}`*. Start with the four blind at both layers: `rpc-growth-funnel`, `rpc-qa-scorecard`, `rpc-rewards-console`, `rtr-pack-finder`. **Measure, do not count commits:** `node scripts/audit-cowork-artifact-failure-handling.mjs` → **0** clean · **1** vulnerable · **2** store missing/empty. ⛔ **Republishing is a Cowork DESKTOP action (`update_artifact`) — Claude Code's `Artifact` tool addresses a different system and cannot do it, and hand-editing the store's `index.html` bypasses its versioning and desyncs its index.**
- ⛔ **Never derive "last refreshed" for a materialized view from `pg_stat_all_tables.last_autoanalyze`/`last_analyze`.** Postgres stores no refresh timestamp, and autoanalyze only fires on a row-churn threshold, so a small MV reads as years stale while refreshing on cadence. Measured 2026-09-14: `mv_topshot_pack_reality_top_ev` showed 16 days by that proxy and had succeeded 51 minutes earlier; four MVs showed the 1970 epoch. **The authoritative instrument is `cron.job_run_details` joined to `cron.job`** (`max(start_time) filter (where status='succeeded')`). An MV with no refresh job at all is the real finding — surface THAT, and date the data from its own newest row.

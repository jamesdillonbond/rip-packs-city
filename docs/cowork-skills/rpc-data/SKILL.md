...
- **`editions`** (29 cols) — `external_id` (integer-pair `setID:playID` is CANONICAL for TS; UUID-pair rows are inert dupes). **Always filter product metrics to canonical: `external_id ~ '^[0-9]+:[0-9]+$'`** — there are thousands of inert UUID rows. Denormalized `player_name`/`set_name`/`tier`/`team_name`/`circulation_count` are safe to select. `players.jersey_number` exists (drives jersey-match special serials). Pinnacle lives in `pinnacle_editions` + `pinnacle_catalog` (render_id PK).
- **`fmv_snapshots`** — partitioned, NO source column. `confidence` enum UPPERCASE: `HIGH|MEDIUM|LOW|ASK_ONLY|SALES_ONLY|STALE|NO_DATA`. **Latest per edition:** `SELECT DISTINCT ON (edition_id) ... ORDER BY edition_id, computed_at DESC`. Daily duplicates are intentional history. Pinnacle FMV: current per-render values on `pinnacle_catalog.fmv_*`; per-render CHANGE HISTORY in `pinnacle_fmv_history` (trigger-fed from pinnacle_catalog, seeded 2026-06-08); legacy `pinnacle_fmv_snapshots` still live with many readers — don't assume retired.

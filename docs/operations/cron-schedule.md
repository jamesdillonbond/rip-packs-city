...
- **RPC TopShot Pack Opens History** → `POST https://bxcqstmqfzmuolpuynti.supabase.co/functions/v1/ingest-topshot-pack-opens-history?mode=backfill&key=rpc_pls_7q4w2z8n_tsopenhist` — schedule `4,19,34,49 * * * *` (every 15 min, clean slots with 1-2 jobs/min), no Authorization header needed (verify_jwt=false, key gate in URL), expect **200**. Backfills historical TopShot pack-opens walking DOWN from block 137,390,145 to the spork-proxy floor (27,341,470). Edge function deployed as v2 ACTIVE 2026-07-11. NOT YET WIRED to cron-job.org — Chrome unavailable at time of ship. Wire on next Chrome session: add as a new Supabase edge fn job on console.cron-job.org (COMMON tab only, never ADVANCED). Verify: pack_rips row count climbing for collection 95f28a17-224a-4025-96ad-adf8a4c63bfd. Remove this entry once the cron is confirmed running.
...
- **2026-07-10 ~22:40 PT — cron-job.org dropout (~10.5h)**: All Vercel-route cron jobs went silent from ~10:40 PM PT to ~9:00 AM PT the next day. Supabase edge function jobs (compute-topshot-pack-ev, wmc-fmv-populate) were unaffected. GHA backstops kept sales ingest alive; fmv-recalc was dark ~6.5h. Self-healed without operator action. Root cause unknown (cron-job.org service issue or account-level transient). Full dashboard audit pending (Chrome required). No auto-disabled jobs confirmed but unverified — check disabled status on next Chrome session.

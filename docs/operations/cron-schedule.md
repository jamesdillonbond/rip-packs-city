...
| rpc-ts-listings-atlas-sync | 466 | `*/2 * * * *` | `atlas_listing_verify_tick(3)` as `postgres` | 20260907020428: rebuilds `ts_listings` (the Top Shot sniper's serial feed) from open Atlas nba listings verified in the last 24 h, then re-reads 3 listings as `{nftId}` so cancellations flip. Even minutes; the market drain (463) runs odd minutes. Pipeline `ts-listings-atlas-sync`; watchlist 20/45. |

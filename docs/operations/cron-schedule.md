...
|---|---|---|---|---|
| rpc-wmc-metadata-reconcile | 456 | `15,45 * * * *` (was `*/10` 09-04 → 09-07) | `reconcile_wmc_metadata_from_editions(1200, 45)` as `postgres` | 20260907014949: the wmc set-name/mint-count reconciler converged 09-04 ~21:00Z and was the DB's #1 physical reader at `*/10` (~57K reads/tick for 2–220 rows/hour — inbox 2026-09-05T1630Z). Every 30 min now; cadence watchlist row armed 20260907015019 (100/200 min). |
| rpc-topshot-moments-hydrate-wmc | 468 | `19,35,55 * * * *` | `hydrate_topshot_moments_from_wmc(15000)` as `postgres` (110 s) — was `(5000)` until 21:37Z 09-07 | 20260907051909: hydrates `moments` for pack-pulled Top Shot nfts from `wallet_moments_cache` (50,414 of the 212,479-row queue were resolvable with NO external call, measured 09-07); walks the queue newest-first behind a `backfill_state` cursor (`topshot-moments-hydrate-wmc`), ~5.7 s / ~1,300 rows per tick, a full pass ≈ 14 h then wraps. Writes through `replace_topshot_moments_batch`. Cadence watchlist 70/140 min (20260907051953). The paused `topshot-moments-hydrator` worker (dead host) is untouched. **09-07 15:26Z (`20260907152641`): second source — `topshot_atlas_market_events`** (a listing or sale names edition + serial; 2,478 resolvable at apply); wmc rows win when both exist. **15:50Z (`20260907155014`): third source — `sales`** (a recorded sale names edition + serial + buyer; 5.7 % of the newest 20K, and 22 of the chain lane's first 54 `no_nft`). Rows none of the three has seen go to the chain hydrator (469, below). **21:37Z (`20260907213736`): the page is a bounded slice of the queue INDEX (15,000 rows, named or not), not "5,000 unresolved rows" — the old page walked every already-named row at the top of each pass, unbounded once the backlog is gone.** |
| rpc-topshot-moments-hydrate-chain | 469 | `3-59/4 * * * *` | `topshot_moment_hydrate_tick(80)` as `postgres` | 20260907153117 (+ 153248 classifier): the REST of the pack-pull queue, read ON-CHAIN from the database — each tick drains the answered requests, then POSTs ≤80 Cadence `borrowMoment(id)` scripts to `rest-mainnet.onflow.org/v1/scripts` via pg_net (0.33 req/s), recorded in `topshot_moment_hydrate_requests` (request_id = pg_net id). 200 → decode JSON-CDC, resolve `set:play[::sub]`, write via `replace_topshot_moments_batch`; 400 `panic: no nft` / `no collection` → the Moment moved on (`no_nft` / `no_collection`, retried after 30 d); other → `error` (1 d); no response in 3 min → `timeout`. Pipeline `topshot-moments-hydrate-chain`; watchlist 15/30 min. First cron drain 15:39Z: 80 → 56 written / 24 `no_nft` / 0 error, 1.6 s. Backlog 174,970 at the first tick → ~6 days at 28.8K/day, minus the ~30 % that have left the puller's wallet. **15:50Z (`20260907155014`): candidates any free source (wmc / Atlas events / sales) will name are skipped** — `no_nft` fell from 30 % to 1–2 % of answers. **Throttle signal: `status_code = 429` in the requests table.** 16:11Z (`20260907161134`): the tick prunes `written` requests older than 2 days (`extra.pruned`); retry-window rows are kept. 16:22Z (`20260907162237`): the sentinel's 4xx arm attributes this lane by request_id — a moved-Moment panic reports as `flow-rest-moment-moved-400` (info), any other 4xx as `flow-rest-upstream-<code>` (high). 21:19Z (`20260907211915`): the dispatch walk is CURSORED (`backfill_state` `topshot-moments-hydrate-chain`, 3,000 rows examined per tick, wraps at the end) — the newest-first re-scan had grown to 69K buffers/tick in six hours; 21:42Z (`20260907214240`) the page is walked in order with per-row tests, exiting at the 80th candidate — 5.5 s → 1.1 s. |
| rpc-sales-atlas-sync | 471 | `6-59/10 * * * *` | `sync_sales_from_atlas(2000)` as `postgres` (110 s) | 20260907233444: Top Shot listing sales the on-chain indexer DROPS (no edition resolver since the GraphQL host died; ~1,350/day skipped) are written from `topshot_atlas_market_events` as `sales.source='atlas'` — walks the events behind a `last_seen_at` cursor, purchases > 2 h old, deduped by nft ±10 min against every other writer. ~1,100 live sales/day + the histories the edition verify lane walks. Watchlist 30/60. Addresses normalised by `flow_addr_0x()` (20260907233849). Register #67. |
| rpc-prune-atlas-market-events | 473 | `27 * * * *` | `prune_topshot_atlas_market_events(7, 20000)` as `postgres` (110 s) | 20260908004325: retention for `topshot_atlas_market_events` (it had none; ~90–100K rows / ~45 MB a day). Deletes completed `nba` events older than 7 days, oldest first off `idx_tame_completed_seen`, ≤ 20,000 per run, behind two guards — never past the `sales-atlas-sync` cursor (`backfill_state`), never an nft still in `v_moments_needing_hydration`. `nfl` rows untouched. First non-zero run ≈ 2026-09-14; steady state ≈ 700K rows. `pipeline_runs` `atlas-market-events-prune` (`extra.page/deleted/kept_hydration/cutoff`); watchlist row info 180/360. Watch `kept_hydration` — if it nears the page size the head is full of kept rows and the walk needs a cursor. |
| rpc-topshot-unmapped-atlas-resolver | 476 | `50 * * * *` | `topshot_resolve_unmapped_via_atlas()` as `postgres` (60 s) | 20260908141132: writes `nft_edition_map` rows for OPEN parked Top Shot sales (`unmapped_sales`, parked by `83820bd8`) from `topshot_atlas_market_events` — newest event per nft, `atlas_edition_id` → `topshot_atlas_edition_map` → canonical edition + serial, via `upsert_nft_edition_map_batch`, never overwriting. Nothing else feeds `nft_edition_map` for Top Shot since the GraphQL ingest died (~08-28), so without this a parked sale that is not an Atlas LISTING purchase never promotes. Runs 4 min ahead of jobid 474 (`:54`), which promotes the mapped rows with their tx hash and on-chain price. First run 14:11Z: 337 mapped / 337 promoted / 0 duplicates. **LEG 2 added the same day** (`audit_20260908_topshot_unmapped_probe_leg` + `..._probe_leg_retries_transport_failures`) because the stated watch fired — `open_unseen_by_firehose` went 18 → 36 in four hours: it probes Atlas for parked nfts the firehose has NEVER named (`{nftId}`, marker `offset_at=-6` / `error=__nft__<id>`), max **12 per run minus in-flight**, one probe per nft per 14 days — and the 14-day stamp binds only while the recorded response is not a transport failure (`resolution_hint.atlas_probe_req` → `net._http_response`; a 403 is retried next tick, a 200 that simply does not name the nft keeps its 14 days). `pipeline_runs` `topshot-unmapped-atlas-resolver` (`extra.candidates/mapped_from_events/probes_dispatched/probes_retrying_transport_failure/open_unresolved/open_unseen_by_firehose`); watchlist row info 180/360. Proven end-to-end 19:0xZ: 12 probes → 9×200 / 3×403 → 8 nfts newly named → 8 mapped → `open_unseen_by_firehose` 42 → 34. ⚠ `open_unseen_by_firehose` should now FALL; if it climbs while `probes_dispatched` > 0, the probes are answered without the nft (a real Atlas gap), not a budget problem. |
| rpc-topshot-dupe-sales-watch | 480 | `38 3,9,15,21 * * *` | `check_topshot_dupe_sales(7)` as `postgres` (60 s) | 20260908: the DETECTOR for known-issue #68 (33,000 duplicate Top Shot sales, drained 2026-09-08). Counts groups sharing `(nft_id, transaction_hash, price_usd)` in a 7 h window (1 h overlap) and logs **`ok=false` when the count is non-zero**, so the `failure_rate` arm of `get_pipeline_alerts()` reports it with no edit to that function; watchlist row high 480/1440 covers silence. ⛔ A RED RUN MEANS A SECOND WRITER IS DUPLICATING SALES AGAIN — read `extra.sample` for the nft/tx/sources. The unique index CANNOT catch this class: it is keyed `(transaction_hash, nft_id, sold_at)` and the writers disagree about `sold_at` by ~3.4 s. ⚠ **Do NOT widen the window** — 45 d costs 75,473 buffers / 23,700 disk reads / 14.2 s and spills to disk; the 7 h window costs 429 buffers / 48 ms. Proven green→red→green on a restored real duplicate 2026-09-08 23:4xZ. |
| rpc-ts-listings-atlas-sync | 466 | `*/2 * * * *` | `atlas_listing_verify_tick(2)` as `postgres` (⚠ live command reads `(2)` — this row said `(3)` until 2026-09-07; the EDITION verify lane inside the tick is 4/tick since `20260908035519`, was 2) | 20260907020428: rebuilds `ts_listings` (the Top Shot sniper's serial feed) from open Atlas nba listings verified in the last 24 h, then re-reads 3 listings as `{nftId}` so cancellations flip. Even minutes; the market drain (463) runs odd minutes. Pipeline `ts-listings-atlas-sync`; watchlist 20/45. **09-07 13:58Z: both listing syncs are DIFFERENTIAL (`20260907135757`) — the open set (~15K rows) was being deleted and re-inserted every tick; now delete-gone / insert-new / update-changed, tick 3.3 s → 1.6 s.** |
...
| rpc-refresh-candy-scarcity-board | 436 | `26 * * * *` | `SET statement_timeout = '300s'; REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_candy_scarcity_board` (as `postgres`) | 20260903221326: `candy_scarcity_board` is a wrapper over the MV now (was a live 16k-buffer wmc scan on every read; 390 s tail). Watchlisted in `board_mv_refresh_watchlist` at 6 h. |
| rpc-weekly-wmc-reindex-1 / -2 / -3 / -4 | 438–441 | weekly Sunday `3 2`, `23 2`, `43 2`, `3 3` (as `cron_heavy`) | one bare `REINDEX INDEX CONCURRENTLY` each: idx_wmc_coll_ek_serial_cover / idx_wmc_moment_collection_cover / wallet_moments_cache_wallet_collection_moment_key / idx_wmc_cohort_cover | 20260903223235: the 08-30/31 campaign became maintenance (register #56). Sized off the second wave (33–50 s each in this band) inside the standing 600 s. ⚠ A `failed` here is not "work not done" — read the verify row. |
| rpc-weekly-wmc-reindex-verify | 442 | weekly Sunday `23 3` (as `cron_heavy`) | `run_wmc_reindex_verify()` → `pipeline_runs` `wmc-reindex-verify` | First reading 2026-09-06 03:23Z; `invalid_left` there = a `<index>_ccnew` to drop by hand as ONE bare `DROP INDEX CONCURRENTLY IF EXISTS`. |
| rpc-pipeline-gap-hourly-rollup | 443 | hourly `7 * * * *` (as postgres) | `rollup_pipeline_gaps(3)` → `pipeline_gap_hourly` (last 3 complete hours, idempotent) + `pipeline_runs` `pipeline-gap-hourly-rollup` | Calibration series for the correlated-skip arm (inbox 2026-09-03T0300Z) — NOT an alarm, nothing reads it yet. Migration `20260904024810`, 2026-09-04. |
| rpc-selfheal-impossible-parallel-circ | 219 | `43 0,6,12,18 * * *` (was `52 */6`) (as `cron_heavy`) | `raise_impossible_parallel_circ()` | 20260903221540: moved BEFORE trust leg 324 (`48 0,6,12,18`) so the arm reads the post-heal residue instead of publishing every routine inflow as a six-hour BREACH. |
| rpc-backfill-pinnacle-acquisitions | 78 | `17 */6 * * *` (as `postgres`, 120 s default) | `backfill_pinnacle_acquisitions(50000, 14)` (was `(50000)`) | 20260904014220: 14-day `sold_at` window — the whole candidate join was 6,573 rows, all already inserted, and the unbounded scan was killed at 120 s one tick in five. ~300 ms now. NULL window = the historical (unbounded) behaviour. |
...
|---|---|---|
| rpc-pipeline.yml | 5,25,45 | Steps (5, pinned by `EXPECTED_STEPS`): fmv-recalc, fmv-backfill, backfill-player-names, backfill, price-snapshots. ⛔ **The `ingest` step (POST `/api/ingest`) was RETIRED 2026-09-07** (#67 (3)): its host `public-api.nbatopshot.com` has answered 530 since ~08-28 and the Atlas lane (`sync_sales_from_atlas`, jobid 471) writes those sales now; the route file stays, the step and its gate slot are gone. ⚠ The listing-cache step was REMOVED 2026-06-25; this row claimed it as a GHA-only backstop until 2026-09-02. `/api/topshot-listing-cache` is single-trigger (Vercel cron 15,35,55). |
...
- **2026-09-08 — three Top Shot GraphQL lanes REMOVED from `vercel.json`, schedule-only:** `/api/cron/ingest-topshot-challenges` (`10 8 * * *`), `/api/admin/backfill-topshot-subedition-circulation` (`10 21 * * *`), `/api/admin/drain-topshot-misattribution` (`0 11 * * *`). All three read `public-api.nbatopshot.com` (directly or via the topshot-proxy worker), which has answered 530 / CF 1033 since ~2026-08-28; measured over the full `pipeline_runs` window (inbox `2026-09-08T0530Z-five-lanes-…`): 3 of 3 runs each, 0 ok, 0 rows, `last_success` NULL. The filing assumed cron-job.org was the caller — it was this file's sibling, `vercel.json`. Routes, gates and route tests untouched; each route header carries an `UNSCHEDULED 2026-09-08` note; pinned by `__tests__/topshot-gql-dead-host-crons-are-retired.test.ts` (+ the inverted daily-drain assertion in `fossil-drain-schedule-is-retired.test.ts`). Re-add an entry only after a manual run writes rows, or re-point the lane to Atlas first. ⚠ NOT touched from the same filing: pg_cron jobid 15 `rpc-backfill-pack-supply` (daily 08:15) — the filing's condition for pausing it (name the path that keeps `pack_distributions` fresh — 1,995 rows updated in one shot at 12:13:09Z 09-08 with no coinciding `pipeline_runs` row and no repo caller for the three DB writers) is still unmet.

...
>
> ✅ **Verified as you asked:** `__tests__/api-error.test.ts` **25/25** · `tsc` **0** · `lint:ratchet` **715 vs baseline 715** · full `npm test` green. `boardUnavailable` → `apiErrorResponse` was confirmed by grep, so all **29** `/api/public/insights/*` routes inherit it. ✅ **YOUR POST-DEPLOY VERIFY STEP IS DONE — the outage was the forced backing failure you said to look for.** At 08:48 AM PT, cache-busted with `x-vercel-cache: MISS` on both: `/api/public/insights/squeeze` → **503 / `Retry-After: 30` / `{"code":"upstream_unavailable","retryable":true}`** (a 500 with `retryable:false` before the deploy), and your requested positive control holds — `/api/public/insights/pack-reality` still returns **503 `code:"timeout"`** through the pre-existing branch, so the new code did not swallow everything into one bucket. Deploy `dpl_24f5VuHkxCWjKtrqDL8SZKD9aTcD`, READY, apex aliases attached.
>

...
// ── On-chain LaLiga Golazos listings indexer (triple storefront scan) ────────
//
// Mirrors allday-listings-indexer's triple-scan design (see that file's
// header for the full architecture rationale). Drops the Sentry breadcrumbs +
// listing_resolution_failures retry queue + Cadence seller-borrow that AllDay
// carries — Golazos has lower listing churn so the lean shape is fine.
//
...
// V2 Flowty with source='direct'. Cancellation marking is source-scoped.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    try {
            ...
            for (const blk of blocks) {
              ...
              for (const evt of blk.events ?? []) {
                ...
                try {
                  ...
                  if (!nftTypeId || !nftTypeId.endsWith(GOLAZOS_NFT_TYPE_SUFFIX)) continue
      ...
      if (editionExternalIds.length > 0) {
        for (let i = 0; i < editionExternalIds.length; i += 500) {
          ...
          if (readErr2) throw new Error(`editions read failed: ${readErr2.message}`)

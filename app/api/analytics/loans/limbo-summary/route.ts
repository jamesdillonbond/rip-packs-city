...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[analytics/loans/limbo-summary] start collections=${collections?.join(",") ?? "all"}`
    )

    const { data, error } = await rpcWithRetry<AnalyticsLimboSummary>(
      supabaseAdmin,
      "flowty_analytics_limbo_summary",
      {
    ...
    console.log(`[analytics/loans/limbo-summary] ok elapsed=${Date.now() - t0}ms`)
    ...
    console.log("[analytics/loans/limbo-summary] error", e?.message || e, `elapsed=${Date.now() - t0}ms`)
    return NextResponse.json({ error: "limbo_summary_failed" }, { status: 500 })
  }
}

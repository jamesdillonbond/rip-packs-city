...
//
// Platform health summary. Phase 2: in addition to the legacy "ok/degraded"
// verdict, surface per-collection freshness from the Supabase health_check
// RPC so the overview page and profile ticker can render per-collection
// staleness without each page re-computing it.
//
// Response shape (back-compat preserved — everything old is still present):
// {
...
//     {
//       slug, name,
//       fmv_coverage_pct, fmv_last_computed,
//       sales_24h, last_sale_at, total_sales,
//       listing_count, listings_last_cached_at,
//       staleness_minutes (fmv age in minutes, rounded)
//     }
//   ],
...
export async function GET() {
  try {
    ...
    // Fold fmv_pipeline.per_collection + sales_pipeline.per_collection +
    // listing_cache.per_collection into one per-collection array.

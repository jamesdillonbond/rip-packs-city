...
// older than max_age_days. The route groups by wallet, runs one Cadence call
// per wallet over the wallet's full moment-id slice, and writes back through
// apply_lock_check_batch(p_results jsonb).
//
// Scope: NBA Top Shot only (uses TopShotLocking.isLocked). AllDay, Golazos,
// UFC, and Pinnacle do not have an analogous on-chain locking primitive yet —
...
// being lock-checked. When per-contract lock semantics land for those
// collections, add their slug to SUPPORTED and write a parallel Cadence path.
//
...
const SUPPORTED_SLUGS = new Set<string>(["nba_top_shot"])
...
async function runCadenceLock(wallet: string, ids: string[]): Promise<Record<string, boolean>> {
  const out: Record<string, boolean> = {}
  for (let i = 0; i < ids.length; i += PER_CADENCE_CHUNK) {
    const chunk = ids.slice(i, i + PER_CADENCE_CHUNK)
    const body = {
      script: btoa(TOPSHOT_LOCK_SCRIPT),
      arguments: [
...
export async function POST(req: NextRequest) {
  ...
  for (const [k, items] of groups.entries()) {
    ...
    try {
      const locked = await runCadenceLock(wallet, ids)
      for (const id of ids) {
        results.push({

...
async function run(startedAt: string, startedMs: number) {
  ...
  try {
    ...
    // Two bounded index range scans, not one `.or()` — the single-query form
    // full-scanned the 32,850-row tail every tick to find nothing once the
    // reattempt horizon went unreached. See lib/unmapped-rotating-window.ts.
    const { data: openData, error: openErr, armCounts } = await loadRotatingWindow(
      supabaseAdmin,
      {
        collectionId: ALLDAY_COLLECTION_ID,
        columns: "nft_id, transaction_hash, buyer_address, serial_number, block_height, sold_at",

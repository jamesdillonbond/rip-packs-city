// app/api/og/series/route.tsx
// Branded per-series OG card (2x2 montage). GET /api/og/series?collection=<slug>&slug=<seriesSlug>
...
export async function GET(req: NextRequest) {
  ...
  try {
    const [d, eds] = await Promise.all([
      sb.rpc("get_series_detail", { p_collection_id: coll.id, p_series_slug: slug }),
      sb.rpc("get_series_editions", { p_collection_id: coll.id, p_series_slug: slug, p_limit: 4, p_offset: 0 }),
    ])
  ...
  return renderEntityOg({
    eyebrow: `${label.toUpperCase()} · SERIES`,
    title: detail.display_label ?? "Series",
    subtitle: detail.season ? String(detail.season) : (edCount ? `${edCount.toLocaleString()} editions` : null),
    accent,
    images,

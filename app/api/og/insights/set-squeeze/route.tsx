...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    count = await fetchBoardCount(origin, "/api/public/insights/set-squeeze?sort=squeeze", 100)
    const r = await ogFetch(`${origin}/api/public/insights/set-squeeze?sort=squeeze&limit=3`, {
      cache: "no-store",
    })
    if (r.ok) {
      fetched = true
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          ...
          <div style={{ fontSize: 18, color: "rgba(255,255,255,0.55)", display: "flex" }}>
            {boardCountLabel(count, 'sets ranked')}
          </div>
        </div>
        ...
        <div style={{ marginTop: 34, display: "flex", flexDirection: "column", gap: 12 }}>
          {rows.length === 0 ? (
            <div style={{ fontSize: 22, color: "rgba(255,255,255,0.45)", display: "flex" }}>{boardEmptyCopy(fetched, "board")}</div>
          ) : (

// app/api/og/insights/allday-pack-reality/route.tsx
//
// Open Graph card for /insights/allday-pack-reality. 1200x630 PNG via next/og.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const r = await ogFetch(`${origin}/api/public/insights/allday-pack-reality`, { cache: "no-store" })
    if (r.ok) {
      const j = await r.json()
      qualifying = j?.model_vs_reality?.qualifying_dists ?? null
    }
  } catch {
    ...
    (
      ...
      >
        ...
        <div style={{ marginTop: 12, fontSize: 24, color: "rgba(255,255,255,0.68)", letterSpacing: 0.5, lineHeight: 1.35, display: "flex", maxWidth: 1040 }}>
          What the model says vs what packs actually pull — modeled EV against the value real opens delivered, resolved on-chain.
        </div>

        <div style={{ marginTop: 48, display: "flex", gap: 24 }}>
          {[
            { label: "MODELED EV", value: "vs" },
            { label: "REALIZED PULLS", value: "on-chain" },
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 18, color: "rgba(255,255,255,0.55)" }}>
          <div style={{ display: "flex" }}>Honest model-vs-reality check · stale-FMV dists excluded</div>
          <div style={{ display: "flex" }}>rippackscity.com/insights/allday-pack-reality</div>
        </div>
      </div>
    ),

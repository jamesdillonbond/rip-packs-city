...
export async function GET(req: Request) {
  ...
  const { data, error } = await boundedRead(supa.rpc("get_player_editions", {
    p_collection_id: coll.id,
    p_player_slug: playerSlug,
    p_limit: limit,
    p_offset: offset,
  }), "api/entity/player/get_player_editions")
  if (error) return apiErrorResponse(error, "api/entity/player")

...
export async function GET(req: Request) {
  ...
  const { data, error } = await boundedRead(supa.rpc("get_series_editions", {
    p_collection_id: coll.id,
    p_series_slug: seriesSlug,
    p_limit: limit,
    p_offset: offset,
  }), "api/entity/series/get_series_editions")
  if (error) return apiErrorResponse(error, "api/entity/series")

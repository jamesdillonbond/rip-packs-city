// app/api/public/insights/topshot-pack-market/route.ts
//
// PUBLIC INSIGHTS — NBA Top Shot Pack Market (sealed-pack secondary resale).
//
// Read-only JSON endpoint backing /insights/topshot-pack-market. Lives under
// /api/public/* so the proxy.ts allowlist lets it through with no auth.
//
// Reads the public view v_topshot_pack_market — the complete sealed-pack
// secondary sale history (Dapper Studio Platform, backfilling to Top Shot's 2020
...
// require a real retail price (premium packs have retail NULL → null ratio, so
// they only appear in the most-traded bucket).
//
...
export async function GET(_req: NextRequest) {
  ...
  if (error) {
    return boardUnavailable(error, "topshot-pack-market/market");
  }
  ...
  const rows: MarketRow[] = ((data ?? []) as MarketRow[]).map((r) => ({
    ...
    depletion_pct: num(r.depletion_pct),

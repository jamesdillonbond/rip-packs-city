      ...
      if (missingExternalIds.length > 0 && !dryRun) {
        ...
        for (const r of hydratedRows) {
          if (r.redirect) {
            ...
            // map int-pair external_id → canonical id and skip upsert.
        ...
        console.log(
          `[flowty-listings] hydrate-at-insert: candidates=${candidates} hydrated=${hydratedCount} redirected=${redirectedCount} fallback=${fallbackCount}` +

...
function Ticker() {
  const items = [
    ...
    "PROFILE — saved wallets · dashboards · search history",
  ];
  ...
  return (
    <div style={{ background: "#0D0D0D", borderBottom: "1px solid rgba(224,58,47,0.2)", overflow: "hidden", height: 28, display: "flex", alignItems: "center" }}>
      <div style={{ background: "#E03A2F", padding: "0 12px", fontSize: 9, fontFamily: monoFont, letterSpacing: "0.15em", color: "#fff", height: "100%", display: "flex", alignItems: "center", flexShrink: 0, fontWeight: 700 }}>
        LIVE
      </div>
      <div style={{ overflow: "hidden", flex: 1 }}>
        <div style={{ display: "flex", gap: 64, animation: "ticker 38s linear infinite", whiteSpace: "nowrap", paddingLeft: 24 }}>
          {doubled.map(function(item, i) {
            return (
              <span key={i} style={{ fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.45)", letterSpacing: "0.07em" }}>
                {"⚡ " + item}
              </span>
            );
          })}
...
function StatTile(props: { label: string; value: string; sub: string; change: string; up: boolean; icon: string; color: string; delay: number }) {
  ...
  return (
    <div style={tileStyle}>
      ...
      <div style={{ fontSize: 24, fontFamily: condensedFont, fontWeight: 800, color: "#fff", letterSpacing: "0.02em", lineHeight: 1, marginBottom: 6 }}>
        {props.value}
      </div>
...
function WalletCard(props: { wallet: SavedWallet; onLoad: (addr: string, user?: string) => void; onRemove: (addr: string) => void }) {
  ...
  const cardStyle: React.CSSProperties = {
    ...
    border: "1px solid " + (hovered ? w.accent_color + "55" : "rgba(255,255,255,0.07)"),
    borderRadius: 10,
    padding: "14px 16px",
    cursor: "pointer",
    transition: "all 0.2s",
  ...
  const hasFmv = w.cached_fmv != null;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: "50%", background: w.accent_color + "22", border: "1px solid " + w.accent_color + "44", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: w.accent_color, fontFamily: condensedFont, flexShrink: 0 }}>
          {initials}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: condensedFont, fontWeight: 700, fontSize: 14, color: "#fff", letterSpacing: "0.04em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {label}
          </div>
          <div style={{ fontSize: 9, fontFamily: monoFont, color: "rgba(255,255,255,0.3)" }}>
            {w.wallet_addr.slice(0, 14) + "…"}
          </div>
        </div>
        {(w.cached_badges ?? []).slice(0, 3).map(function(b, i) {
          return <span key={i} style={{ fontSize: 11 }}>{b}</span>;
        })}
      </div>
      ...
      {hasFmv ? (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 8, fontFamily: monoFont, color: "rgba(255,255,255,0.3)", letterSpacing: "0.12em", marginBottom: 2 }}>FMV</div>
            <div style={{ fontSize: 13, fontFamily: condensedFont, fontWeight: 700, color: "#fff" }}>{fmtDollars(w.cached_fmv ?? 0)}</div>
          </div>
          <div>
            <div style={{ fontSize: 8, fontFamily: monoFont, color: "rgba(255,255,255,0.3)", letterSpacing: "0.12em", marginBottom: 2 }}>MOMENTS</div>
            <div style={{ fontSize: 13, fontFamily: condensedFont, fontWeight: 700, color: "#fff" }}>{w.cached_moment_count ?? "—"}</div>
          </div>
          <div>
            <div style={{ fontSize: 8, fontFamily: monoFont, color: "rgba(255,255,255,0.3)", letterSpacing: "0.12em", marginBottom: 2 }}>24H</div>
            <div style={{ fontSize: 13, fontFamily: condensedFont, fontWeight: 700, color: changeColor }}>{changeStr}</div>
          </div>
        </div>
      ) : (
        <div style={{ fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.25)", marginBottom: 10 }}>
          Load wallet to populate stats
        </div>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <span style={{ fontSize: 9, fontFamily: monoFont, color: "rgba(255,255,255,0.25)" }}>
          {w.last_viewed ? "Viewed " + relTime(w.last_viewed) : "Never loaded"}
        </span>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {confirm ? (
            <>
              <button
                onClick={function(e) { e.stopPropagation(); setConfirm(false); }}
                style={Object.assign({}, btnBase, { fontSize: 9 })}
              >
                Cancel
              </button>
              <button
                onClick={function(e) { e.stopPropagation(); props.onRemove(w.wallet_addr); }}
                style={Object.assign({}, btnBase, { background: "rgba(239,68,68,0.15)", color: "#F87171", borderColor: "rgba(239,68,68,0.35)", fontSize: 9 })}
              >
                Remove
              </button>
            </>
            ...
            <>
              <button
                onClick={function(e) { e.stopPropagation(); setConfirm(true); }}
                style={Object.assign({}, btnBase, { background: "transparent", border: "none", opacity: hovered ? 0.5 : 0, transition: "opacity 0.15s", fontSize: 9 })}
              >
                ✕
              </button>
              <button
                onClick={function(e) { e.stopPropagation(); props.onLoad(w.wallet_addr, w.username ?? undefined); }}
                style={Object.assign({}, btnBase, { background: w.accent_color + "22", color: w.accent_color, borderColor: w.accent_color + "44", opacity: hovered ? 1 : 0.6, fontSize: 9 })}
              >
                {"Load Wallet →"}
              </button>
            </>
  ...
  return (
    <div style={{ display: "flex", gap: 8, marginBottom: 12, animation: "fadeIn 0.2s ease both" }}>
      <input
        ...
        onKeyDown={function(e) { if (e.key === "Enter" && val.trim()) { props.onAdd(val.trim()); } }}
        placeholder="Username or 0x address…"
        style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(224,58,47,0.35)", borderRadius: 6, padding: "7px 12px", color: "#fff", fontFamily: monoFont, fontSize: 11, outline: "none", letterSpacing: "0.04em" }}
      />
      <button
        onClick={function() { if (val.trim()) props.onAdd(val.trim()); }}
        style={Object.assign({}, btnBase, { background: "#E03A2F", color: "#fff", borderColor: "#E03A2F", fontSize: 11 })}
      >
        Save
      </button>
...
export default function ProfilePage() {
  ...
  // ── Load ownerKey from localStorage on mount ──────────────
  useEffect(function() {
    try {
  ...
  const loadProfile = useCallback(async function(key: string) {
    ...
    try {
      ...
      if (wRes.ok) {
        ...
        setSavedWallets(d.wallets ?? []);
      }
      if (sRes.ok) {
        const d = await sRes.json();
        setRecentSearches(d.searches ?? []);
      }
    } catch (err) {
  ...
  useEffect(function() {
    ...
    fetch("/api/sniper-feed?limit=5")
      ...
      .then(function(data) {
        if (!data) return;
        // sniper-feed returns { deals: [...] } based on the route built in the sniper thread
        ...
        const rows: SniperRow[] = raw.slice(0, 5).map(function(r: any) {
          ...
          return {
            player: r.playerName ?? r.player ?? "Unknown",
            set: r.setName ?? r.set ?? "",
            serial: r.serialNumber ? "#" + r.serialNumber : "",
            price,
            fmv,
            pct,
            tier: r.tier ?? r.rarity ?? "Common",
          };
        });
        setSniperRows(rows);
      })
  ...
  // ── Record search to Supabase ─────────────────────────────
  function recordSearch(query: string, queryType?: string) {
    if (!ownerKey || !query.trim()) return;
    fetch("/api/profile/recent-searches", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ownerKey, query: query.trim(), queryType }),
    })
      .then(function(r) { return r.ok ? r.json() : null; })
      .then(function(d) {
        if (!d?.search) return;
        setRecentSearches(function(prev) {
          const filtered = prev.filter(function(s) { return s.query !== query.trim(); });
          return [d.search, ...filtered].slice(0, 20);
        });
      })
      .catch(function() {});
  }
  ...
  async function handleAddWallet(input: string) {
    if (!ownerKey) {
      alert("Set your Profile Key below first, then add wallets.");
      return;
    }
    let walletAddr = input;
    let username: string | undefined;
    if (!input.startsWith("0x")) {
      // Resolve username → address via existing user-resolve route
      try {
        const r = await fetch("/api/user-resolve?username=" + encodeURIComponent(input));
        if (r.ok) {
          const d = await r.json();
          if (d.address) { walletAddr = d.address; username = input; }
        }
      } catch {}
    }
    const accentColor = pickAccent(savedWallets.length);
    const r = await fetch("/api/profile/saved-wallets", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ownerKey, walletAddr, username, accentColor }),
    });
    if (!r.ok) return;
    const d = await r.json();
    if (d.wallet) {
      setSavedWallets(function(prev) {
        const filtered = prev.filter(function(w) { return w.wallet_addr !== walletAddr; });
        return [d.wallet, ...filtered];
      });
    }
  ...
  async function handleRemoveWallet(walletAddr: string) {
    ...
    await fetch("/api/profile/saved-wallets", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ownerKey, walletAddr }),
    });
    setSavedWallets(function(prev) {
      return prev.filter(function(w) { return w.wallet_addr !== walletAddr; });
    });
  }

  // ── Load wallet from card ─────────────────────────────────
  ...
  // ── Analytics tile data ───────────────────────────────────
  const totalFmv = useMemo(function() {
    return savedWallets.reduce(function(sum, w) { return sum + (w.cached_fmv ?? 0); }, 0);
  }, [savedWallets]);
  ...
  const tiles = [
    ...
    { label: "Live Deals", value: sniperLoading ? "…" : (sniperRows.length + " below FMV"), sub: "Sniper feed preview", change: "Live", up: true, icon: "⚡", color: "#F59E0B" },
    { label: "Recent Searches", value: String(recentSearches.length), sub: "saved queries", change: "Synced", up: true, icon: "⌕", color: "#3B82F6" },
    { label: "Searches Today", value: String(recentSearches.filter(function(s) { return Date.now() - new Date(s.searched_at).getTime() < 86400000; }).length), sub: "last 24 hours", change: "Active", up: true, icon: "◎", color: "#10B981" },
  ];
        ...
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 24px", height: 56, display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ flex: 1, position: "relative", maxWidth: 480 }}>
            <input
              ...
              onKeyDown={function(e) { if (e.key === "Enter" && heroSearch.trim()) handleSearch(heroSearch); }}
              placeholder="Search wallet, player, edition…"
              style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "7px 34px 7px 14px", color: "#fff", fontFamily: monoFont, fontSize: 11, outline: "none", letterSpacing: "0.04em" }}
              onFocus={function(e) { e.target.style.borderColor = "rgba(224,58,47,0.5)"; }}
            ...
            {navItems.map(function(item) {
              ...
              return (
                <button
                  ...
                  style={{
                    background: active ? "rgba(224,58,47,0.15)" : "transparent",
                    border: active ? "1px solid rgba(224,58,47,0.4)" : "1px solid transparent",
                    color: active ? "#E03A2F" : "rgba(255,255,255,0.5)",
                    padding: "4px 10px",
                    borderRadius: 4,
                    fontSize: 10,
                    fontFamily: condensedFont,
        ...
        <section style={{ marginBottom: 28, animation: "fadeIn 0.45s ease both", textAlign: "center" }}>
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            ...
            <h1 style={{ fontFamily: condensedFont, fontWeight: 900, fontSize: 36, letterSpacing: "0.04em", color: "#fff", textTransform: "uppercase", lineHeight: 1, marginBottom: 16 }}>
              {"Your "}
              <span style={{ color: "#E03A2F" }}>Profile</span>
            </h1>
            <div style={{ display: "flex", gap: 8, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 10, padding: "8px 8px 8px 16px", alignItems: "center" }}>
              <input
                ...
                onChange={function(e) { setHeroSearch(e.target.value); }}
                onKeyDown={function(e) { if (e.key === "Enter" && heroSearch.trim()) handleSearch(heroSearch); }}
                placeholder="Enter any Top Shot username or wallet address…"
                style={{ flex: 1, background: "transparent", border: "none", color: "#fff", fontFamily: monoFont, fontSize: 12, outline: "none", letterSpacing: "0.04em" }}
              />
              <button
                onClick={function() { if (heroSearch.trim()) handleSearch(heroSearch); }}
                style={{ background: "#E03A2F", border: "none", borderRadius: 7, padding: "8px 20px", color: "#fff", fontFamily: condensedFont, fontWeight: 800, fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", flexShrink: 0 }}
        ...
        <section style={{ marginBottom: 22 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
            {tiles.map(function(t, i) {
              return <StatTile key={t.label} label={t.label} value={t.value} sub={t.sub} change={t.change} up={t.up} icon={t.icon} color={t.color} delay={i * 70} />;
            })}
          </div>
        </section>
          ...
          <section>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ background: "rgba(224,58,47,0.15)", border: "1px solid rgba(224,58,47,0.3)", color: "#E03A2F", fontSize: 9, fontFamily: monoFont, padding: "1px 6px", borderRadius: 3 }}>
                  {savedWallets.length}
                </span>
              </div>
              <button
                onClick={function() { setShowAddWallet(function(v) { return !v; }); }}
                style={Object.assign({}, btnBase, showAddWallet ? { background: "rgba(224,58,47,0.15)", color: "#E03A2F", borderColor: "rgba(224,58,47,0.4)" } : {})}
              >
            ...
            {showAddWallet && (
              <AddWalletForm
                ...
                onCancel={function() { setShowAddWallet(false); }}
              />
            )}
            ...
            {profileLoading ? (
              ...
              <div style={{ fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.25)", textAlign: "center", padding: "28px 16px", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 8, lineHeight: 1.7 }}>
                No saved wallets yet.<br />Click + Add to pin a wallet here.
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {savedWallets.map(function(w) {
                  return (
                    <WalletCard
                      key={w.id}
                      wallet={w}
                      onLoad={handleLoadWallet}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <section style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", flex: 1 }}>
              <div style={{ padding: "12px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button
                  onClick={function() { router.push("/sniper"); }}
                  style={Object.assign({}, btnBase, { background: "rgba(52,211,153,0.1)", color: "#34D399", borderColor: "rgba(52,211,153,0.25)", fontSize: 9 })}
                >
                  {"Full Sniper →"}
                </button>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 60px 60px 62px", padding: "7px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                {["Moment", "Tier", "Ask", "FMV", "Disc%"].map(function(h, i) {
                  return (
                    <span key={h} style={{ fontSize: 8, fontFamily: monoFont, color: "rgba(255,255,255,0.25)", letterSpacing: "0.15em", textAlign: i === 0 ? "left" : "right" }}>
                      {h}
                    </span>
                  );
                })}
              ...
              {sniperLoading ? (
                ...
                <div style={{ padding: "24px", fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.25)", textAlign: "center" }}>
                  No deals loaded. Visit the{" "}
                  <span style={{ color: "#34D399", cursor: "pointer" }} onClick={function() { router.push("/sniper"); }}>Sniper page</span>
                  {" "}to refresh.
                </div>
                ...
                return (
                  <div
                    key={i}
                    style={{ display: "grid", gridTemplateColumns: "1fr 80px 60px 60px 62px", alignItems: "center", padding: "9px 14px", borderBottom: "1px solid rgba(255,255,255,0.04)", cursor: "pointer", transition: "background 0.15s" }}
                  ...
                  >
                    ...
                    <span style={{ fontSize: 8, fontFamily: monoFont, color: tc, background: tc + "18", border: "1px solid " + tc + "33", padding: "2px 5px", borderRadius: 3, textAlign: "center", letterSpacing: "0.05em" }}>
            ...
            <section style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 10 }}>
                {recentSearches.length === 0 ? (
                  <span style={{ fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.25)" }}>
                    {ownerKey ? "No searches yet" : "Set your Profile Key to track searches"}
                  </span>
                  ...
                  return (
                    <button
                      key={i}
                      onClick={function() { handleSearch(s.query); }}
                      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 5, padding: "5px 10px", color: "rgba(255,255,255,0.65)", fontFamily: monoFont, fontSize: 10, cursor: "pointer", display: "flex", alignItems: "center", gap: 7, letterSpacing: "0.04em", transition: "all 0.15s" }}
                      onMouseEnter={function(e) {
                        e.currentTarget.style.background = "rgba(224,58,47,0.1)";
                        e.currentTarget.style.borderColor = "rgba(224,58,47,0.3)";
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10 }}>
            {quickLinks.map(function(link) {
              return (
                <button
                  key={link.label}
                  onClick={function() { router.push(link.href); }}
                  style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: "14px 16px", textAlign: "left", cursor: "pointer", transition: "all 0.2s", position: "relative", overflow: "hidden" }}
                  onMouseEnter={function(e) {
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 10, fontFamily: monoFont, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>
                {ownerKey ? ("Active: " + ownerKey) : "Set your Top Shot username to save wallets and search history across sessions."}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input
                ...
                onKeyDown={function(e) { if (e.key === "Enter" && ownerInput.trim()) { setOwnerKey(ownerInput.trim()); loadProfile(ownerInput.trim()); } }}
                placeholder="your username…"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, padding: "6px 12px", color: "#fff", fontFamily: monoFont, fontSize: 11, outline: "none", width: 200 }}
              />
              <button
                onClick={function() { if (ownerInput.trim()) { setOwnerKey(ownerInput.trim()); loadProfile(ownerInput.trim()); } }}
                style={Object.assign({}, btnBase, { background: "#E03A2F", color: "#fff", borderColor: "#E03A2F" })}
              >

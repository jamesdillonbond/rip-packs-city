...
//
// Fire-and-forget: returns 200 immediately so cron-job.org's 30s HTTP
// timeout doesn't mark every run as failed. The drain loop continues in
// the background via next/server `after()` and surfaces progress through
// Vercel runtime logs. Each cron tick picks up wherever the last one left
// off (drain RPC self-tracks progress in wmc_dedup_pairs.processed).

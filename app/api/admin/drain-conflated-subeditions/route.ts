...
async function handle(req: NextRequest): Promise<NextResponse> {
  ...
  const ok = !out.fatal && !out.seed_error && !out.catalog_error && !out.split_error && !out.guard_error;
  ...
  try {
    await sb.from("pipeline_runs").insert({
      ...
      rows_skipped: 0,
      ok,
      error: (out.fatal ?? out.seed_error ?? out.catalog_error ?? out.split_error ?? out.guard_error ?? null) as string | null,
      extra: out,
    });
  } catch { /* best-effort */ }

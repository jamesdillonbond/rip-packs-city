...
function mapNftToDeal(nft: any): PinnacleSniperDeal | null {
  try {
    ...
    const variant = parseVariant(stripBrackets(parseTrait(traits, "Variant")));
    const franchise = parseFranchise(stripBrackets(parseTrait(traits, "Studios")));
    const characterRaw = parseTrait(traits, "Characters");
    const characterName =
      parseCharacters(characterRaw) ||
      nft.card?.title?.split("[")[0]?.trim() ||
      "Unknown";
    const setName = stripBrackets(parseTrait(traits, "SetName")).trim();
    const seriesYear =
...
export async function GET(req: Request) {
  ...
  // Compute price range for debugging
  const prices = deals.map((d) => d.askPrice);
  const minPrice = prices.length > 0 ? Math.min(...prices) : 0;
  const maxPriceVal = prices.length > 0 ? Math.max(...prices) : 0;

  console.log(
    `[pinnacle-sniper] ${deals.length} deals, price range $${minPrice.toFixed(2)} – $${maxPriceVal.toFixed(2)}`
  );
  ...
  return NextResponse.json(
    {
      ...
      priceRange: { min: minPrice, max: maxPriceVal },

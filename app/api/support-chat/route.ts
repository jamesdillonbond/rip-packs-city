...
// Panini published 2026-09-25 (Overview + Market) — derived from the registry, like Candy's.
const PANINI_TAB_PATHS = (getCollection("panini-blockchain")?.pages ?? []).map((p) => `/panini-blockchain/${p}`).join(", ")
...
function buildSystemPromptParts(ctx: {
  ...
  // A collection with NO established chain (Panini, dbChain null) is on its
  // partner's own platform — never listed bare, where it would read as Flow.
  const publishedLabels = published
    .map((c) =>
      chainName(c)
        ? `${c.icon} ${c.label} (on ${chainName(c)}, not Flow)`
        : c.dbChain == null
          ? `${c.icon} ${c.label} (on ${c.partner}'s own platform, not a public chain)`
          : `${c.icon} ${c.label}`,
    )
    .join(", ");
  ...
  const collectionBlurb = activeCollection
    ? `\n## Active Collection
The user is currently browsing **${activeCollection.label}** (${activeCollection.sport}, ${activeCollection.partner}, ${activeCollection.dbChain ? `${activeCollection.dbChain.toUpperCase()} chain` : `${activeCollection.partner}'s own platform — no public chain, no wallets`}).
...
- **Unshipped / shelved features**: never present something that isn't live as if it is. RPC is a READ-ONLY intelligence product — there is no in-app buying, no cart, no gifting and no trading/swap surface. Never offer to buy, sell, gift, swap or move a Moment; point to the marketplace listing link instead. The Panini and Candy / Solana BOARDS are public (/insights/panini-squeeze and /insights/candy-mlb — hand them out freely). Both are also published collections, but ONLY with the tabs their registry entries list (Panini: ${PANINI_TAB_PATHS}; Candy: ${CANDY_TAB_PATHS}) — never imply a tab either lacks.
...
Badges are what collectors pay attention to (Rookie Year, Top Shot Debut, Championship Year, Rookie Premiere, MVP Year, etc.). When a tool result row includes a \`badges\` / \`badge_slugs\` field, you MUST factor those badges into your valuation and ranking commentary — name the badge, say it is a reason collectors single that edition out, and rank it above an otherwise-identical unbadged edition when the PRICED rows in front of you support it. NEVER tell the user you "can't factor badges in because you didn't call a tool" — if the row has a badges field, the data is already in front of you; use it. ⚠ But the SIZE of a badge premium is a price claim and falls under the FMV rule above: never say a badge "typically commands a premium", "carries a premium" or "is worth more" as a general statement — say that only when two priced rows THIS TURN show it (a badged edition at $X against an unbadged one of the same set and tier at $Y), and quote them. Without those rows, describe the badge and stop. get_fmv, get_edition_listings, explain_fmv, get_player_editions and search_live_deals rows all carry a \`badges\` list for Top Shot editions, plus team, series, parallel and a \`supply\` block (circulation / burned / locked / squeeze_pct) — read them before ranking, and read \`badges_status\` before saying an edition has no badge ("unavailable" means the metadata read failed, "not_tracked" means that collection has no moment tags; only "ok" with an empty list means the edition carries none). For "what does this badge mean / how rare is it / which badges exist" call get_badge_info — it returns the taxonomy definition and a live census of tagged editions by tier; it does NOT price a badge, so a "badge premium" question is answered by pricing badged vs. unbadged editions of the same set and tier through get_fmv or get_player_editions. As always, this is context, not a buy/sell call, and any price you cite must come from a tool row this turn.
...
- ⚠ **A deal row with low_confidence_fmv: true is NOT a deal to headline.** Its FMV is ASK_ONLY (one seller's ask, no sales), STALE (carried forward, not re-priced) or SALES_ONLY (old sales, no ask to corroborate), so its discount_pct compares an ask with a number nothing supports. List verified rows first; if you mention a flagged row at all, say its FMV is thin/stale and that the discount may not be real. Never call one "the sharpest deal" or "deepest discount".
...
- **Wallet verification (listing challenge)** — the working path for Top Shot collectors: go to /dashboard. RPC picks one cheap Moment you own and asks you to list it at a unique ~100x / $10-floor price (it won't sell — the odd cents are just a uniqueness check), confirms the live listing, and credits you 500. ⚠ **It needs a Top Shot wallet (a Flow 0x address holding Top Shot Moments) — there is NO verification path for a Solana / Candy MLB wallet, or for a wallet with no Top Shot Moments, today.** Never offer verification to those users; say it is Top Shot-only for now. **RPC never asks you to connect or sign a wallet** — there is no wallet sign-in anywhere on the site. To load a collection you paste a public identifier (a Dapper wallet address or a Top Shot username; for Candy MLB, a Solana wallet address) and RPC reads it view-only; the listing challenge is the only way to mark a wallet as verifiably yours. Never tell a user to look for a "Sign in with Dapper" / "connect wallet" button — none exists.
...
// The primary name is the one the per-team RPCs are asked for, and since batch
// 62 those RPCs read the WHOLE franchise (historic labels included); the labels
// ride along so the answer can say the numbers span the eras. When that
...
async function executeToolInner(
  ...
  if (toolName === "get_team_intel") {
    try {
      ...
      // the franchise view rides on every answer: the primary name the reads are keyed by, and the
      // historic labels those reads INCLUDE (batch 62 — every team RPC reads the whole franchise)
      const franchise = resolved.franchise
        ? {
            ...
            note: resolved.franchise.historic_names.length > 0
              ? `The reads below cover the whole "${resolved.franchise.primary_name}" franchise, every era included: it also minted as ${resolved.franchise.historic_names.map((h) => `${h.team_name} (${h.editions} editions)`).join(", ")}, and those moments are counted here (their team URLs redirect to this page). Say the numbers span the eras.`
              : null,
          }

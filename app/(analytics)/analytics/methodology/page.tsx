  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[color:var(--rpc-text-primary)] tracking-tight">Methodology</h1>
          <p className="text-sm text-[color:var(--rpc-text-secondary)]">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {METHODOLOGY_LIST.map((m) => (
          <Link
            ...
            className="group rounded-xl border border-[color:var(--rpc-border)] bg-[color:var(--rpc-surface-raised)] p-5 transition-all hover:border-emerald-500/40 hover:bg-[color:var(--rpc-surface-hover)]"
          >
            <div className="flex items-start justify-between mb-2">
              <h2 className="font-semibold text-[color:var(--rpc-text-primary)]">{m.title}</h2>
              <ArrowUpRight
                size={14}
                className="text-[color:var(--rpc-text-ghost)] group-hover:text-emerald-400 transition-colors mt-1"
              />
            </div>
            <p className="text-sm text-[color:var(--rpc-text-secondary)] leading-relaxed mb-3">{m.blurb}</p>

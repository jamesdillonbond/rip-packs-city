...
// Minimal, dependency-free branded card. Renders even when the collection
// snapshot fetch fails or the data-rich card throws — guarantees a /share link
// never unfurls a blank box.
function fallbackCard() {
  return new ImageResponse(
    (
      ...
      >
        <div style={{ fontSize: 64, fontWeight: 900, color: "#E03A2F", letterSpacing: "0.08em" }}>
          RIP PACKS CITY
        </div>
        <div style={{ fontSize: 24, color: "#888", letterSpacing: "0.12em", textTransform: "uppercase" }}>
          Flow Collectibles Intelligence
        </div>
      </div>
    ),
    { ...size }
...
export default async function Image(props: { params: Promise<{ wallet: string }> }) {
  ...
  try {
    ...
    console.log(`[share-og] snapshot fetch failed for ${wallet}:`, err instanceof Error ? err.message : err)
  }
  ...
  try {
    ...
    console.log(`[share-og] ImageResponse threw for ${wallet}:`, err instanceof Error ? (err.stack ?? err.message) : err)
    return fallbackCard()
  }
}

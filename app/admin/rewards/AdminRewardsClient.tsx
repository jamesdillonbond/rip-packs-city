      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        {loading ? (
          ...
          <>
            ...
            >
              {/* ⚠ `num()` coerces an absent value to 0, so a failed read used to publish
                * "Outstanding liability 0" and "Pending redemptions 0" — telling an operator
                * the programme owes nothing and there is nothing to fulfil, from our own
                * outage, in the same 8-tile band that carries the real figures.
                *   Fixed HERE rather than inside `num()` deliberately: every other call site
                * takes a value off a row that DID load, and one of them interpolates into a
                * confirm dialog ("Refund 500 credits and cancel?"), where an em-dash would be
                * worse than a zero. The band is WITHHELD, not em-dashed, because eight tiles
                * of dashes reads as "every figure is genuinely absent" rather than "we could
                * not ask" — and a genuine zero still renders as 0. */}
              {economy == null ? (
                <div
                  style={{ gridColumn: "1 / -1", fontFamily: MONO, color: "#d6a13a", fontSize: 12 }}
                >
                  Couldn&apos;t load the economy figures — no liability or redemption counts
                  below are available. Retry before drawing any conclusion from this screen.
                </div>
              ) : (
                <>

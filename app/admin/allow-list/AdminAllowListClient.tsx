...
export default function AdminAllowListClient() {
  // ⚠ The Suspense boundary stays HERE, not on the server page. `useSearchParams` is called
  // deep inside `AdminAllowListInner`, so the boundary has to wrap that subtree; hoisting it
  // to the server shell would move the file into the gate without making the inner component
  // renderable by a test.
  return (
  ...
  const decide = useCallback(
    async (id: string, action: "approve" | "hold" | "deny" | "reset", reason: string | null) => {
      ...
      try {
        ...
        const data = await res.json();
        const next = data.row as AllowRow | undefined;
        // ⚠ A 200 that carries no `row` used to be written straight into state, replacing
        // the row with `undefined` — the very next render then crashed on `r.id` and took
        // the WHOLE console down, after an action that had just reported success. An
        // operator is left with a white screen and no way to tell whether the approval
        // landed, on the screen that gates who gets into the product.
        //   Falling back to a full refetch is the honest recovery: it shows the row's REAL
        // state rather than an optimistic guess about a response we did not understand.
    ...
    // fetchRows is a dependency now: the no-`row` recovery path refetches.
      ...
      >
        ...
        <section style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {loading && rows.length === 0 ? (
            ...
            /* ⚠ Gated on `error`, not only on emptiness. The loader returns early on a
             * failure, so `rows` stays at its initial `[]` and this rendered "Nothing in
             * this view." — telling an operator there are no signups waiting to be
             * approved, produced entirely by our own outage, on the one screen that gates
             * who gets into the product. The error banner renders above it, so the page
             * contradicted itself; same shape as the listing-retry-queue and
             * /insights/pack-reality cases.
             *   Deliberately `rows.length === 0` and not just `error`: a failed REFRESH
             * keeps the previous rows, and last-good beats a blank queue — it is only the
             * EMPTY case that must stop claiming the queue is clear. */
            <div
              style={{
                fontFamily: monoFont,
                fontSize: 12,
            ...
            >
              Couldn&apos;t load the allow list — this says nothing about who is waiting.
            </div>

...
export default function ListingRetryQueueClient() {
      ...
      <div style={{ overflowX: "auto", background: "var(--rpc-surface)", border: "1px solid var(--rpc-border)", borderRadius: 8 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "var(--font-mono)", fontSize: 11 }}>
          ...
          <tbody>
            ...
            {/* ⚠ Gated on `error`, not only on emptiness. `fetchRows` returns early on a
              * failure, leaving `rows` at its previous value — `[]` on a first load — so
              * without this branch a failed read rendered "No rows for this filter": the
              * retry queue reported as CLEAR, produced by our own outage, on the one screen
              * an operator uses to decide whether the drain is working. The error banner
              * above it did not help; the page simply contradicted itself, which is the same
              * shape as the /insights/pack-reality defect (one site consulted `error`, the
              * claim sites did not).
              *   A failed REFRESH still shows the previous rows beneath the banner —
              * last-good beats a blank operations board — but they are no longer a claim
              * that the queue is empty. */}
            {!loading && !error && sortedRows.length === 0 && (
              <tr><Td colSpan={9}><span style={muted}>No rows for this filter</span></Td></tr>
            )}
            {!loading && error && sortedRows.length === 0 && (
              <tr><Td colSpan={9}><span style={muted}>Could not load rows — the queue may or may not be empty.</span></Td></tr>
            )}
            {sortedRows.map((r) => {

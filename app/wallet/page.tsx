...
// ── Auto-search reader (isolated for Suspense) ────────────────────────────────
// Reads ?q= from URL and fires onSearch callback once on mount.
...
export default function WalletPage() {
  ...
  async function enrichWithBadges(rowsIn: MomentRow[]): Promise<MomentRow[]> {
    ...
    const playerNames = Array.from(new Set(rowsIn.map(r => r.playerName).filter(Boolean)))
  ...
  // ── Core search logic, extracted so AutoSearchReader can call it ──────────
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100">
      {/* AutoSearchReader isolated in Suspense — reads ?q= and triggers search */}
      ...
      <div className="mx-auto max-w-[1600px] px-3 py-4 md:px-6">
        ...
        {hasSearched && rows.length > 0 && (
          <div className="mb-5 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <div
                className={"cursor-pointer rounded-xl border p-3 transition " + (badgeFilter ? "border-red-600 bg-red-950/30" : "border-zinc-800 bg-zinc-950 hover:border-zinc-600")}
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          <select value={teamFilter} onChange={(e) => setTeamFilter(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white">
            {availableTeams.map((team) => <option key={team} value={team}>{team === "all" ? "All Teams" : team}</option>)}
          </select>
          <select value={leagueFilter} onChange={(e) => setLeagueFilter(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white">
            {availableLeagues.map((league) => <option key={league} value={league}>{league === "all" ? "All Leagues" : league}</option>)}
          </select>
          <select value={rarityFilter} onChange={(e) => setRarityFilter(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white">
            {availableRarities.map((tier) => <option key={tier} value={tier}>{tier === "all" ? "All Rarities" : tier}</option>)}
          </select>
          <select value={parallelFilter} onChange={(e) => setParallelFilter(e.target.value)} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white">
            {availableParallels.map((parallel) => <option key={parallel} value={parallel}>{parallel === "all" ? "All Parallels" : parallel}</option>)}
          </select>
          ...
          <input value={searchWithin} onChange={(e) => setSearchWithin(e.target.value)} placeholder="Filter loaded moments" className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white placeholder:text-zinc-500" />
        </div>

        {/* Sort buttons */}
        <div className="mb-4 flex flex-wrap gap-2">
          {([["player","Player"],["series","Series"],["set","Set"],["parallel","Parallel"],["rarity","Rarity"],["serial","Serial"],["held","Held"],["fmv","FMV"],["bestOffer","Best Offer"],["badge","Badge Score"]] as [SortKey, string][]).map(([key, label]) => (
            <button key={key} onClick={() => toggleSort(key)} className={"rounded-lg border px-3 py-1 text-sm hover:bg-zinc-900 " + (sortKey === key ? "border-red-600 text-white" : "border-zinc-700 text-zinc-400")}>
              {label}{sortKey === key && <span className="ml-1 text-zinc-500">{sortDirection === "asc" ? "↑" : "↓"}</span>}
            </button>
          ))}
          <button onClick={() => setShowDebug((prev) => !prev)} className="rounded-lg border border-zinc-700 px-3 py-1 text-sm text-zinc-400 hover:bg-zinc-900">{showDebug ? "Hide Debug" : "Show Debug"}</button>
          <button onClick={copySeedCandidates} className="rounded-lg border border-zinc-700 px-3 py-1 text-sm text-zinc-400 hover:bg-zinc-900">Copy Seed Candidates</button>
        </div>

        {error ? <div className="mb-4 rounded-lg border border-red-800 bg-red-950 p-3 text-red-300">{error}</div> : null}
            ...
            <table className="w-full min-w-[1900px] border-collapse text-xs">
              <thead className="bg-zinc-900">
                <tr className="border-b border-zinc-800 text-left">
                  {["Player","Series (raw)","Season (mapped)","Edition Key","Parallel","Scope Key","Held","Locked","Badge Score","Badges","TS Ask","Flowty Ask","Best Market","Row Low Ask","Row Offer","Edition Low Ask","Edition Offer","Last Sale","FMV","FMV Method","Confidence","Reason"].map(h => <th key={h} className="p-2">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                ...
                return (
                  <Fragment key={row.momentId}>
                    <tr className={"border-b border-zinc-800 align-top " + (isLocked ? "opacity-60" : "")}>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap gap-1">
                              {officialBadges.map((badge) => <span key={"official-" + badge} className={"rounded px-2 py-0.5 text-[10px] font-semibold " + badgeClass(badge)}>{badge}</span>)}
                              {supaBadges.map((title) => <span key={"supa-" + title} className={"rounded px-1.5 py-0.5 text-[10px] font-semibold " + supadgePillClass(title)}>{title}</span>)}
                      ...
                      <td className="p-3">
                        <div className={"inline-flex min-w-[90px] flex-col rounded-lg border px-2 py-1 " + (primaryBadge ? "border-red-700 bg-red-950/50" : "border-zinc-800 bg-black")}>
                          <div className={"text-base font-black " + (primaryBadge ? "text-red-300" : "text-white")}>{"#" + (getSerial(row) ?? "-")}</div>
                          <div className="text-xs text-zinc-400">{"/ " + (getMint(row) ?? "-")}</div>
                          {primaryBadge ? <div className="mt-1 rounded bg-white px-1.5 py-0.5 text-[10px] font-bold text-black">{primaryBadge}</div> : null}
                        </div>
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <div className={"font-semibold " + (fmv.muted ? "text-zinc-500" : "text-white")}>{fmv.text}</div>
                        <div className={"text-[10px] " + conf.color}>{conf.label}</div>
                      </td>
                      <td className="p-3 text-zinc-300">{formatCurrency(row.bestOffer)}</td>
                      <td className="p-3">
                        <button onClick={() => toggleExpanded(row.momentId)} className="rounded-lg border border-zinc-700 px-3 py-1 text-xs text-white hover:bg-zinc-900">
                          {expanded ? "Hide" : "Show"}
                        </button>
                      </td>
                    </tr>
                            ...
                            <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-3">
                              ...
                              <div className="space-y-1 text-sm">
                                ...
                                <div className="flex flex-wrap gap-1 pt-1">
                                  {getTraits(row).map((trait) => <span key={trait} className="rounded bg-red-950 px-2 py-0.5 text-[10px] text-red-300">{trait}</span>)}
                                </div>
                              </div>
                            </div>
                                ...
                                <div className="space-y-1 text-sm">
                                  ...
                                  <div className="flex flex-wrap gap-1 pt-1">
                                    {(row.badgeInfo.badge_titles ?? []).filter(t => BADGE_PILL_TITLES.has(t)).map(title => (
                                      <span key={title} className={"rounded px-1.5 py-0.5 text-[10px] font-semibold " + supadgePillClass(title)}>{title}</span>
                                    ))}
                                  </div>

  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", paddingBottom: 80 }}>
      ...
      <main className="rpc-profile-main" style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 24px 60px", display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {/* ── Pin Your Collection (IPFS export) ── */}
        {groupedWallets.length > 0 && (
          <PinCollectionCard
            wallet={
              groupedWallets.find((g) => g.verifiedAt)?.addr ?? groupedWallets[0].addr
            }
          />
        )}
...
// PIN YOUR COLLECTION — per-wallet IPFS export. Dapper pinned the full Top Shot
// media corpus to IPFS (2026-06-08); /api/pin-list joins this wallet's holdings
// to the distinct CIDs that back them, so a collector can host their own copy.
// Fetches a lightweight summary on mount (a few seconds for big wallets — the
// rest of the dashboard has already loaded, so this never blocks). Downloads go
// straight through cookie-authed <a download> links to the txt/script formats.
function PinCollectionCard({ wallet }: { wallet: string }) {
  const [summary, setSummary] = useState<{
    cid_count: number;
    total_human: string;
    split: { video: { count: number }; artwork: { count: number } };
  } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/pin-list?wallet=" + encodeURIComponent(wallet), {
          cache: "no-store",
        });
        if (!res.ok) {
        ...
        setSummary({
          cid_count: d.cid_count,
          total_human: d.total_human,
          split: d.split ?? { video: { count: 0 }, artwork: { count: 0 } },
        });
        setState("ready");
      } catch {
        if (live) setState("error");
  ...
  // Hide entirely if this wallet has no pinnable Top Shot media — don't add
  // noise to a wallet that holds none.
  if (state === "empty" || state === "error") return null;

  const video = summary?.split?.video;
  const image = summary?.split?.artwork;
  const base = "/api/pin-list?wallet=" + encodeURIComponent(wallet);
      ...
      >
        {state === "loading" || !summary ? (
          ...
          <>
            ...
            >
              Your Top Shot media is{" "}
              <span style={{ fontFamily: condensedFont, fontWeight: 800, color: ACCENT_RED, fontSize: 16 }}>
                {summary.total_human}
              </span>{" "}
              across{" "}
            ...
            {(video || image) && (
              ...
              >
                {video ? `${video.count.toLocaleString("en-US")} video` : ""}
                {video && image ? " · " : ""}
                {image ? `${image.count.toLocaleString("en-US")} artwork` : ""}
              </div>
            )}
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <a href={base + "&format=txt"} download style={{ ...primaryBtnStyle, textDecoration: "none", display: "inline-block" }}>
                CID list (.txt)
              </a>
              <a href={base + "&format=script"} download style={{ ...secondaryBtnStyle, textDecoration: "none", display: "inline-block" }}>
                Pin script (.sh)
              </a>
              <Link
                href="/blog/permanent-moments-ipfs"
                style={{ ...linkBtnStyle, textDecoration: "underline", display: "inline-block" }}
              >
                What is this?
              </Link>
            </div>

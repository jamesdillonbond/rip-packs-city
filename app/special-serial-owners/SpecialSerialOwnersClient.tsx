...
export default function SpecialSerialOwnersClient() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--rpc-black)", color: "var(--rpc-text-primary)" }}>
      ...
      <main className="rpc-sso-main">
        ...
        {/* ⚠ EVERY KPI IS GATED ON `!error`, NOT ON `loading`. Until 2026-08-16 this band was
            ungated, so a failed read rendered "0 special serials" and "0 distinct holders"
            — claims about who holds the chase serials — while the list section DIRECTLY
            BELOW correctly said "Failed to load". The page contradicted itself on screen.

            The list's ladder was already right (error, then loading, then empty), which is
            exactly the trap: a page is not made honest by fixing the section that reports
            the failure. Every panel consuming the same failed read needs the same branch.

            ⚠ It is gated on `error` rather than on `rows.length`, because a REFRESH failure
            keeps the PREVIOUS rows — so a value-based guard would happily publish stale
            figures as current. Withheld, not zeroed: an em-dash is not a measurement. */}
        <section className="rpc-sso-kpi-row" aria-label="Summary">
          <div className="rpc-sso-kpi">
            ...
            <div className="rpc-sso-kpi-value">{error ? "—" : fmtInt(kpis.count)}</div>
          </div>
          <div className="rpc-sso-kpi">
            <div className="rpc-sso-kpi-label">Distinct holders</div>
            <div className="rpc-sso-kpi-value">{error ? "—" : fmtInt(kpis.holders)}</div>
          </div>
          <div className="rpc-sso-kpi">
            <div className="rpc-sso-kpi-label">Top edition FMV</div>
            <div className="rpc-sso-kpi-value">{error ? "—" : fmtMoney(kpis.topFmv)}</div>
          </div>
        </section>

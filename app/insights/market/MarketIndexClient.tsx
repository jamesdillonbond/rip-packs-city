...
// app/insights/market/MarketIndexClient.tsx
//
// Client interactivity layer for the public RPC Index (tier-segmented market
// index). The server component (page.tsx) fetches the trailing-120-day rows
// from topshot_market_index_daily server-side and passes them in as
// `initialRows`, so the per-tier headline cards + the inline-SVG index chart +
// the volume bars all render in the raw server HTML (crawlable) instead of only
// after JS — the chart is computed by pure functions from the rows, so it
// server-renders fully. The only client interactivity is the legend
// show/hide toggle. There are no fetch-changing filters, so this component
// never refetches; freshness is handled by the server page's revalidate.
...
export default function MarketIndexClient({ initialRows, initialFetchedAt }: Props) {
  ...
  const fetchedAt = initialFetchedAt
  ...
  return (
    <main style={styles.page}>
      ...
      <section className="rpc-mk-chart-wrap" aria-label="Daily volume">
        ...
        {built.volume.length === 0 ? (
          <div className="rpc-mk-state">No volume in range.</div>
        ) : (

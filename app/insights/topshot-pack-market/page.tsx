// app/insights/topshot-pack-market/page.tsx
//
// PUBLIC INSIGHTS — NBA Top Shot Pack Market (sealed-pack secondary resale).
//
// Server-rendered board: what a SEALED Top Shot pack actually trades for on the
// secondary market, vs its original retail price. Reads v_topshot_pack_market
// (complete Dapper Studio Platform sale history, backfilling to Top Shot's 2020
...
interface MarketRow {
  ...
  depletion_pct: number | string | null
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "var(--font-mono)", fontSize: 12 }}>
        ...
        <tbody>
          {rows.map((r) => {
            const opened = num(r.depletion_pct)
            return (
              <tr key={r.dist_id} style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                <td style={{ padding: "8px 10px" }}>
                  <Link
                    href={`/nba-top-shot/pack/dist/${encodeURIComponent(r.dist_id)}`}
    ...
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 18px 64px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <header style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <p style={{ margin: 0, fontFamily: "var(--font-mono)", fontSize: 13, color: "rgba(255,255,255,0.65)", lineHeight: 1.55, maxWidth: 720 }}>
          What a <strong>sealed</strong> NBA Top Shot pack actually trades for — and whether it resells above or below the
      ...
      {hasData ? (
        <>
          <Bucket
            ...
            blurb="Sealed packs reselling for the biggest discount to their original drop price — packs that settle below retail once the drop hype fades."
            rows={discount}
          ...
          <Bucket
            ...
            blurb="The packs the market pays a premium for — sealed resale above the original drop price, usually low-supply or chase-heavy pools."
            rows={premium}
      ...
      <footer style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "rgba(255,255,255,0.4)", lineHeight: 1.7 }}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 16, flexWrap: "wrap" }}>
          ...
          <a
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent("NBA Top Shot Pack Market — what sealed packs actually resell for, above or below retail.")}&url=${encodeURIComponent(`${SITE_URL}/insights/topshot-pack-market`)}`}
            target="_blank"

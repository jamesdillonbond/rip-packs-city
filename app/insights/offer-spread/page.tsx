...
//
// Public Bid vs Floor board. No auth. Top Shot editions where the highest
// standing offer (bid) meets or approaches the lowest ask (floor). The signal
// no native Top Shot surface shows — the top bid relative to the floor ask.
//
// HONEST FRAMING (per the "rank, not price" lesson): a bid at/above the floor
// can mean instant liquidity OR a stale / different-serial cheap listing. We
// show the floor ask next to the bid so the reader can judge. The board is
// gated to low_ask >= 5 by default so penny-floor ratio artifacts ($1 ask +
// $400 bid = 40000%) don't headline. NOT promoted as guaranteed arbitrage.
//
// Data source: GET /api/public/insights/offer-spread -> reads the public
// `topshot_offer_ask_spread` view (security_invoker=on, anon SELECT-only;
// backing edition_offers hardened 2026-06-01). Board view sends min_ask=5;
// player/set drill-downs send min_ask=0 so they never empty.
//
// Brand tokens only (the table SVG/OG hex exception does not apply here — no
// charts). Per-row link -> /nba-top-shot/edition/<external_id>.
...
export default function OfferSpreadPage() {

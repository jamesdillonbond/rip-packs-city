...
// app/insights/first-mint/FirstMintBoardClient.tsx
//
// Client interactivity layer for the public First-Mint Trophy Tracker. The
// server component (page.tsx) fetches the default-view cohort stats +
// multiplier-ranked trophies server-side and passes them in as `initial`, so
// the ranked table + per-row /nba-top-shot/edition/<id> drill-down links
// render in the raw server HTML (crawlable) instead of only after JS. This
// component layers on the multiplier / tier / drill-down filters as
...
export default function FirstMintBoardClient({ initial, initialDegraded = null }: Props) {
  ...
  // (no multiplier floor, all tiers, no drill-down).
  const isFirstRun = useRef(true)

  useEffect(() => {
    if (isFirstRun.current) {
      isFirstRun.current = false
      if (bucket === "ALL" && tier === "ALL" && !playerFilter && !setFilter) {
        return
      }
    }
    ...
    <main style={styles.page}>
      ...
      <section className="rpc-fm-hero">
        ...
        {/*
          ...
          snapshot AT ANY AGE — this board failed 63.7% of warm ticks over the 48h
          to 2026-08-21 and reached 336 min (5.6h) stale. degradedFromSource
          deliberately does NOT flag stale-cache, and its stated reason is that
          "the clients already surface as an age" — which was false here: the
          payload has carried meta.fetched_at since the board shipped and nothing
          rendered it. No banner AND no stamp means a reader cannot tell hours-old
          data from live.
...
.rpc-fm-meta-row { font-family: var(--font-mono); font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: var(--rpc-text-muted); margin-top: 14px; }
.rpc-fm-meta-sep { margin: 0 8px; color: var(--rpc-text-ghost); }

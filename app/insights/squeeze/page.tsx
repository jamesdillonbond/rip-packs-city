...
//
// Public lock-rate squeeze board. No auth. The single biggest "Top Shot's site
// won't tell you this" surface, per the 2026-05-29 research thread:
// nbatopshot.com shows you nominal circulation; we show you effective supply
// after locks and burns. Wemby Rookie Revelation = 81% locked, Origins rookies
// 60–65% locked, Jalen Brunson RIB:Origins 95% locked (12 of 229 actually
// purchasable).
//
// Data source: GET /api/public/insights/squeeze → reads the public
// `topshot_squeeze_board` view (shipped 2026-05-30 via
// audit_20260530_topshot_squeeze_board_view). View is filtered to
// squeeze_pct >= 50, joined to badge_editions (hourly refresh) + editions +
// latest fmv_snapshots; granted to anon.
//
// Anatomy, top to bottom:
//   1. Hero band (display H1 + lede)
//   2. Filter row: tier pills + max-buyable shortcut pills + sort dropdown
//   3. KPI strip (count + median squeeze + median buyable + total locked)
//   4. Sortable, click-to-deep-link table (per-row → /moment/<edition_id>)
//   5. Footer: methodology + "Share this" twitter intent button
//
// Brand tokens only. Per-row click goes to /moment/<edition_id> which the
// public moment page resolves as the edition UUID (aggregate view).

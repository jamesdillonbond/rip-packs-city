...
export default function EarlyAccessClient() {
  ...
  // Latest-value ref for the async on-chain probe below. `checkWalletOnChain`
  // closes over the `wallet` of the render that produced the blur handler, so
  // it cannot observe a later edit; this is the only view of the current value
  // an in-flight probe has.
  ...
  async function checkWalletOnChain() {
    ...
    try {
      ...
      //
      // ⚠ THIS READS THE REF, NOT `wallet`, AND THAT IS THE WHOLE GUARD. It was
      // written as `wallet.trim() === w`, which is a TAUTOLOGY: `w` is computed
      // from `wallet` at the top of this same function, and both are the
      // render-scoped const captured when this closure was created — so the
      // comparison is a value against itself and can never be false. The guard
      // read as present while doing nothing, so a slow probe about the OLD
      // address rendered as a verdict on the address now in the box: a false
      // claim about the user's own wallet, pointing them to "fix" a correct
      // one. The ref is the only thing here that sees the CURRENT value.
      if (typeof tm === "number" && tm === 0 && walletRef.current.trim() === w) {
        setWalletWarning(

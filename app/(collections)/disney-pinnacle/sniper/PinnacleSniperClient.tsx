...
export default function PinnacleSniperClient() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--rpc-black)", color: "var(--rpc-text-primary)" }}>
      ...
      {/* Stats bar
          ⚠ EVERY FIGURE HERE IS GATED ON `data`, NOT ON `loading`. Until 2026-08-16 this
          bar was ungated, so a failed FIRST load rendered "0 pins" and "FMV coverage: 0
          editions" — a claim about the Pinnacle market and a claim about our own pricing
          coverage, both manufactured out of our own outage, and both rendered ABOVE the
          FEED ERROR banner further down. The page contradicted itself on screen.

          The empty state below was already correct (`&& data`), which is exactly the trap:
          a page is not made honest by fixing the one component that failed. Every panel
          consuming the same failed read needs the same branch — the /insights/pack-reality
          lesson, met again here on a page no honesty guard covered, because that guard
          keeps a hand-picked list of sniper pages and this one was not on it.

          Withheld, not zeroed: an absent figure falls out of the bar entirely rather than
          being asserted as a measurement. */}
      <div style={{ borderBottom: "1px solid var(--rpc-border)", background: "var(--rpc-surface-raised)", padding: "8px 16px" }}>
        ...
        >
          ...
          {/* ⚠ `?? 0` here was the sharpest of the two: it is a claim about OUR OWN data —
              "we have priced 0 Pinnacle editions" — produced by a failed read of that very
              data. Withheld now, and deliberately NOT defaulted, so a real zero (which the
              API can legitimately return) and an unknown stay distinguishable. */}
          {data ? (
          <span>
            FMV coverage: <span style={{ color: "var(--rpc-text-primary)", fontWeight: 600 }}>{data.fmvCoverage}</span> editions
          </span>
          ) : (
            <span>FMV coverage —</span>
          )}
          {data?.lastRefreshed && (
      ...
      <div style={{ maxWidth: "var(--max-width)", margin: "0 auto", padding: "16px" }}>
          ...
          >
            ...
            {data && (
              <span style={{ display: "block", marginTop: 4, opacity: 0.85 }}>
                Showing the last successful read — the figures above are not current.
              </span>
            )}
          </div>
        )}
        ...
        >
          {/* ⚠ THE SECOND SITE, and it was found by the test written for the FIRST one.
              Fixing the stats bar alone left this legend still publishing "FMV coverage: 0
              editions" out of the same failed read — and this copy is worse, because it
              pairs the fabricated 0 with an EXPLANATION ("once ingest pipeline is active"),
              so a reader is told both a false number and a false reason for it.
              *Every panel consuming the same failed read needs the same branch.* */}
          {data ? (
            <span>FMV coverage: {data.fmvCoverage} editions — discount and FMV show once ingest pipeline is active</span>
          ) : (

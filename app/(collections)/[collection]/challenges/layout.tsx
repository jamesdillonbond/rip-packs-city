...
export async function generateMetadata(
  ...
  if (!collection) return unknownCollectionMetadata("challenges", id)
  return { alternates: { canonical: foldedTabCanonical("challenges", collection.id) } }
}
...
export default async function FoldedTabLayout(
  ...
  const { collection: id } = await props.params
  return <FeatureTabGate id={id} page="challenges">{props.children}</FeatureTabGate>
}

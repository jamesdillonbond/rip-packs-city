...
// tierColor / fmtUsd / fmtPct / buildCdf / sampleEdition / stddev extracted to
// @/lib/pack-simulator-math (unit-tested there).
...
export default function PackSimulatorClient({ collectionSlug, distId }: Props) {
  ...
  // ⚠ SEPARATE FROM `error`. `error` is only ever set by the catch, so a non-2xx
  // response left it null and fell through to the "not indexed" branch. This answers
  // "did the READ succeed", which is a different question from "what did it contain".
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok || json.error || !json.pack || !json.pool) {
          // A non-2xx, or a route-level `error`, is US failing — not a fact about
          // the pack. Only an OK response carrying an empty pool is the latter.
          if (!res.ok || json.error) setReadFailed(true)
          setPayload(json)
        } else {
          setPayload(json)
  ...
  if (error || !payload || payload.error || !payload.pack || !payload.pool || payload.pool.length === 0) {
    return (
      <div style={{ maxWidth: 640, margin: "40px auto", padding: 24, border: "1px solid var(--rpc-border, #27272a)", borderRadius: 10, background: "var(--rpc-surface, #0d0d0d)", textAlign: "center" }}>
        {/* ⚠ TWO DIFFERENT ANSWERS, AND THEY USED TO SHARE ONE. "Drop pool not indexed
          * — usually because it's sold out and being secondary-traded" is a specific
          * factual claim about THIS PACK, and it was rendered for a failed read too: a
          * 503 or a thrown fetch told a collector their pack is un-indexed and sold out,
          * manufactured entirely from our own outage. It is also a dead end — it says
          * the simulator will never work here, so they go away rather than retry. */}
        <div style={{ fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 800, color: "#fff", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 12 }}>
          {readFailed ? "Couldn\u2019t load this pack" : "Drop pool not indexed"}
        </div>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, lineHeight: 1.6, color: "rgba(255,255,255,0.65)" }}>
          {readFailed ? (
            <>We couldn&apos;t reach the drop pool for this pack just now. This is on us, not the pack — try again in a moment.</>
          ) : (
            <>

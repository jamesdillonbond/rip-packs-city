...
//
// The Disney Pinnacle per-render EDITION page body.
//
// ── WHY IT LIVES HERE NOW (2026-09-20) ──────────────────────────────────────
// It used to be a route of its own at `/pinnacle/moment/<render_id>` — the only
// published collection whose canonical entity page sat OUTSIDE the
// `(collections)/[collection]/...` namespace, under a slug (`pinnacle`) that is
// not the collection's URL slug (`disney-pinnacle`), named with a noun
// (`moment`) the page itself never used: its own metadata said "Pinnacle
// edition" and its legacy title said "N editions on ...". Meanwhile the
// house-correct URL, `/disney-pinnacle/edition/<render_id>`, existed and
// 308'd AWAY to it - the convention inverted.
//
// A Pinnacle render is the edition-grain object (2,600 of them, one per
// character x variant x printing), exactly what `/[collection]/edition/<slug>`
// means for the other four collections. So this is now that route's body for
// Pinnacle, and the old URL is a permanent redirect into it.
//
// ⚠ NO CHROME OF ITS OWN. `(collections)/layout.tsx` supplies the header,
// footer and concierge, and `[collection]/layout.tsx` adds the ticker, banner,
// tabs and the `<main>` wrapper. The old `PinnacleShell` rendered the first
// three itself and the body opened a second `<main>`; both are gone, so the
// page now gains the collection chrome every other edition page has instead of
// duplicating the outer shell.
//
// ── THE KEY SPACE (unchanged) ───────────────────────────────────────────────
//   [slug] = render_id (e.g. OEV1-SWHM-KYLO-S5) -> the per-pin detail page,
//      reading pinnacle_catalog (identity + per-render fmv_* + floor_ask + art)
//      + pinnacle_sales (history, by render_id) + wmc (tracked holders).
//   [slug] = legacy edition_key (e.g. STAR-OEV1-SWHM:Digital Display:1,
//      contains ':') -> a DISAMBIGUATION page listing every render that shared
//      that set-level key, so old links stay honest instead of silently showing
//      one arbitrary character's price.
//
// ⛔ This key space is NOT `get_edition_detail`'s, which is why the segment
// layout's existence gate skips Pinnacle and this module 404s for itself.
...
/** Metadata for a Pinnacle edition URL. Called by the shared edition route's
 *  `generateMetadata`, which owns the segment. */
export async function pinnacleEditionMetadata(id: string): Promise<Metadata> {
  ...
  if (data.kind === "legacy") {
    ...
    const canonical = `${SITE_URL}/disney-pinnacle/edition/${encodeURIComponent(id)}`
    return {
      title,
  ...
  const canonical = `${SITE_URL}/disney-pinnacle/edition/${encodeURIComponent(ed.render_id)}`
  return {
    title,
    description,
    alternates: { canonical },
...
/** The Pinnacle arm of `/[collection]/edition/[slug]`. `id` is the RAW slug -
 *  `load()` decodes it itself, so it must not be decoded twice. */
export default async function PinnacleEditionDetail({ id }: { id: string }) {
  const { data, ok } = await load(id)
...
function PinnacleShell({ children }: { children: ReactNode }) {
  // The chrome this used to render (GlobalSiteHeader / SiteFooter /
  // SupportChatConnected) is supplied by `(collections)/layout.tsx`. Kept as a
  // named pass-through rather than deleted so the three call sites stay
  // readable and a future wrapper has exactly one place to go.
  return <>{children}</>
}

// ⚠ minHeight / background / color / padding were dropped when this moved under
// the collection layout: that layout already paints the page and wraps children
// in `<main className="rpc-main">` with its own padding, so keeping them nested
// a full-viewport box inside a padded one and double-spaced the hero.
const pageStyle = {

              ...
              {sortedNotable.map(r => {
                ...
                return (
                  <div key={`${r.tag}-${r.serial}`} style={{ display: "grid", gridTemplateColumns: "minmax(0,140px) 70px 1fr 140px", gap: 12, alignItems: "center", padding: "8px 10px", border: "1px solid var(--rpc-border)", borderRadius: 4, minWidth: 460 }}>
                    ...
                    {owner ? <WalletLink address={owner} name={ownerNames.get(normalizeAddress(owner)) ?? null} collectionUrlSlug={collection} /> : <span className="rpc-mono" style={{ fontSize: 10, color: "var(--rpc-text-muted)", textAlign: "right" }}>owner —</span>}
                  </div>
                )
              })}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {topOwners.map((t, i) => (
              <div key={t.owner_address} style={{ display: "grid", gridTemplateColumns: "26px 1fr 96px", gap: 12, alignItems: "center", padding: "8px 10px", border: "1px solid var(--rpc-border)", borderRadius: 4 }}>
                ...
                <WalletLink address={t.owner_address} name={ownerNames.get(normalizeAddress(t.owner_address)) ?? null} collectionUrlSlug={collection} />
